...
/// 量 RegretTable::current_strategy_smallvec 单次 cost（6-action 典型）。
/// 用真实游戏 state 取一个 SimplifiedNlheInfoSet 模拟 trainer typical 调用。
#[test]
#[ignore = "diagnostic-only; release/--ignored opt-in"]
fn current_strategy_cost_6action() {
...
/// 量 sample_discrete 6-action 单次 cost。
#[test]
#[ignore = "diagnostic-only; release/--ignored opt-in"]
fn sample_discrete_6action_cost() {
