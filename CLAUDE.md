...
- **下一步**：**git tag `stage3-v1.0` 指向 F3 closure commit + 由用户授权后 milestone checkpoint artifact 上传 GitHub Release**（10M anchor checkpoint snapshot 通过 `cargo run --release --bin nlhe_blake3_anchor` 复跑 + GitHub Release artifact 上传由 gh CLI workflow 落地）。**Stage 4 起步**（6-max NLHE blueprint 训练 + nested subgame solving）+ §8.1 carve-out 并行清单 (I)..(VII) 推进。详见 `docs/pluribus_stage3_report.md` §11 + `docs/pluribus_stage4_6_path.md`。
