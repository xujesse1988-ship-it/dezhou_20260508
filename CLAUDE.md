...
Stage 1 of an 8-stage Pluribus-style 6-max NLHE poker AI. **Step E2 ([实现]) is done**（多核 host carve-out 同 E-rev0/D-rev0 形态保留）：E2 [实现] 把 E1 留下的 2/5 失败 SLO 断言（`slo_eval7_single_thread`、`slo_simulate_full_hand`）转绿，1-CPU host 上 5/5 SLO 全绿（多线程一条走 skip-with-log）。两个产品代码改动：(a) `src/eval.rs` 把 `NaiveHandEvaluator` 内核替换为 bitmask O(1) 评估（5 个 13-bit u16 mask + 阶梯位提升 `pair → trip → quad` + 8 KiB const `STRAIGHT_HIGH_TABLE`，零分配 / 零浮点 / 零 unsafe，`HandRank` 数值字节级与朴素实现一致）；(b) `src/rules/state.rs` 去掉 `GameState::apply` 的全状态克隆——`apply_inner` 各子路径已经是「先校验后变更」原子语义，I-005 不需要 clone-then-commit 兜底，simulate 热路径直接收益 ~4×。`tests/perf_slo.rs` / `tests/evaluator.rs` / `benches/baseline.rs` 等 [测试] 范畴文件**未修改一行**——5/5 SLO 全绿是 [测试] 在 E1 写好的断言被 [实现] 在 E2 写的产品代码满足，0 越界。详见 `docs/pluribus_stage1_workflow.md` §修订历史 E-rev1。**前置 E1 ([测试]) is done**（详见 §修订历史 E-rev0）。

E2 出口数据（截至本 commit；PATH 含 `.venv-pokerkit`/`python3.11` + PokerKit 0.4.14；本机 1-CPU AMD64 release profile）：

- `cargo test`（默认）：63 passed / 10 ignored / 0 failed across 13 crates；耗时 ~10s（E1/D2 时 ~50–60s）。100 手规则交叉验证 + 1k PokerKit category 交叉验证均 0 diverged。
...
Stages 2–8 source code does not exist yet. Next step：**F1 [测试]**（兼容性升级 + 错误路径测试；workflow §F1），随后 F2 [实现] + F3 [报告] 收尾 stage 1。在进入 F 系列之前的 follow-up 项（不阻塞 F1 起步）：(a) 完整 100k cross-validation 在多核 host 实跑产出 0 diverged 时间戳（D-rev0 / E-rev1 carve-out）；(b) 24h 夜间 fuzz 在 self-hosted runner 连续 7 天无 panic / 无 invariant violation（D2 出口）；(c) `slo_eval7_multithread_linear_scaling_to_8_cores` 在 ≥2 核 host 上跑出 efficiency ≥ 0.70 实测数据（E-rev0 carve-out，E-rev1 继承不变）。三项都与代码合并解耦。
...
When the user asks you to do stage-1 work, identify which step (A0 / A1 / B1 / …) the task belongs to and operate within that role. The current step is **F1 [测试] not started**：E2 [实现] 已闭合（详见 workflow §修订历史 E-rev1；本节上方 E2 出口数据）。历史关键边界事件：(1) B2 closure crossed the [实现]→[测试] boundary by completing two test files that B1 had deliberately left as stubs — see workflow §修订历史 B-rev1; (2) C2 closure carved out 「规则引擎 100k cross-validation 测试」 留给 [测试] agent — see §C-rev1; (3) D1 [测试] 实跑 100k cross-validation 暴露 105 条产品代码分歧 — see §C-rev2; (4) D2 [实现] 修完 105 条分歧 + 同 commit 翻新 2 条 scenario 测试到 D-037-rev1 语义；越界以 carve-out 追认 — see §D-rev0; (5) E1 [测试] 0 越界落地 bench harness + SLO 断言；多核多线程 SLO 在 1-CPU host 走 skip-with-log 留给多核 follow-up — see §E-rev0; (6) E2 [实现] 0 越界把 5/5 SLO 断言全部转绿（多线程在 1-CPU host 走 skip-with-log 不变）；apply 路径去 clone + 评估器换 bitmask 顺带让 1M fuzz / 1M determinism / 1M three-piece evaluator 等正确性测试加速 5–24× — see §E-rev1.
