...
- **Stage 4 A1..D1 closed 2026-05-14..2026-05-15**：A1 [实现] scaffold（6 新 module + 2 CLI stub + Cargo.toml +reqwest/serde_json/rayon + 5 既有 module additive 扩展 + api_signatures stage 4 trip-wire）；B1 [测试] 24 新 test 覆盖 D-401/402/403/409/422（warmup BLAKE3 anchor + 14-action raise sizes + RM/RM+ 数值单元）；B2 [实现] Linear MCCFR + RM+ + warm-up routing + Pluribus 14-action 全套落地，**§B2-revM carve-out** 用户授权 [测试]↔[实现] 边界破例追认（详 `pluribus_stage4_workflow.md` §修订历史）；C1 [测试] 3 新 integration crate 48 新 test 覆盖 D-410/411/412/414/416/420/422/423/449 + API-411/440/441/494；C2 [实现] NlheGame6 Game trait 8 方法 + 3 构造 + InfoSetId 14-action mask bits 33..47 + `impl ActionAbstraction for PluribusActionAbstraction` 全套翻面；D1 [测试] 2 新 integration crate（checkpoint_v2 round-trip 18 + training_24h_continuous 3 release/ignored）+ cfr_fuzz +6 stage 4 NlheGame6 fuzz + api_signatures 扩 v2 schema sanity，27 新 test 覆盖 D-445/449/461/431/412/401/402/409 + API-440/441，留 3 default-active panic-fail 作 D2 trip-wire。各步 5 道 gate 全绿；stage 1/2/3 baseline byte-equal 维持。**6 套独立 `[RegretTable; 6]` 扩展 deferred 到 D2** — C2 当前走 single shared RegretTable + `traverser = update_count % n_players` alternating（与 stage 3 SimplifiedNlheGame 路径字面等价），D-412 字面 6 套表扩展与 Checkpoint v2 sub-region encoding 在 D2 schema_version 1 → 2 bump 同 commit 落地（继承 stage 3 D-321-rev1 → D-321-rev2 deferred 同型模式）。
- **下一步**：**Stage 4 D2 [实现]** — `src/training/checkpoint.rs` SCHEMA_VERSION bump 1 → 2 + HEADER_LEN bump 108 → 128 + Checkpoint struct 加 4 pub 字段 `traverser_count: u8` / `linear_weighting_enabled: bool` / `rm_plus_enabled: bool` / `warmup_complete: bool`（D-449 字面）+ Checkpoint::save schema_version=2 路径写 v2 128-byte header（4 新字段 + regret_offset/strategy_offset u64 LE + pad_a/pad_b reserved 全 0 + trailer BLAKE3 over header+body）+ Checkpoint::open schema_version=1 vs 2 dispatch（schema=1 走 stage 3 既有 108-byte layout / schema=2 走 v2 128-byte layout）+ `EsMccfrTrainer<NlheGame6>` 走 `[RegretTable<NlheGame6>; 6]` + `[StrategyAccumulator<NlheGame6>; 6]` 数组 + 6-traverser alternating routing（D-321-rev2 rayon par_iter_mut 扩展到 6-player）+ save_checkpoint 在 `config.linear_weighting_enabled && config.rm_plus_enabled` 时走 schema_version=2 + TrainerVariant::EsMccfrLinearRmPlus + 6-traverser 数组 bincode 序列化 + load_checkpoint schema=2 路径反序列化 6 套表 + preflight 校验 4 新字段 + bucket_table_blake3 + `TrainerError::OutOfMemory` dispatch（A1 已落地 variant，D2 触发让 24h continuous run 短路）。**通过标准**：D1 24 release/--ignored opt-in 测试 D-491 AWS / vultr host 触发后全转绿 + D1 3 default-active panic-fail 全套翻面 + 3 anchor pass 持续维持 + B1 22 default-active + C1 30 default-active 全套不退化 + stage 1/2/3 baseline byte-equal 维持。D-416 HU 退化 1M × 3 BLAKE3 byte-equal anchor 由用户手动触发 vultr / AWS host 实测钉死。详见 `docs/pluribus_stage4_workflow.md` §D2。
