...
Stage 1 of an 8-stage Pluribus-style 6-max NLHE poker AI. **Step F2 ([实现]) is done**（0 越界，4/4 F1→F2 carry-over `--ignored` 全部翻绿）：F2 [实现] trade-off 选择 「错误前移到 from_proto」，仅触 `src/history.rs` 一个产品文件，新增 4 处 input 校验：(a) `config_from_proto` 加 `button_seat < n_seats` 校验；(b) `from_proto` 加 per-action `seat < n_seats` 越界扫描；(c) `from_proto` 加 board u64 mask uniqueness 检测（52-bit 掩码，零分配 / 零浮点 / 零 unsafe）；(d) 搭车加 `final_payouts[*].seat` + `showdown_order[*]` < n_seats 越界扫描，让 「decode 后 seat 全部合法」 成单点不变量。`tests/`、`benches/`、`fuzz/`、`tools/`、`proto/` **未修改一行**——F2 角色边界审计 0 越界。`cargo test -- --ignored` history_corruption 4/4 全绿（F1 出口为 1/4），`replay_diverged_when_board_swapped` / `_hole_cards_swapped` 在 seed 27/28 上未与 51↔0 swap 碰撞、default 全绿。`HandHistory.schema_version` 不 bump（序列化格式未动），公开签名不变（`HistoryError::Corrupted` 已在 API §8 列出），无需 D-NNN-revM / API-NNN-revM 入账。详见 `docs/pluribus_stage1_workflow.md` §修订历史 F-rev1。**前置 F1 ([测试]) is done**（详见 §修订历史 F-rev0）。

F2 出口数据（截至本 commit；PATH 含 `.venv-pokerkit`/`python3.11` + PokerKit 0.4.14；本机 1-CPU AMD64）：

- `cargo test`（默认 / debug profile）：**104 passed / 19 ignored / 0 failed across 16 test crates**（与 F1 出口同口径，校验加严不破坏默认 active 路径）；`replay_diverged_when_board_swapped` / `_hole_cards_swapped` 在 seed 27/28 上未与 51↔0 swap 碰撞，default 全绿。
...
- `cargo test --release --test history_roundtrip --test history_corruption --test cross_lang_history -- --ignored`：3 个 release ignored 套件全绿。`history_roundtrip_full_100k` 100,000/100,000 ok 2.86s；`cross_lang_full_10k` 10,000/10,000 ok 4.95s；`history_corruption` 4/4 0.45s。F2 校验在 100k+ 规模下无回归。
- `cargo fmt --all --check` / `cargo clippy --all-targets -- -D warnings` / `RUSTDOCFLAGS="-D warnings" cargo doc --no-deps`：全绿。
- F2 角色边界审计：仅 `src/history.rs` 单文件加 5 处校验 + `docs/pluribus_stage1_workflow.md` §F-rev1 + `CLAUDE.md` 状态同步；`tests/`、`benches/`、`fuzz/`、`tools/`、`proto/` **未修改一行**——0 越界（与 §C-rev1 / §E-rev1 「常规闭合 + 0 越界」 同型）。
...
Stages 2–8 source code does not exist yet. Next step：**F3 [报告]**（验收文档 + git tag；workflow §F3）—— 产出 `docs/pluribus_stage1_report.md`（测试手数 / 错误数 / 性能数据 / 随机种子 / 版本哈希 / 已知偏离）+ git tag `stage1-v1.0`，stage 1 闭合。stage-1 与代码合并解耦的 follow-up 项（不阻塞 F3 起步）：(a) 完整 100k cross-validation 在多核 host 实跑产出 0 diverged 时间戳（D-rev0 / E-rev1 carve-out）；(b) 24h 夜间 fuzz 在 self-hosted runner 连续 7 天无 panic / 无 invariant violation（D2 出口）；(c) `slo_eval7_multithread_linear_scaling_to_8_cores` 在 ≥2 核 host 上跑出 efficiency ≥ 0.70 实测数据（E-rev0 carve-out，E-rev1 继承不变）。三项都与代码合并解耦。
