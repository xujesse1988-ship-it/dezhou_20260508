...
- **下一步**：**Stage 4 F3 [报告]** — 等待用户授权 D-491 AWS c7a.8xlarge first usable `10⁹` update 训练（wall-time ~4-6h with §E-rev2 batch=8 实测 66K update/s × $1.63/h ≈ $6.85）启动后跑 LBR / Slumbot / baseline 评测，落地 `docs/pluribus_stage4_report.md` + `docs/pluribus_stage4_external_compare.md`（OpenSpiel LBR 对照 + Slumbot 公开评测对照）+ git tag `stage4-v1.0` + first usable blueprint checkpoint artifact 上传 GitHub Release（用户手动触发）。详见 `docs/pluribus_stage4_workflow.md` §F3。
