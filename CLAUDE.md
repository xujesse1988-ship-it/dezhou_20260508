...
Stage 1 of an 8-stage Pluribus-style 6-max NLHE poker AI. **Step F1 ([测试]) is done**（0 越界，三件套全部默认绿，4 条 F1→F2 carry-over 走 `#[ignore]`）：F1 [测试] 把 §F1 §输出 三件套（schema 版本兼容性 / corrupted history / 评估器 lookup-table 加载失败错误路径）落地为 3 个独立测试文件：(a) `tests/schema_compat.rs` 10 条全 active —— v1 round-trip baseline + mutate_schema_version 攻击 v0/v2/v999/u32::MAX/边界扫描 + PB-002 哨兵 + 空字节兜底；(b) `tests/history_corruption.rs` 23 active + 4 ignored —— 结构性 corrupted（2k byte flip / 全 prefix truncation / 1k random garbage / multi-byte flip）+ 16 条域违规（13 from_proto 直接拒绝 + 3 留 F2 前移 carry-over）+ 3 条 replay 语义 corrupted + 4 条边界 sanity；(c) `tests/evaluator_lookup.rs` 8 active —— 文件 doc-comment 落地「const-baked 8 KiB STRAIGHT_HIGH_TABLE → 加载失败路径结构性缺位」carve-out + (A) 结构性断言（`Default` / `Copy` / `Send + Sync`）+ (B) 1k×3 确定性扫描 + (C) wheel + 普通 9 档 + straight flush + 反向 case 完备性。`src/`、`benches/`、`fuzz/`、`tools/`、`proto/` **未修改一行**——F1 角色边界审计 0 越界。攻击 bytes 由测试文件 inline `mod mirror`（prost 派生镜像）+ inline varint 手术构造，不暴露 `src/history.rs::mod proto` 私有。详见 `docs/pluribus_stage1_workflow.md` §修订历史 F-rev0。**前置 E2 ([实现]) is done**（详见 §修订历史 E-rev1）。

F1 出口数据（截至本 commit；PATH 含 `.venv-pokerkit`/`python3.11` + PokerKit 0.4.14；本机 1-CPU AMD64 debug profile）：

- `cargo test`（默认）：**104 passed / 19 ignored / 0 failed across 16 test crates**；总耗时 ~55s（cross_validation 100-hand vs PokerKit dominates）。F1 三件套出口：`schema_compat` 10 active / 0 ignored；`history_corruption` 23 active / 4 ignored（4 条 = F1→F2 carry-over：3 条 from_proto 严校验前移候选 + 1 条 100k fuzz opt-in）；`evaluator_lookup` 8 active / 0 ignored。
...
Stages 2–8 source code does not exist yet. Next step：**F2 [实现]**（兼容性升级器 + 错误处理；workflow §F2），随后 F3 [报告] 收尾 stage 1。F2 自由 trade-off：根据 F1 落地的 4 条 `#[ignore = "F1 → F2"]` carry-over，决定是否在 `from_proto` 阶段提前拒绝 3 条域违规（action_seat / button_seat / duplicate_card 越界）+ 是否把 100k fuzz 接入 CI nightly。当前实现已**完全满足** validation §5 字面要求（明确错误、不静默错读、不 panic）；F2 改动是错误前移 / fuzz 规模扩展的优化项，可零产品代码闭合（同 §C-rev1 路径）也可触代码增补严校验。F2 闭合后进入 F3 验收报告 + git tag stage1-v1.0。stage-1 与代码合并解耦的 follow-up 项（不阻塞 F2 / F3 起步）：(a) 完整 100k cross-validation 在多核 host 实跑产出 0 diverged 时间戳（D-rev0 / E-rev1 carve-out）；(b) 24h 夜间 fuzz 在 self-hosted runner 连续 7 天无 panic / 无 invariant violation（D2 出口）；(c) `slo_eval7_multithread_linear_scaling_to_8_cores` 在 ≥2 核 host 上跑出 efficiency ≥ 0.70 实测数据（E-rev0 carve-out，E-rev1 继承不变）。三项都与代码合并解耦。
