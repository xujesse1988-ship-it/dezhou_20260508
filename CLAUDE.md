...
- **Stage 3 E2-rev1-vultr-measured carve-out 2026-05-14**（SLO 部分成功 / 双 fail，触发 E2-rev2 评估）：vultr 4-core EPYC 实测 6 SLO（commit `5c39989`）：(a) **NLHE 单线程 4,357 update/s**（E2 baseline 4,313 → +1.0%）< SLO 10K **FAIL**（43%）；(b) **NLHE 4-core 7,741 update/s**（E2 baseline 4,929 → +57%）< SLO 50K **FAIL**（15%）；(c) 4-core efficiency **1.78×**（baseline 1.14× → 改善 1.56×，但远低于 ideal 4×）；(d) 4 条非 NLHE SLO 全 PASS 不退化（Kuhn 10K iter 80K iter/s / Leduc 10K iter 293 iter/s / Kuhn BR 0.04 ms / Leduc BR 10.74 ms）。**E2-rev1 部分成功**：4-core scaling 改善（rayon + append-only delta 解了 D-321-rev1 sort merge 主导），但 SmallVec hot path 在单线程几乎无收益（+1%）→ 真实 single-thread 瓶颈在更深路径（`SimplifiedNlheState::clone()` 复制 `Vec<SimplifiedNlheAction>` / `canonical_observation_id` 重算不缓存 / `BucketTable::lookup` 528 MiB mmap 随机读 / `eval7` hand evaluator 每次重算）。**E2-rev1 未达 closure 标准**（SLO 双 fail），但代码 ship 保留作为渐进改善基线 + 4-core efficiency anchor，触发 E2-rev2 评估 deeper bottleneck 路径（state borrow 替 clone 需 Game::next 签名 break / canonical_observation_id 缓存 / bucket_table 预热 / sample_discrete CDF lookup-table 等），由用户授权进 E2-rev2 [实现] iteration。
- **下一步**：**用户授权决策**（E2-rev2 路径选择）— Option A：进 E2-rev2 [实现] 深度优化（state clone → Cow/borrow 改 Game::next 签名 = API-300-rev2，~1-2 周新工作量）；Option B：D-361-revM 翻面降低 SLO 阈值（与原 Pluribus path.md 字面冲突，需充分理由）；Option C：接受 E2-rev1 现状当 stage 3 出口 baseline，把 NLHE SLO 双 fail 列入 stage 3 报告 §8 已知偏离 + carry-forward 到 stage 4（继承 stage 2 §G-batch1 §3.4-batch2 deferred 同型 carve-out 模式）。
