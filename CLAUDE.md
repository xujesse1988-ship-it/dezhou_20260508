...
**Stage 2 B2 closed**（2026-05-09，本 commit）：让 B1 (B-rev0 + B-rev0 batch 2) 全绿，按 `pluribus_stage2_workflow.md` §B2 §输出 全部 5 类产品代码落地（详见 `pluribus_stage2_workflow.md` §修订历史 §B-rev1）：
...
- **`derive_substream_seed`** D-228 SplitMix64 finalizer + `BucketConfig::new` D-214 [10, 10_000] 校验 + `BucketTable::stub_for_postflop(BucketConfig)`（B-rev0 batch 2 carve-out option (1) 落地：cfg(test) 无关的 in-memory stub 路径，`lookup` 返回 `Some(0)`）+ `AbstractAction::to_concrete` API §7 字段提取桥接 + `InfoSetId` getters / `from_game_state` 桥接 / `pack_info_set_id` 整数 bit pack helper（位于 `abstraction::map` 子模块顶 `#![deny(clippy::float_arithmetic)]`，D-252 锁死浮点边界）。

**stage 1 [实现] 越界 carve-out（API-004-rev1）**：B2 [实现] 在 `InfoAbstraction::map` 落地 `stack_bucket` D-211-rev1 协议时发现 stage 1 `GameState` 未公开 `config(&self) -> &TableConfig` getter，按 stage-2 API §F21 carve-out 字面 "B2 [实现] 在落地实际逻辑时若 stage 1 GameState getter 缺位，同 PR 触发 stage 1 `API-NNN-revM`" 显式触发——同 commit 在 `pluribus_stage1_api.md` §11 修订历史新增 `API-004-rev1`（additive 只读 getter）+ `src/rules/state.rs` 加 `pub fn config(&self) -> &TableConfig` 单行实现。

**[测试] 角色越界 carve-out 4 处**（继承 stage-1 §B-rev1 §3 / §B-rev0 batch 2 carve-out 1+2）：本 commit 触 `tests/`：(1) 取消 12 条 C 类 equity `#[ignore]`（`MonteCarloEquity` 落地后 12 条断言全绿）；(2) 取消 2 条 D 类 D-228 `#[ignore]`（`derive_substream_seed` 落地后 SplitMix64 byte-equal + 32 sub_seed 区分性两条全绿）；(3) 取消 1 条 `info_abs_postflop_bucket_id_in_range` `#[ignore]` 并填充测试体（用 `BucketTable::stub_for_postflop` 构造路径）；(4) 修订 `bet_ratio_from_f64_half_to_even` 1 条断言（B-rev0 batch 3 carve-out：原期望 0.5015 → 502 与 IEEE-754 实际 `0.5015 * 1000.0 = 501.4999...` 不一致，修正为 501）。详见 `pluribus_stage2_workflow.md` §修订历史 §B-rev1。

B2 出口数据（截至本 commit；本机 1-CPU AMD64 debug profile）：

- `cargo fmt --all --check` ok / `cargo build --all-targets` ok / `cargo clippy --all-targets -- -D warnings` ok（含 1 处 `#[allow(clippy::incompatible_msrv)]` 在 `BetRatio::from_f64` 上；项目 `Cargo.toml` `rust-version = "1.75"` 是保守 metadata，`rust-toolchain.toml` 实际 pin 到 1.95.0）/ `RUSTDOCFLAGS="-D warnings" cargo doc --no-deps` ok。
- `cargo test --no-run` 编译 22 test crates 成功，`tests/api_signatures.rs` trip-wire byte-equal 不变，stage 2 公开 API 0 签名漂移。
- `cargo test --no-fail-fast`（默认）：**154 passed / 21 ignored / 0 failed across 22 test crates**。其中：
    - **stage-1 baseline 16 crates 维持** `104 passed / 19 ignored / 0 failed`，与 stage1-v1.0 tag **byte-equal**（D-272 不退化要求满足）。
    ...
    - 实测耗时（debug profile）：equity_self_consistency 145s 主导（`equity_determinism_repeat_1k_smoke` 10M MC iter + `ehs_squared_finite_range_smoke` flop 10.8M MC iter；release profile 预计 < 5s，E2 SLO 路径接管）。
- B2 §出口标准 三条全部满足：line 280 "B1 全部测试通过（含 preflop 169 lossless 1326 → 169 枚举）" ✓；line 281 "equity Monte Carlo 反对称误差在 D-220 容差内"（postflop river/turn/flop 1e-9 + preflop strict dual RngSource 1e-9 + preflop noisy 10k 0.005 + 1k 0.02 全绿）✓；line 282 "阶段 1 全套测试仍 0 failed" ✓。

B2 角色边界审计：仅触 `src/abstraction/`（8 文件 stub fill）+ `src/rules/state.rs`（API-004-rev1 单行 getter）+ `tests/`（4 处 carve-out：3 个文件取消 ignore + 1 文件填充测试体 + 1 文件 IEEE-754 断言修正，`tests/api_signatures.rs` 等 21 文件未修改）+ `docs/pluribus_stage1_api.md` §11 新增 API-004-rev1 + `docs/pluribus_stage2_workflow.md` §修订历史 §B-rev1 + `CLAUDE.md` 状态翻 "stage 2 B2 closed"。`src/lib.rs` / `src/core/` / `src/eval.rs` / `src/history.rs` / `src/error.rs` / `proto/` / `benches/` / `fuzz/` / `tools/` / `Cargo.toml` / `Cargo.lock` **未修改一行**——B2 carve-out 限定在上述 4 处 [测试] + 1 处 stage 1 API rev 内（继承 stage-1 §D-rev0 / §B-rev1 §3 carve-out 形态）。

**下一步：stage 2 C1 [测试]**（postflop 聚类质量测试）— 按 §C1 §输出 落地 `tests/bucket_quality.rs`（500/500/500 配置下每条街每 bucket 至少 1 个 canonical sample / EHS std dev < 0.05 / 相邻 bucket EMD ≥ T_emd / bucket id ↔ EHS 中位数单调一致）+ `tests/equity_features.rs`（EHS² / OCHS 自洽）+ `tests/scenarios_extended.rs` 阶段 2 版扩到 200+ 固定 GameState 场景（API §F20 影响 ② 字面要求 ≥ 2 条 all-in call 场景）+ `tools/bucket_quality_report.py`。预期 C1 [测试] 闭合后暴露 N 条 fail（B2 stub `bucket_id = 0` 不可能过 EHS std dev / EMD / 单调性等质量门槛，留 C2 修复）+ preflop 169 lossless 全套保持全绿（C1 不动 preflop 信任锚）。
