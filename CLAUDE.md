...
- **下一步**：**Stage 4 F1 [测试]** — 钉死 Slumbot 100K 手评测协议 + 1M 手 vs 3 类 baseline + corner case + cross-host BLAKE3 baseline。产出测试 `tests/slumbot_eval.rs` 6 条（D-460/461/463/468/469） + `tests/baseline_eval.rs` 12 条（D-480 3 baseline × 4 metric） + `tests/cross_host_blake3.rs` 扩展 stage 4 baseline `tests/data/checkpoint-hashes-linux-x86_64-stage4.txt` 32-seed × 6-traverser × first usable 10⁵ checkpoint anchor + `tests/api_signatures.rs` 扩展 API-450..API-499 全套 trip-wire。**通过标准**：default profile 30 测试 panic-fail（F2 实现后转绿）+ 8 active anchor 测试通过（Cross-host baseline 32-seed × stage 4 不退化 + api_signatures 全套）。详见 `docs/pluribus_stage4_workflow.md` §F1。

  **E2 → F1 carve-out 状态**：(i) **D-401-revM lazy decay 仍 deferred** — E2 commit 维持 `DecayStrategy::EagerDecay` 主路径（`TrainerConfig::default()`）；lazy decay 翻面条件 = "E1 SLO ① 单线程 < 5K update/s 实测 fail" 由用户授权 AWS c7a.8xlarge 实测触发后决定（实测 8.4K → 10.9K ≫ 5K SLO,**不翻面** carry-forward stage 4 F-rev / F3 报告）。(ii) **OOM step-path dispatch 仍 deferred** — `TrainerError::OutOfMemory` variant A1 已落地，E2 训练循环未接入 runtime trigger（D2 carve-out 字面延续，F2 [实现] metrics 接入后实际触发 dispatch）。
