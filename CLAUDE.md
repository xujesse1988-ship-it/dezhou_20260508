...
- **下一步**：**Stage 4 B1 [测试]** — `tests/nlhe_6max_warmup_byte_equal.rs` 5 测试（D-409 warm-up phase 1M update BLAKE3 anchor / warm-up boundary deterministic byte-equal / warm-up → Linear+RM+ 切换数值连续 / Linear weighting cumulative formula 单元 / RM+ clamp R_t-1 < 0 boundary 单元）+ `tests/nlhe_6max_raise_sizes.rs` 14 测试（每 PluribusAction 一条走 stage 1 GameState::apply byte-equal regression / D-422）+ `tests/regret_matching_numeric.rs` 扩 5 条 Linear weighted regret / RM+ clamp 数值容差测试。default profile 全 23 测试 panic-fail（B2 [实现] 落地后转绿）+ 1 active anchor。详见 `docs/pluribus_stage4_workflow.md` §B1。
