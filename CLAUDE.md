...
**§E-rev1 §5 carve-out（host-load 敏感的单线程 SLO）→ closed 2026-05-11**：D-282 字面 «10k iter × 1k hand/s = 10M eval/s = stage-1 SLO 10M eval/s» 在 hero-rank precompute 后从 «2 × eval/iter» 路径降至 «1 × eval/iter + 1 × table-lookup» 路径，与 D-282 footnote 字面一致。主 1-CPU host with claude background ~16% CPU 实测 821-1059 hand/s 区间（10 次 mean 931 / peak 1059）borderline；按 stage-1 §E-rev0 carve-out 同形态书面记录 + idle host 复跑闭合。**closure 实测**（vultr 4-core EPYC-Rome / 7.7 GB / Linux 5.15 idle box，load 0.00）：50-run aggregate `mean 1102.1 hand/s / std 10.6 / min 1061.7 / max 1117.3 / 50/50 PASS 100%`，最差一次仍超 1k 门槛 6.2%，原 carve-out 实质是「测量条件 host-load contention」而非「实现不达标」（与 §D-rev1 §3 cross_arch_bucket_id_baseline 实跑闭合 §D-rev0 §4 carve-out 同型「实测后追认」）。F3 [报告] 不需要再为 D-282 单独复跑——§E-rev1 §5 closure 段直接作为 F3 SLO 出口数据来源。残留：vultr 共享 vCPU 同时段实测 stage-1 `slo_eval7_multithread_linear_scaling_to_8_cores` efficiency 0.38 不达 0.70，stage-1 follow-up (c)「≥ 2 核 host」carve-out 仍待 bare-metal host（Hetzner AX 一类）才能闭合。
...
- `cargo test --release --test perf_slo -- --ignored --nocapture stage2_`：**3 hard pass on idle host**（详细见 §E-rev1 §3 表 + §5 closure）。(1) `stage2_abstraction_mapping` **PASS** 31 803 162 mapping/s（318× over 100k 门槛，vs §E-rev0 baseline 16M+ mapping/s ~2× 加速受 hero-rank precompute 路径间接影响 hot loop ILP）；(2) `stage2_bucket_lookup` **PASS** P50=91 ns / P95=131 ns / P99=180 ns（76× under 10 μs 门槛，vs §E-rev0 baseline P95=188 ns ~30% improvement 受 inline(always) eval7 路径间接加速）；(3) `stage2_equity_monte_carlo` **PASS on idle host**（vultr 4-core EPYC-Rome idle 50-run aggregate `mean 1102.1 / std 10.6 / min 1061.7 / max 1117.3 hand/s / 50/50 PASS`，最差一次超 1k 门槛 6.2%；主 1-CPU host with claude background 821-1059 hand/s 区间 borderline，§E-rev1 §5 carve-out closed 2026-05-11）。
