...
8-stage Pluribus-style 6-max NLHE poker AI。**Stage 1 closed**（git tag `stage1-v1.0`，验收报告 `docs/pluribus_stage1_report.md`）；**Stage 2 closed**（git tag `stage2-v1.0`，验收报告 `docs/pluribus_stage2_report.md`，A0..F3 全 13 步 closed）。**Stage 3 起步 batch 1 §G-batch1 §1..§3.3 closed 2026-05-11 + §3.4-batch1 closed 2026-05-12**（D-218-rev2 决策 + 5 条契约测试 + canonical_enum.rs 算法 + N 修正 + postflop forward + bucket_table v2 schema + fixture n_train K×100 cap + `TrainingMode` enum + `--mode` CLI flag + doc-comment drift carry-forward + `nth_canonical_form` 逆函数 + Production dual-phase 训练实现 + phase 1/2 progress logging + vultr 5 gate 全绿 + byte-equal Fixture artifact 验证）；下一步 §G-batch1 §3.4-batch2 [实现]（production artifact 重训 ~120 min release on vultr with realtime progress log + artifact BLAKE3 录入 + GitHub Release 上传 + `tools/fetch_bucket_table.sh` helper；详见下文 §下一步：Stage 3 起步 batch 1）。
