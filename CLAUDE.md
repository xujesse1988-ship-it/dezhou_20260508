...
- **下一步**：**Stage 3 F2 [实现]** — doc drift 修复 + 0 产品代码改动 carve-out closure（继承 stage 2 §F2 字面模式）：(a) 修 1 条已知 doc drift `pluribus_stage3_api.md` §API-313 ⑤ 变体名 doc=`CheckpointError(#[from] CheckpointError)` → code=`Checkpoint(#[from] CheckpointError)`；(b) `pluribus_stage3_workflow.md` 字面 F2 [实现] 步骤；(c) F2 closure 后 → F3 [报告]（stage 3 验收报告 + git tag stage3-v1.0 + checkpoint artifact 上传 GitHub Release）。
