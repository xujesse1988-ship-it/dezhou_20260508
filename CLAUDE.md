...
### 下一步：Stage 3 A1 [实现] scaffold

按 `pluribus_stage3_workflow.md` §步骤 A1 字面 deliverables：

(a) `src/training/` 9 文件骨架（`mod.rs` / `game.rs` / `kuhn.rs` / `leduc.rs` / `nlhe.rs` / `regret.rs` / `trainer.rs` / `sampling.rs` / `best_response.rs` / `checkpoint.rs`），所有公开方法体 `unimplemented!()` 占位；
(b) `src/error.rs` 追加 `CheckpointError` + `TrainerError` 枚举（继承 stage 1 + stage 2 错误追加不删模式）；
(c) `src/lib.rs` 追加 `pub mod training;` + re-export（API-380）；
(d) `tools/train_cfr.rs` CLI 骨架 + `Cargo.toml [[bin]]` 入口；
(e) `Cargo.toml` 新增 3 个依赖（`bincode 1.3` / `tempfile 3` / thread-safety TBD 由 D-321 batch 3 [实现] 之前 lock）；
(f) `tests/api_signatures.rs` 扩展 stage 3 API surface trip-wire（继承 stage 2 §A1 模式，签名漂移 `cargo test --no-run` fail）；
(g) `cargo build --all-targets` / `cargo fmt --all --check` / `cargo clippy --all-targets -- -D warnings` / `cargo doc --no-deps` 全绿；
(h) doc-test 占位走 `#[doc(hidden)]` 至 B2 / C2 [实现] 落地后翻面（API-378）。

A1 不引入任何业务逻辑，仅 trip-wire；B1 [测试] 起步前 stage 1 + stage 2 baseline 不退化（continue passing `stage1-v1.0` + `stage2-v1.0` tag tests byte-equal）。
