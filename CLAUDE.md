...
- **下一步**：**Stage 4 A1 [实现] scaffold** — `src/training/{nlhe_6max,lbr,slumbot_eval,baseline_eval,metrics}.rs` + `src/abstraction/action_pluribus.rs` 6 新 module unimplemented stub + `tools/{lbr_compute,eval_blueprint}.rs` 2 CLI scaffold + `Cargo.toml` 追加 rayon + reqwest + serde_json 3 crate + `src/lib.rs` 7 新 re-export + `tests/api_signatures.rs` 扩 API-400..API-499 trip-wire；通过 `cargo build/fmt/clippy/doc` 全绿（5 道 gate）。详见 `docs/pluribus_stage4_workflow.md` §A1。
