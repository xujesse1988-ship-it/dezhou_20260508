...
- **Stage 5 A0 [决策] closed 2026-05-16**：4 docs 落地（`pluribus_stage5_{validation,decisions,api,workflow}.md` 共 ~1100 行 + D-500..D-599 + API-500..API-599）。**核心 lock**：(a) **训练吞吐 SLO 200K update/s @ AWS c6a.8xlarge 32-vCPU 硬钉死**（D-530，3 trial min ≥ 200K acceptance；baseline ref = stage 4 §E-rev2 c7a 85K → c6a 等效 ~72-75K，gap 2.67-2.78× stretch goal）；(b) **内存 SLO RegretTable + StrategyAccumulator section ≤ baseline × 0.5**（D-540，path.md §5 #4 字面）；(c) **update metric 计量语义 = sampled-decision node visit**（D-504，pruning 不为 update/s 贡献，服务 wall-time + path.md §5 字面 + D-550 ablation）；(d) **测试 host c6a.8xlarge on-demand 总预算 ≤ $150**（D-590 含 30% safety margin）；(e) **stage 3 D-350+ + stage 4 D-409 BLAKE3 cross-version anchor 翻面失效**（D-508 + D-549，`#[ignore = "§stage5-rev0"]` 归档）+ D-560..D-563 4 新 anchor 取代（LBR ±5% + baseline 3 类 mean delta + Slumbot 95% CI overlap + Checkpoint round-trip self-consistency）；(f) **5 优化实施顺序 + gate**（D-570..D-576 — A 紧凑 array +30%/gate 20% → B q15 quantization +20%/gate 12% → C SoA + AVX2 +20%/gate 12% → D bucket layout lightweight +15%/gate 8% → E rayon +8%/gate 4%；连 2 fail 强制 §X-revN carve-out）；(g) **紧凑 RegretTable = Open-addressed Robin Hood + FxHash + load factor 0.75 + SoA 三 Vec + capacity 2^20 起步**（D-510）；(h) **q15 per-row scale 36 byte/row + scale-only lazy decay + 1e6 iter renorm**（D-511，stage 4 carry-forward P2 D-401-revM 翻面关闭）；(i) **256 shard 高 8 bit InfoSetId 路由 + first usable RAM / production mmap + LRU 128 pin**（D-512，production 10¹¹ 估 432 GB 必走分片）；(j) **AVX2 cfg-gate 3 hot path SIMD + scalar fallback**（D-513）；(k) **bucket layout = lightweight 路径不重训 v4**（D-514 维持 v3 BLAKE3 anchor `67ee5554...`）；(l) **pruning 阈值 regret_f32 < -300M 绝对**（D-520 Pluribus §S2 字面）+ **ε resurface 周期 1e7 iter / 比例 0.05 / reset -150M**（D-521）+ **warm-up 1M update 同步切 Linear + RM+ + pruning**（D-522）；(m) **Checkpoint schema_version 2 → 3 不向前兼容 + HEADER_LEN 128 → 192 byte + 8 新 header field + 6×2 sub-region bincode+zstd + body BLAKE3 self-consistency**（D-549）；(n) **TrainerVariant::EsMccfrLinearRmPlusCompact = 4 + EsMccfrLinearRmPlusCompactTrainer 完整 trait impl**（API-540..API-559）；(o) **`tools/perf_baseline.rs` 新 binary 16 CLI flag + preflight check + 3-trial aggregate + AcceptanceSummary**（API-580..API-589，独立 binary 不合并 train_cfr）；(p) **3 新 integration test crate**：`checkpoint_v3_round_trip` / `stage5_anchors` / `regret_table_compact_collision`（API-596..API-598）。**stage 4 carry-forward 9 项最终分流**（详 `pluribus_stage5_workflow.md` §4）：2 closed（D-514 v4 不重训 / D-401-revM lazy decay）+ 3 stage 5 主线触发（production 10¹¹ 训练 F2 / LBR 收敛 F3 / 100 采样点 F3）+ 2 并行清单不阻塞（200 BB HU 重训 / OpenSpiel LBR）+ 2 carry-forward 到 stage 6（nested subgame skeleton / stage 3 §8.1 7 项）。**4 batch 落地 commit**：c2fa4f4（batch 1）/ 63154ec（batch 2）/ 40f9b3b（batch 3）/ 本 commit（batch 4 A0 closure）。

- **Stage 5 A1 [实现] scaffold closed 2026-05-16**：单 commit 落地 — 4 新 module（`src/training/{regret_compact,quantize,shard,pruning}.rs`）+ `trainer.rs` additive 扩展（`Trainer` trait 加 `regret_table_compact_opt` / `strategy_accum_compact_opt` 默认 `None` + `EsMccfrLinearRmPlusCompactTrainer<G>` 字段集字面锁 + 7 必实现方法 + 3 builder + 9 getter 全 `unimplemented!()` 占位）+ 2 binary（`tools/perf_baseline.rs` 全新 stub stderr 提示 + `[[bin]]` 注册 / `tools/train_cfr.rs` 加 5 stage 5 CLI flag parse-only 经 stage 5 path guard 触发 deferred 错误）+ 既有 module additive 扩展（`checkpoint.rs` 加 `SCHEMA_VERSION_V3=3` / `HEADER_LEN_V3=192` / `STAGE3_HEADER_LEN` / `STAGE4_HEADER_LEN` 常量 + `CheckpointHeaderV3` struct 16 字段字面锁 + `ensure_trainer_schema` preflight 函数实落地 + `TrainerVariant::from_u8(3)` 识别 / `error.rs` 加 `TrainerVariant::EsMccfrLinearRmPlusCompact = 3` + `expected_schema_version` const fn / `metrics.rs::TrainingMetrics` 加 14 stage 5 新字段全 `0` 起步 + `MetricsCollector::sample_throughput_window` / `record_warm_up_complete` stub）+ `tests/api_signatures.rs` 扩 `_stage5_api_signature_assertions()` 全 API-500..API-599 trip-wire（含 `PruningConfig::default()` 4 字段值 runtime assert -300M / 1e7 / 0.05 / -150M）+ Cargo.toml +memmap2 0.9 + rustc-hash 2 dep + `[[bin]] perf_baseline`。**5 道 gate 全绿**（fmt / build --all-targets / clippy -D warnings / RUSTDOCFLAGS=-D warnings cargo doc / cargo test --no-run）+ stage 1+2+3+4 baseline byte-equal 维持（stage 2 bucket_quality 9 known fail 不退化）+ A1 stub 全返 `!`（错签名 silently compile fail 由 api_signatures 捕获）。

- **下一步**：**Stage 5 B1 [测试]** — ≥ 20 新 unit test（紧凑 RegretTable Robin Hood collision + q15 quantization + StrategyAccumulator）+ ≥ 5 pruning unit test（D-527 字面）+ D-530/D-540 SLO assertion harness `#[test] #[ignore]` opt-in + D-560..D-563 4 anchor assertion harness + 3 新 integration crate scaffold（`tests/checkpoint_v3_round_trip.rs` / `tests/stage5_anchors.rs` / `tests/regret_table_compact_collision.rs`）。详 `pluribus_stage5_workflow.md` §10 B1 entry/exit。first usable blueprint checkpoint artifact 上传 GitHub Release `stage4-v1.0` 由用户手动触发（`gh release upload stage4-v1.0 artifacts/.../nlhe6max_linear_rm_plus_t00000000001000000000_final.ckpt`，95.7 MB << 2 GB GitHub limit）。
...
1. `docs/pluribus_stage{1,2,3,4}_report.md` — 各 stage 验收报告 + carve-out 索引（stage 5 F3 报告闭合后生成）。
2. `docs/pluribus_stage{1,2,3,4,5}_workflow.md` §修订历史 — 所有 §X-revN carve-out 全文（[测试]↔[实现] 越界追认、D-NNN-revM 翻语义、错误前移、procedural follow-through 等）。
3. `git log --oneline stage1-v1.0..` — stage-2 + stage-3 + stage-4 + stage-5 实施提交时间线。
...
5. Stage 4（locked at `stage4-v1.0`）：`pluribus_stage4_{validation,decisions,api,workflow}.md`（D-400..D-499 + API-400..API-499）。
6. Stage 5（A0 closed 2026-05-16）：`pluribus_stage5_{validation,decisions,api,workflow}.md`（D-500..D-599 + API-500..API-599）。
...
- **`tests/api_signatures.rs` is the spec-drift trip-wire.** A1 stubs 返回 `!` 与任意返回类型 unify — 否则错签名 silently compile。任何 `pluribus_stage{1,2,3,4,5}_api.md` 签名改动（via API-NNN-revM）必须同 PR 更新此文件；否则 `cargo test --no-run` fail。
