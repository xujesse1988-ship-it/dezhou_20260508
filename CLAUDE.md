...
- **下一步**：**Stage 3 F3 [报告]** — stage 3 验收报告 + git tag `stage3-v1.0` + checkpoint artifact 上传 GitHub Release：(a) 起草 `docs/pluribus_stage3_report.md`（§1..§11 完整，含 §8 D-361 NLHE 双 SLO 已知偏离 + carry-forward 到 stage 4 起步并行清单）；(b) vultr 4-core EPYC 实测 stage 3 6 SLO 完整数字 + Kuhn 1000× / Leduc 10× / NLHE 3× BLAKE3 byte-equal + 简化 NLHE 100M update D-362 BLAKE3 anchor；(c) OpenSpiel 收敛轨迹趋势对照（D-366 `tools/external_cfr_compare.py` 一次性 instrumentation + `docs/pluribus_stage3_external_compare.md`）；(d) `tools/checkpoint_reader.py` D-357 Python reader 落地；(e) git tag `stage3-v1.0` + Kuhn / Leduc / 简化 NLHE milestone checkpoint artifact 上传 GitHub Release。详见 `docs/pluribus_stage3_workflow.md` §步骤 F3。
