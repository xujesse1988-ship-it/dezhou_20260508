...
- **下一步**：**Stage 4 F3 [报告]** — §F2-revM commit 后立即触发 (a) AWS c7a.8xlarge 32-vCPU host 配置（rust 1.95.0 + clone repo + scp v3 artifact 528 MiB）；(b) `target/release/train_cfr --game nlhe-6max --updates 1000000000 --threads 32 --parallel-batch-size 8 --abort-on-alarm p0` 后台启动（wall-time ~4.2h × $1.63/h ≈ $6.85，FIXED_SEED `0x53_54_47_34_5F_46_33_18` = ASCII "STG4_F3\x18"）；(c) 训练完成后 `lbr_compute --six-traverser` + `eval_blueprint` 跑 LBR / Slumbot / baseline 三轨评测；(d) 落地 `docs/pluribus_stage4_report.md` + `docs/pluribus_stage4_external_compare.md` + git tag `stage4-v1.0` + first usable blueprint checkpoint artifact 上传 GitHub Release（用户手动触发）。详见 `docs/pluribus_stage4_workflow.md` §F3 + §F2-revM。
