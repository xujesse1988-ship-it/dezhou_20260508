...
8-stage Pluribus-style 6-max NLHE poker AI。**Stage 1 closed**（git tag `stage1-v1.0`，验收报告 `docs/pluribus_stage1_report.md`）；**Stage 2 closed**（git tag `stage2-v1.0`，验收报告 `docs/pluribus_stage2_report.md`，A0..F3 全 13 步 closed）。**Stage 3 起步 batch 1 §G-batch1 §1..§3.4-batch1 closed 2026-05-11..2026-05-12**（D-218-rev2 真等价类 + canonical_enum.rs + N 实测修正 + postflop forward + bucket_table v2 schema + fixture n_train K×100 cap + `TrainingMode` enum + `--mode` CLI flag + `nth_canonical_form` 逆函数 + Production dual-phase 训练 + phase 1/2 progress logging + vultr 5 gate 全绿 + byte-equal Fixture artifact）。**Stage 3 A0 [决策] 6-batch closed 2026-05-12**（4 docs 落地：`pluribus_stage3_validation.md` 179 行 + `pluribus_stage3_decisions.md` 310 行 D-300..D-379 + `pluribus_stage3_api.md` 838 行 API-300..API-392 + `pluribus_stage3_workflow.md` 441 行 13-step test-first；总 1768 行；用户授权 stage 3 [决策] 优先于 §G-batch1 §3.4-batch2..§4 closure 启动 carry-forward carve-out）。下一步 **Stage 3 A1 [实现] scaffold**（`src/training/` 9 文件骨架 + `tools/train_cfr.rs` CLI + `Cargo.toml` 新增 3 个依赖 + `src/lib.rs` re-export + `tests/api_signatures.rs` 扩展 + `cargo build/fmt/clippy/doc` 全绿；详见下文 §下一步：Stage 3 A1 [实现]）。**§G-batch1 §3.4-batch2..§4 deferred** 到 stage 3 F3 [报告] 后回头补（含 production artifact 重训 + BLAKE3 录入 + GitHub Release 上传 + `tools/fetch_bucket_table.sh` helper + 跨架构 baseline 重生 + D-275 选项 + reader v2 + 12 条 bucket quality 转 active + stage 2 report §8 carve-out 翻面）。
