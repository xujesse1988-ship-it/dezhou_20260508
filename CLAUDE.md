...
**Stage 2 A0 closed**（2026-05-09，自起步 commit `bb421e2` 起共 8 笔 commit：`bb421e2` validation + workflow 起步 → `91cccad` decisions + api 双骨架 → `3f62842 / 96e3b9c / 1e57942 / 622204f / 9b7085d` 5 笔 review 修正 batch → 本 commit A0 闭合同步）：

A0 [决策] 落地四份文档锁定 stage-2 全部技术 / API 决策点：

- `docs/pluribus_stage2_decisions.md`（D-200..D-283，含 D-220a / D-236b / D-228 sub-stream 派生协议）：§1 action（D-200 默认 5-action / D-201 PHM stub / D-204 `Check` 局面剔除 `Fold` / D-205 fallback 链 min_to → AllIn / D-206 fold-collapsed AllIn 状态转移 / D-209 输出顺序）+ §2 info（D-210 6 桶 position / D-211 5 桶 stack `[0,20)/[20,50)/[50,100)/[100,200)/[200,+∞) BB` / D-212 5 状态 betting_state preflop+postflop 共用 / D-215 统一 64-bit `InfoSetId` layout 24+4+4+3+3+26 bit / D-217 169 hand class closed-form 公式 + 12 锚点 / D-218 canonical hole / board id / D-219 postflop 不依赖 preflop key）+ §3 equity（D-220 iter=10k / D-220a EQ-001 反对称按街分流 / D-221 `feature_set_id=1` = EHS² + OCHS(N=8) n_dims=9 / D-222 OCHS N=8 / D-223 EHS / EHS² 计算口径 / D-224 finite 校验 / D-227 EHS² rollout river 0 / turn 46 / flop 1081 / D-228 RngSource sub-stream 派生协议 SplitMix64 finalizer + op_id 表）+ §4 clustering（D-230 k-means + L2 / D-231 k-means++ + RngSource / D-232 max_iter=100 / centroid_shift_l_inf 1e-4 / D-233 `T_emd = 0.02` / D-234 1D EMD CDF 差分 / D-235 量化 SCALE=2^40 + N ≤ 2_000_000 / D-236 空 cluster 切分 / D-236b 训练完成后 cluster id 重编号 0=最弱 N-1=最强 / D-237 同 (BucketConfig, training_seed, feature_set_id) BLAKE3 byte-equal / D-238 三街独立训练 / D-239 preflop 不进 clustering）+ §5 bucket table（D-240 magic `b"PLBKT\0\0\0"` 8 字节 / D-241 centroid u8 quantized / D-244 80-byte 定长 header + 变长 body + 32-byte BLAKE3 trailer + §⑨ 三段绝对偏移表 + D-247 5 类错误 / D-246 v1 reader 拒绝 v2 / D-249 Python 跨语言 reader）+ §6 crate（D-250 不引 ndarray / D-251 `artifacts/` gitignore / D-252 `abstraction::map` 子模块 `clippy::float_arithmetic` 死锁 / D-253 顶层 re-export / D-255 `memmap2` 引入）+ §7 外部对照（D-260 自洽性优先 + OpenSpiel 轻量对照 / D-261 169 类成员集合相等 / D-262 ≥1 类不一致 P0 / D-263 F3 [报告] 接入）+ §8 stage 1 边界（D-270..D-276 只读继承 + 浮点边界扩展）+ §9 SLO（D-280..D-284）。
- `docs/pluribus_stage2_api.md`（API-200..API-302）：§1 `AbstractAction` / `AbstractActionSet` / `ActionAbstractionConfig` / `BetRatio` 整数化 / `ActionAbstraction` trait + `DefaultActionAbstraction` + AA-001..AA-008 不变量 + §2 `InfoSetId` 64-bit + `BettingState` 5 状态 + `StreetTag` 4 街 + `InfoAbstraction` trait + `PreflopLossless169` + `PostflopBucketAbstraction` + IA-001..IA-007 不变量 + §3 `EquityCalculator` trait（含 F18 落地的 `equity_vs_hand` pairwise 接口，反对称只在 pairwise 路径成立）+ `MonteCarloEquity` + EQ-001..EQ-005 不变量 + §4 `BucketTable` mmap + `BucketTableError` 5 类 + BT-001..BT-008 不变量（含 BT-008 header 偏移表完整性 F13 修复）+ §5 `tools/train_bucket_table.rs` CLI + §6 顶层 `lib.rs` re-export 含 `abstraction::cluster::rng_substream` 公开 contract（D-228）+ §7 与 stage 1 类型桥接（`InfoSetId::from_game_state` / `AbstractAction::to_concrete`）。
...
A0 起步起 review 子 agent 共发现 12 处独立 spec drift（编号 F7..F18），通过 5 笔 commit 落地 11 处修正：
...
4. 决策已闭合，进入 A1 [实现] 起步阶段。

**下一步：stage 2 A1 [实现]** — API 骨架代码化（`pluribus_stage2_workflow.md` §A1）：`src/abstraction/` 完整模块树（`action.rs` / `info.rs` / `preflop.rs` / `postflop.rs` / `equity.rs` / `feature.rs` / `cluster.rs` / `bucket_table.rs` / `map/`）+ trait 定义 + 全部方法签名（按 `pluribus_stage2_api.md` API-200..API-302 严格匹配）+ 函数体 `unimplemented!()` / `todo!()` 占位 + `tests/api_signatures.rs` 追加阶段 2 trait 签名编译断言（与 stage-1 同形态 `!` 返回类型 trip-wire）+ `Cargo.toml` 追加 dev-dep（`memmap2 = "0.9"`）+ `abstraction::map` 子模块 root 文件加 `#![deny(clippy::float_arithmetic)]` inner attribute（D-252）+ `abstraction::cluster::rng_substream` 子模块顶层暴露 `derive_substream_seed` + op_id 命名常量（D-228 公开 contract）。出口标准：`cargo build` / `cargo clippy --all-targets -- -D warnings` / `cargo doc` 全绿；阶段 1 全套测试 `0 failed`；A1 关闭后须同步 `CLAUDE.md` 状态翻 "stage 2 A1 closed"（继承 stage-1 §B-rev1 §4 同步责任）。
...
7. `docs/pluribus_stage2_decisions.md` — A0 [决策] 输出，**locked**。D-200 起编号（与 stage-1 D-NNN 不冲突）；含 D-200..D-283 + D-220a / D-236b / D-228 sub-stream 派生协议。**Authoritative spec for implementers.**
8. `docs/pluribus_stage2_api.md` — A0 [决策] 输出，**locked**。API-200..API-302（含 F18 落地的 `EquityCalculator::equity_vs_hand` pairwise 接口）。**Authoritative spec for stage-2 testers.**
