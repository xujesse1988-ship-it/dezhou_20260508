...
- `lib.rs` D-253-rev1 顶层 re-export 全部 14 个公开类型 / trait / helper 落地：`AbstractAction / AbstractActionSet / ActionAbstraction / ActionAbstractionConfig / BetRatio / ConfigError / DefaultActionAbstraction / BettingState / InfoAbstraction / InfoSetId / StreetTag / BucketConfig / BucketTable / BucketTableError / EquityCalculator / EquityError / MonteCarloEquity / PreflopLossless169 / PostflopBucketAbstraction / canonical_hole_id / canonical_observation_id`。
...
**下一步：stage 2 B1 [测试]** — 核心场景测试 + harness 骨架（`pluribus_stage2_workflow.md` §B1）：A 类 10–15 个 fixed scenario 测试（`action_abs_default_5_actions_open_raise_legal` / `preflop_169_*` / `info_abs_*` 等）+ B 类 preflop 169 lossless 完整 1326 → 169 枚举覆盖（`tests/preflop_169.rs`）+ C 类 equity Monte Carlo 自洽性 harness（`tests/equity_self_consistency.rs`，含 EQ-001-rev1 反对称容差按街分流）+ D 类 determinism harness 骨架 + E 类 criterion benchmark harness 骨架。出口标准：A 类编译通过且因 `unimplemented!()` 全部失败；B 类 1326 → 169 枚举正确性可独立通过（不依赖 stub 之外的实现）；C/D/E harness 流程不 panic；阶段 1 全套测试 `0 failed`。B1 [测试] agent **只写测试 / harness，不修产品代码**（继承 stage-1 §B1 / §C1 角色边界）。
