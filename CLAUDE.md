...
8-stage Pluribus-style 6-max NLHE poker AI。**Stage 1 closed**（git tag `stage1-v1.0`，验收报告 `docs/pluribus_stage1_report.md`）；**Stage 2 closed**（git tag `stage2-v1.0`，验收报告 `docs/pluribus_stage2_report.md`，A0..F3 全 13 步 closed）。**Stage 3 起步 batch 1 §G-batch1 §1 [决策]** + **§G-batch1 §2 [测试]** + **§G-batch1 §3.1 [实现]** + **§G-batch1 §3.2 [实现]** + **§G-batch1 §3.3 [实现]** closed 2026-05-11（D-218-rev2 决策 + 5 条契约测试 + canonical_enum.rs 算法 + N 修正 + postflop forward + bucket_table v2 schema + fixture n_train K×100 cap + `TrainingMode` enum + `--mode` CLI flag + doc-comment drift carry-forward + vultr 5 gate 全绿）；下一步 §G-batch1 §3.4 [实现]（production artifact 重训 ~120 min release on vultr + memory feasibility 实测取舍 + GitHub Release 上传 + BLAKE3 录入；详见下文 §下一步：Stage 3 起步 batch 1）。
...
**下一步：Stage 3 起步 batch 1 §G-batch1 §3.4 [实现]**（按 `pluribus_stage2_workflow.md` §G-batch1 §3.4..§3.8 + §4 字面）：(§3.4) production artifact 重训（vultr 4-core ~120 min release，`--mode production --flop 500 --turn 500 --river 500 --cluster-iter 10000`，关闭 K×100 cap 走 4×N 全覆盖）+ memory feasibility 实测取舍（option (a) mini-batch k-means / (b) sub-sample n_train_river / (c) canonical-enumeration inverse + n_train = N 三选一）+ GitHub Release 上传 + BLAKE3 录入 + `tools/fetch_bucket_table.sh` 新增 helper；(§3.5) 跨架构 baseline 32-seed × 3 街重生（vultr ~3 h release）；(§3.6) D-275 实测取选项 A/B/C；(§3.7) `tools/bucket_table_reader.py` schema=2 解析；(§3.8) 12 条 `tests/bucket_quality.rs` `#[ignore]` 取消 + 验证 4 类质量门槛全绿（path.md 字面 EHS std dev / EMD / monotonicity / 0 空 bucket）；(§4 [报告]) stage 2 report §8 carve-out 状态翻面 + bucket_quality 直方图重生 + CLAUDE.md ground truth hash 漂移。
