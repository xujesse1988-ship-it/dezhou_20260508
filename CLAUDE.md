...
- **下一步**：**Stage 4 E2 [实现]** — `src/training/lbr.rs` LbrEvaluator 4 方法落地（D-450/D-452/D-455/D-456/D-457/D-459） + `src/training/trainer.rs` D-321-rev2 真并发 rayon long-lived pool + append-only delta merge + step_parallel 走 `[RegretTable<NlheGame6>; 6]` + `[StrategyAccumulator<NlheGame6>; 6]` 数组（§D2-revM table-array deferral 翻面）+ Checkpoint v2 body sub-region encoding 落地真 6 套表序列化（继承 D-321-rev2 同型模式）+ D-401-revM lazy decay evaluate（如 E1 SLO ① 单线程 < 5K update/s）。**通过标准**：E1 全 14 active 测试转绿 + AWS c7a.8xlarge × 32-vCPU 实测 D-490 ③ ≥ 20K update/s + D-454 LBR P95 < 30 s + stage 1/2/3 baseline byte-equal 全套维持 + B1/C1/D1 既有 default-active 测试全套不退化。详见 `docs/pluribus_stage4_workflow.md` §E2。
