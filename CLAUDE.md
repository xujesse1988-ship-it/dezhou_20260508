...
- **§F3-revM closed 2026-05-16**（用户授权 — Slumbot HU NLHE 完整集成）：F2 [实现] commit + workflow §F2-revM 字面把 Slumbot HTTP `/new_hand` + `/act` 双向交互的 `chip_delta` 字段读取 deferred；F3 起步 wait for first usable 训练期间用户授权 §F3-revM carve-out 把 Slumbot 完整 protocol 实现 + 100K 手 evaluate 提前到 F3 commit。**Slumbot 2017 protocol lock**（参考 [salujajustin/slumbot_api](https://github.com/salujajustin/slumbot_api)）：200 BB stack / 50-100 blinds / `/api/new_hand` + `/api/act` JSON / action string `b<n>/k/c/f` × `/` 街道分隔 / `client_pos` 0=SB 1=BB / `winnings` blueprint 净 chip。**实现 scope**：~290 行 — `src/training/slumbot_eval.rs` 7 helper（`build_200bb_hu_game / slumbot_err / parse_card_str+array / build_hu_state_with_cards` 走 StackedDeckRng D-028 反推协议 / `apply_slumbot_action_str` / `pluribus_to_slumbot_incr` / 私有 StackedDeckRng 副本） + `play_one_hand` 主循环重写 / `evaluate_blueprint` duplicate_dealing 折叠 no-op + `src/training/nlhe_6max.rs::bucket_table_for_eval()` getter additive。**stack-size 抽象层 mismatch（已知偏离）**：blueprint 训练 6-max 100 BB / Slumbot eval 200 BB HU；`stack_bucket` bin mismatch 预期 Slumbot 100K mean bias -20 至 -30 mbb/g（D-461 first usable ≥ -10 mbb/g 阈值预期 fail，stage 5 翻面评估）。5 道 gate 全绿（fmt / build / clippy 0w / doc / test --no-run）。stage 1+2+3 baseline byte-equal 维持；既有 NlheGame6 `new_hu` 100 BB 路径 + 9 既有 method 签名/实现不退化。
- **下一步**：**Stage 4 F3 [报告]** — §F3-revM commit 后 (a) AWS host `git pull` + `cargo build --release --bin eval_blueprint` 重 build；(b) wait for first usable 10⁹ 训练完成（已跑 8.86% / 58K update/s / ETA 4.3h，估 06:00 UTC 完成）；(c) 跑 `eval_blueprint --slumbot-hands 100000 --baseline-hands 1000000` + `lbr_compute --six-traverser`；(d) 落地 `docs/pluribus_stage4_report.md` + `docs/pluribus_stage4_external_compare.md` + git tag `stage4-v1.0`。详见 `docs/pluribus_stage4_workflow.md` §F3 + §F2-revM + §F3-revM。
