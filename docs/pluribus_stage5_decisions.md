...
| D-534 | metrics.jsonl perf 字段 schema 扩展 | metrics.jsonl 每行加 4 perf 字段：`elapsed_wall_s: f64`（单调累积秒）/ `update_count: u64`（cumulative since start）/ `update_per_s_window: f64`（最近 10s 滑窗 throughput）/ `update_per_s_lifetime: f64`（lifetime 累积均值）。继承 stage 4 既有字段不动 + D-526 pruning 4 字段 + D-540 内存 2 字段（API-590 字面）。**单一来源**：D-595 字面 metrics.jsonl unique source，禁止 commit message / 报告引用非该文件数字。|
| D-535 | acceptance run host scheduling | c6a.8xlarge on-demand 手动 boot（用户授权 D-590 预算后）+ ssh + `git pull` + `cargo build --release` + 启动 `tools/perf_baseline` binary（API-580 字面）→ 自动 3-trial 跑完 → 输出 `perf_baseline.jsonl` → `gh release upload` 或 `scp` 拉回 → host shutdown。**禁止** host 长开（D-590 预算守恒）。**禁止** host 上跑其他 workload（idle box 要求 D-592 字面）。|
| D-536 | acceptance run failure retry policy | 单 trial fail（实测 update/s < 200K）触发 **同 seed 重测 1 次**；连 2 trial 同 seed fail 即视为该 seed 真实 fail，**不**再重测。3 seed 中 ≥ 2 seed 全 fail → D-530 SLO fail → 触发 D-533 carve-out。3 seed 各 fail 1 trial 但重测全 pass → SLO PASS（3 trial min 走重测结果）。**禁止** cherry-pick 单 trial 数字，3 trial 必须全 report（PASS / FAIL 全透明，D-595 字面）。|
| D-537 | host idle detection + cpu freq lock check | acceptance run 启动前 `perf_baseline` binary 内 preflight check：(a) `uptime` load average 5min < 0.5 → 否则 abort + 提示 user；(b) `cpupower frequency-info` 当前 governor == `performance` → 否则 abort + 提示 `cpupower frequency-set -g performance`；(c) `cat /proc/cpuinfo | grep MHz` 全 32 vCPU 频率一致 ± 5% → 否则 abort（检测 turbo throttling）。preflight fail 不烧 host time，user 修复后重启 binary。|
| D-538 | acceptance run wall + warm-up + steady-state slicing | 单 run wall = **30 min** 实际跑（不含 host boot / build），warm-up skip = **前 5 min**（或前 5e7 update，取后者 — 实测 c6a 200K 5e7 update 需 250s ≈ 4.2 min，5 min wall 更宽容）。steady-state slice = `[warm_up_end, run_end]` 内 metrics.jsonl 每行 `update_per_s_window` 字段（D-534 字面 10s 滑窗），取 **mean** 作为该 trial throughput。|
| D-539 | SLO measurement aggregation formula | trial result = `mean(update_per_s_window for ts in [warm_up_end, run_end])`。3 trial 汇总 = `min`/`mean`/`max` 三个数字 全 commit message + report。**SLO PASS 判据** = `min(3 trials) ≥ 200_000`（D-532 字面）。**SLO partial** = `min < 200K but max ≥ 200K` → 走 D-533 carve-out 评估。|
...
| D-543 | RSS 测量采样频率 + 工具 | metrics.jsonl 每 D-595 cadence 行（继承 stage 4 default 10⁵ update）记录 `rss_bytes: u64`（`/proc/self/status` `VmRSS` field 字面读 × 1024）+ `regret_table_section_bytes: u64`（D-544 公式估算）+ `strategy_accum_section_bytes: u64`。30 min run @ 200K × 10⁵ cadence ≈ 3600 sample/run，足以 ladder mean / peak / steady-state slice。|
| D-544 | RegretTable section_bytes 精确公式 | `section_bytes = (keys.capacity() × 8) + (payloads.capacity() × 32) + (scales.capacity() × 4) + sizeof(RegretTableCompact metadata)`。**capacity 而非 len**：紧凑 array dynamic grow 时 capacity 即真实 alloc，metrics 反映真实 RSS 占用；len 仅作 D-519 collision_metrics input。**6 traverser × 2 table 求和**：metrics.jsonl 写**总和**（单字段 `regret_table_section_bytes`），per-traverser 拆分进 verbose log（D-526 字面 metrics cadence 不变）。|
| D-545 | production 10¹¹ scale 内存估算 | 按 Pluribus 论文 §S2 字面 ~10⁹ unique InfoSet（visit），stage 5 紧凑 layout 36 byte/row × 6 traverser × 2 table × 10⁹ InfoSet = **432 GB**（远超 c6a 64 GB RAM）。**production 必须走 D-512 分片加载 + LRU eviction** — hot shards 限 128 shard × ~120 MiB = ~15 GB resident，剩余冷 shard mmap on-demand。D-441-rev0 production training 触发条件 = D-512 分片加载在 stage 5 主线落地 + E1 [测试] 4 anchor 全 PASS + D-530 200K SLO PASS。|
| D-546 | 分片加载下 RSS 含义 | metrics.jsonl `rss_bytes` 字段 = `/proc/self/status` VmRSS = 进程**实际驻留物理内存**（含 mmap resident page）。**首选 measurement**（D-540 SLO 判定走 RSS 不走 section_bytes 估算）：first usable single-host 路径下 RSS 简洁可信；production 分片路径下 RSS 仅反映 hot 128 shard，total RegretTable 全表 byte 由 `mmap_total_bytes` 字段（API-593 新增）反映。**D-540 SLO 判定**：first usable 路径走 RSS；production 走 `regret_table_section_bytes`（含 mmap total）。|
| D-547 | baseline 测量时机 + 一次性 vs 每次回归 | naive HashMap baseline = **一次性**测量后**冻结**写入 `tests/data/stage5_naive_baseline.json`（git tracked，BLAKE3 锚定）。测量协议 = stage 4 first usable 训练 10M update 处实测的 RegretTable + StrategyAccumulator section_bytes 值（stage 4 baseline 280 MB 已锁定 D-541 字面）+ 同 10M update 处 RSS 实测值。**禁止** stage 5 主线 commit 修改 `tests/data/stage5_naive_baseline.json`（D-507 baseline 维持等效 anchor）；任何 baseline 修订走 D-547-revM + user 授权。|
| D-548 | baseline run artifact 持久化 | naive baseline 测量产物：(a) `tests/data/stage5_naive_baseline.json`（structured byte counts + RSS + InfoSet count + 测量 host + seed + git SHA）；(b) `tests/data/stage5_naive_baseline.blake3`（BLAKE3 sum of (a)）；(c) commit message 引用上述 BLAKE3 锁定。stage 5 acceptance run 期间 `perf_baseline` binary 自动读取 (a) 算 reduction ratio，输出 `naive_baseline_ratio: f64` 字段进 `perf_baseline.jsonl`。|
| D-549 | Checkpoint schema_version 2 → 3 翻面 — body sub-region encoding 字面 | stage 5 紧凑 RegretTable + q15 quantization + pruning state 序列化**必然**改 body 编码 → schema_version 2 → 3 不向前兼容。`Checkpoint::open` 走 `ensure_trainer_schema` preflight：stage 4 trainer EsMccfrLinearRmPlus + schema=2 path 维持读取（stage 4 既有 1B checkpoint 不退化）；stage 5 trainer EsMccfrLinearRmPlusCompact（API-540 字面新 variant）+ schema=3 path 落地。**HEADER_LEN bump 128 → 192 byte**（24 byte 新增 + 40 byte 对齐 pad，64 byte boundary aligned 3 cache line）。**header field 新增 8 个**（API-552 字面）：`trainer_variant: u8`（=3 EsMccfrLinearRmPlusCompact）+ `info_set_id_layout_version: u8`（=1 stage 2 D-218 维持）+ `quant_bits: u8`（=15 q15 字面）+ `padding_a: u8` + `capacity_estimate: u64`（per-table capacity）+ `pruning_config_threshold: f32` + `pruning_config_resurface_period: u64` + `pruning_config_resurface_epsilon: f32` + `pruning_config_resurface_reset: f32` + `resurface_pass_id: u64` + `naive_baseline_blake3: [u8; 32]`（D-548 字面锁定 baseline 来源 + 跨 binary version 拒绝 mismatch）+ `padding_b: [u8; remaining]` 对齐到 192。**body sub-region encoding** = 6 traverser × 2 table（RegretTable + StrategyAccumulator）= 12 sub-region，每 region encoding 三 Vec 顺序 = `keys (capacity × 8 byte) | payloads (capacity × 32 byte) | scales (capacity × 4 byte)`，bincode 序列化 + zstd level=3 compress（继承 stage 4 schema=2 body compress 模式）。region 间用 4-byte magic `0xDEADBEEF` 分隔（damage detection）。**self-consistency BLAKE3**（D-563 anchor 字面）：body BLAKE3 写入 header `body_blake3: [u8; 32]` 字段（继承 stage 4 schema=2 同字段），load 时校验 mismatch → `CheckpointError::BodyHashMismatch`。|
...
| D-564 | anchor measurement protocol | 4 anchor 测量在**同一 c6a host run** 内连续完成：(a) D-560 LBR — `lbr_compute --six-traverser --checkpoint <final.ckpt> --bucket-table <v3.bin> --n-hands 1000` × 6 traverser 串行（继承 stage 4 既有 binary 不改）；(b) D-561 baseline — `eval_blueprint --baseline-hands 1000000 --opponent random,callstation,tag` 串行 3 类（stage 4 既有 binary 不改）；(c) D-562 Slumbot — `eval_blueprint --slumbot-hands 10000 --master-seed 42`（继承 stage 4 §F3-rev2 10K 字面规模）；(d) D-563 round-trip — `cargo test --release --test checkpoint_v3_round_trip -- --ignored` 自动 BLAKE3 self-consistency。**总 wall ~30 min**（LBR ~10s × 6 = 1min / baseline ~10min / Slumbot ~7min / round-trip ~5min + overhead）。|
| D-565 | anchor failure retry policy | 单 anchor fail 触发**同 seed 重测 1 次**（继承 D-536 retry pattern）；连 2 fail 即视为真实 fail。**任一 anchor 真实 fail** → D-550-revM 翻面：(a) pruning 阈值调整（D-520-revM，e.g. -300M → -500M）；(b) ε resurface 周期或比例调整（D-521-revM）；(c) 极端情况 pruning off 走 D-501 carry-forward（不阻塞 stage 5 闭合，记入 known deviations）。**禁止** silent skip 单 anchor。|
| D-566 | anchor baseline reference 持久化 | 4 anchor baseline 数字（stage 4 first usable run 实测）冻结写入 `tests/data/stage5_anchor_baseline.json`（继承 D-547 / D-548 baseline 锁定模式）。内含字段：`lbr_six_traverser_mean: 56231`（mbb/g）/ `baseline_random_mean: 1657` / `baseline_callstation_mean: 98` / `baseline_tag_mean: -267` / `slumbot_mean: -1110.92` / `slumbot_ci_lower: -1918.37` / `slumbot_ci_upper: -303.47` / `checkpoint_blake3: 388e8d84...` / 测量 host / git SHA / wall date。BLAKE3 锚定走 `tests/data/stage5_anchor_baseline.blake3`。|
| D-567 | 4 anchor 合成判定 | **全 PASS** = SLO PASS；**任一 anchor fail** = D-565 触发；**禁止** 部分 PASS 部分 carve-out 后继续推进。**例外**：D-562 Slumbot anchor 仅作 regression guard（D-562 字面），若 stage 5 路径 Slumbot mean **下降**（更负，比如从 -1110 → -1500）即视为 fail；若 **上升** 或 **持平** 即 PASS（stack-size mismatch 已知偏离 stage 5 不修，但不允许变更差）。|
| D-568 | production 10¹¹ anchor 复用策略 | first usable 1B blueprint 上 4 anchor PASS 后，production 10¹¹ 训练完成（D-441-rev0）需要 **重测** 4 anchor（绝对数字预期改善）。**production 阈值**：D-560 LBR ≤ 200 mbb/g（path.md §阶段 4 字面 first usable，production 期待 < 100 mbb/g）+ D-561 baseline 3 类全 mean > 0 + D-562 Slumbot mean ≥ -50 mbb/g（stack-size mismatch 下宽松目标）+ D-563 round-trip 不变。**production anchor 重测属 stage 5 F2 [实现] 触发后 stage 5 F3 [报告] 闭合产物**，不是 stage 5 主线 SLO 阻塞项。|
| D-569 | collision metrics anchor（D-510 add'l）| 紧凑 RegretTable Robin Hood probing 健康检查：`load_factor ≤ 0.75` + `max_probe_distance ≤ 16` + `avg_probe_distance ≤ 2.0`。E1 [测试] 必落地 `tests/regret_table_compact_collision.rs` 在 1M update warm-up 后 + 10M update steady-state 后两次 snapshot 全表 collision_metrics（API-519 字面 getter），违反阈值 abort + alarm dispatch（继承 stage 4 D-477 alarm variant pattern 扩展）。|
...
| 编号 | 项 | 触发条件 / 决策时点 |
|---|---|---|
| D-549-decision | Checkpoint v3 body sub-region encoding + HEADER_LEN bump = 192 byte + 8 新 header field + 6×2 sub-region bincode+zstd + body BLAKE3 self-consistency | 本 commit batch 3 关闭 |
| D-530..D-548 实现细节 | SLO 测试协议详化 + 4 anchor 量化协议 + acceptance run failure retry policy | 本 commit batch 3 关闭 |
| API-540..API-589 全集 | Trainer extension + Checkpoint v3 + Shard loader + perf_baseline binary 全套签名 | 本 commit batch 3 关闭 |
...
| D-401-revM lazy decay 评估结果 | batch 2 lock = D-511 lazy 路径（scale-only decay） + D-518 周期 1e6 iter renorm。stage 4 carry-forward P2 项**正式翻面**关闭 | batch 2 关闭 |
...
- **batch 2**（commit 63154ec）= D-510..D-519 紧凑 array + perfect hash + q15 quantization + 分片加载 + SoA + AVX2 + bucket layout（lightweight） + rayon 实现细节字面 lock + D-520..D-529 pruning 阈值 -300M 绝对 + ε resurface 周期 1e7 iter + 比例 0.05 + reset -150M + warm-up 互斥 + 数学正确性 + 不单独 serialize pruning state + CLI flag + metrics + unit test scaffold + RNG 派生具体值 lock。**stage 4 carry-forward P2 D-401-revM lazy decay 路径选定** 同 commit 正式翻面关闭。
- **batch 3**（本 commit）= D-534..D-539 SLO 测试协议详化（metrics.jsonl schema 4 perf 字段 + host scheduling 手动启停 + 单 trial fail 重测 1 次连 2 fail 真 fail + idle / cpupower preflight check + 30 min wall × 5 min warm-up skip × steady-state slice × 3 trial min ≥ 200K 判据）+ D-543..D-548 内存 SLO 详化（采样频率 / section_bytes 精确公式 / production 10¹¹ scale 432 GB 必走分片 + LRU 15 GB resident / RSS 含 mmap resident 部分 / baseline 一次性冻结 `tests/data/stage5_naive_baseline.json` BLAKE3 锁定 + 持久化）+ D-549 Checkpoint v3 body sub-region encoding 字面（HEADER_LEN 128 → 192 + 8 新 header field + 6×2 sub-region bincode+zstd + body BLAKE3 self-consistency）+ D-564..D-569 anchor 详化（measurement protocol 同 host run × 30 min / fail retry 1 次连 2 即真 fail / baseline 持久化 `tests/data/stage5_anchor_baseline.json` + BLAKE3 锚定 / 4 anchor 合成判定全 PASS or D-565 触发 / production 重测策略 / D-510 collision metrics anchor load_factor ≤ 0.75 + max_probe_distance ≤ 16 + avg_probe_distance ≤ 2.0）。
