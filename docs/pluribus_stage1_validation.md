...
- 分池产生不能丢失筹码；不可整除筹码必须按 odd chip rule 处理：每个 pot 独立把全部余数分给按钮左侧最近的获胜者（D-039-rev1，PokerKit 默认语义）。该规则在文档和代码中显式写明，所有场景结果可复现。
...
- 与至少 `1` 个开源参考评估器（如 treys、OMP、SKPokerEval、ACE）交叉验证：相同的 `1,000,000` 组 7-card hand，名次/类型输出完全一致。
    - **D-085 阶段化（与 §7 同口径）**：C1 接入 harness，默认 1k / `#[ignore]` 100k；C2 必须达到 100k 零分歧（**最终通过门槛**）；E2 高性能评估器替换后扩到 1M 完成回归。
    - **比对粒度阶段化**：C1 / C2 阶段仅比对 `HandCategory`（0..9 共 10 类枚举），因为 naive 评估器尚未提供完整 5-best 名次接口、且 PokerKit 类名跨版本不稳定（详见 `tools/pokerkit_eval.py` 顶部注释）；完整 "名次"（5-best rank tuple）的比对留到 E2 高性能评估器接入后并入 1M 回归。
...
- 至少选定 `1` 个公认开源 NLHE 实现作为参考（如 PokerKit、OpenSpiel poker、ACPC server）。当前选定为 PokerKit（D-080）。
- 跑相同 seed 与动作序列；结算筹码、side pot 划分、winner、showdown 顺序必须与参考实现完全一致。**手数按 D-085 阶段化**：B2 已完成 `100` 手；C2 必须达到 `100,000` 手且零分歧（**最终通过门槛**）；E2 后回归扩到 `1,000,000` 手以巩固性能与稳定性。本节正文沿用 `100,000` 作为通过门槛数字，1M 为 E2 后回归目标。
...
- 手牌评估正确率 `100%`，与至少 `1` 个开源参考评估器交叉验证 `1,000,000` 手完全一致（**E2 高性能评估器接入后达成**；C2 通过门槛为 `100,000` 手 0 分歧的 `HandCategory` 一致——见 §4 与 D-085）。
...
- 与至少 `1` 个开源 NLHE 参考实现的 `100,000` 手交叉验证全部一致（或差异已显式记录）；E2 后回归扩到 `1,000,000` 手——见 §7 与 D-085。
...
本文档遵循与 `pluribus_stage1_decisions.md` §10 / `pluribus_stage1_api.md` §11
相同的"追加不删"约定。决策性修订仍以 `D-NNN-revM` 为主导（在 decisions.md
§10 修订历史落地），本节只记录 validation.md 自身的措辞同步。

- **2026-05-08（C1 评审同步）**：§4 与 §7 显式按 D-085 阶段化，把"评估器交叉
  验证 1M 手 / 规则引擎交叉验证 100k 手"两条最终目标拆成 B2 100 → C2 100k →
  E2 1M 三段，避免读者把 1M 误读为 C1/C2 必须达成的门槛。同时在 §4 第二条
  bullet 显式声明：C1/C2 阶段 PokerKit 交叉验证仅比对 `HandCategory`（0..9
  共 10 类枚举），完整 5-best 名次的比对留到 E2 高性能评估器接入后并入 1M
  回归——原因是 (a) naive 评估器尚未提供 5-best 接口，(b) PokerKit 类名跨
  版本不稳定（详见 `tools/pokerkit_eval.py` 顶部注释）。本同步不修改 D-085
  数值，不触发 `D-NNN-revM`。"通过标准" 段落同步对齐相同口径。
