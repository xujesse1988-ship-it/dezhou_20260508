...
## 0. 用户指定的策略流程（2026-06-08）

> 本文「配置照搬用户流程」的流程即此节；§2 逐项对照、§3 是其代码落地。

**主流程（每次我方决策）：**

1. **选择**：preflop 走 blueprint；postflop 启用实时搜索。
2. **Range 估计**：估在场各家范围。
3. **建子树 + CFR 求解**：按当前街与在场人数选 bet 菜单（见下表）建子树，迭代 CFR **解到终局**。
4. **取策略**：读我方当前手的 root 策略，返回对应动作。

**每街重跑、同街复用**：进入新街重新执行 step 2 + step 3；同街内复用同一子树。

**bet 菜单（数字 = pot 倍数；按街，flop 再按在场人数）：**

| 街 | bet 菜单 |
|---|---|
| flop | ≤3 人 `{0.5, 1}` / ≥4 人 `{1}` |
| turn | `{0.5, 1}`（任意人数）|
| river | `{0.33, 0.66, 1}`（任意人数）|

→ **flop 是唯一按人数变菜单的街**；turn/river 菜单与人数无关。

**全局**：CFR **解到终局**（不用 depth-limit）；每次求解**迭代按时间控制、≤10 秒**。

---

## 1. 边界与原则

1. **配置照搬用户流程**（§0 流程，§2 逐项对照）。已知近似/风险**诚实标注**（§4），但**不据此改配置**——强弱判据交给 OpenPoker。
