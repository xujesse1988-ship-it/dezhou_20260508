...
- **降级 = 直接 fold（简化方案）。** 墙钟 anytime 到时限就返回当前平均策略（解到终局的窄树、只是迭代没跑够），这本身
  就是可用结果、不算降级；只有**真解不出来**（建不了树 / 连一轮有用迭代都跑不完 / 会 panic）这种罕见情况才触发降级，
  **降级动作 = fold**。不再搞「多人 equity / push-fold 启发式 → blueprint 最后兜底」那套阶梯——那层启发式在 `src/` 里
  根本不存在（省掉新建），而回落 blueprint 在 off-distribution 下 = 回落到**解错了游戏**的策略（≤3-way / 近 100BB 才是它的
  有效区）。fold 是最省事、绝不会打出「错游戏」动作的兜底，缺口① 里「建 off-tree 兜底启发式」这个子项随之取消。
