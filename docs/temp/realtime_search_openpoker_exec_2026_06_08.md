...
- **blueprint 缺席的格子（深码 / 4–5 人）**：blueprint **根本没有答案**——它把任意码深当 100BB 解、把 4+way
  ...
  像样策略"。**北极星正活在这里，v1 直接攻这里。**

**核心区（深码 / 多人）没有干净的离线 EV 标尺，这是直面它的代价，须诚实承担。** 唯一能给干净标量的格子是
短码 HU——树小到可离线 CFR 收敛当真值、HU 恢复两人零和、exploitability 是确定标量。但短码对北极星帮助很小
（频率低、且"短码打得好"不向深码 / 多人迁移，§0.5），**为一个目标无关的格子建 best-response 标尺是偷懒**，故弃。
核心区的强弱判据只剩两条，都不是干净 EV 标尺：
...
- **(a) 结构性正确（离线、可硬证）**：blueprint 在 off-100BB / 4+way **解的是错的游戏**——树几何（SPR、all-in 阈值）
  ...
  阈值对真实 `GameState` 一致、守恒、byte-equal、对 PokerKit 自洽、实时解 ≈ 同状态离线 CFR 收敛解），**不需要
  exploitability**。它证「肉存在」（blueprint 给的动作在真栈下被 off-tree 强行 clamp、是失真动作），但**不量
  「肉多大」**——量级核心区无干净离线标尺。
...
引擎正确、时限内解得动」（离线可证），EV 量级兑现待外部对手功效预算到位后逐格转可验收。**短码不再是"可证伪的
第一仗"——第一仗是深码/多人的引擎正确性 + 时限可行性（§4 步 A），EV 强弱诚实地留给 live + 时间。**
...
3. 剥削 = 可选外挂（同引擎换对手数据源），置信度门控，**后置**；biased 叶子机制默认弃用。

**2026-06-09（用户拍板）**：**砍掉「短码先做」的产品步骤，直面深码 + 多人。** 理由：短码（尤 HU）只是
*方法论上*最方便证伪的格子（树小、可离线 CFR 收敛当真值、HU 恢复两人零和有干净 exploitability），
但**它对北极星帮助很小**——真实分布由深码 / 多人主导，短码（14–30BB）频率低，且「短码打得好」不向深码 /
多人迁移。先做短码 = 拿方法论便利换目标进展，是偷懒。**短码由同一引擎当小树特例顺手覆盖（码深轴里最便宜解的
一档），不立独立里程碑、不当验证标尺。** 直接后果：**核心区（深码 / 多人）放弃干净的离线 EV 标尺**
（best-response / exploitability 只在短码 HU 成立、不转移），强弱改由「结构性正确（解真游戏而非解错的 100BB 游戏）
+ 弱 live 判据」承担——诚实代价，见 §0.3 / §4。
...
- **短码（14–30BB）= 同一引擎的小树特例，顺手覆盖、不立里程碑**：树小 → 能解到终局（无须叶子值近似、无须
  depth-limit），是码深轴里*最便宜*解的一档；引擎对深码做对了、短码自动落在能力内。注意 100BB blueprint 在
  短码上同样**解错游戏**（SPR/all-in 树几何错 + range 先验 stack-agnostic 错），所以短码若出现在真实牌桌，由实时
  搜索现解、而非回落 blueprint。但短码对北极星帮助很小（频率低、不向深码迁移），**不当独立步骤、不当验证标尺**
  （§0.5）。
...
- **桶层须先补 S3 多人验证**：当前桶是 HU 单对手 equity 桶、S3 只验证过 ≤3-way 可复用，4+way 桶重排未量；
  N-way range 采样 + card removal 成本随座位线性升。4+way 上生产前先补 S3 多人桶验证（OCHS-multiway / hist
  重标边界）= C0（§4.1），vultr 可跑。
...
- 算力参照：Pluribus 实时搜索 28-core/128GB **平均 ~20s/手**（设计 §8）；vultr 4-core 设计文档只敢承诺
  **P95 < 30s**，已高于 5–20s 目标轴下界。**5s 内深码/多人能否解动须先实测**——很可能须把目标轴收到 10–20s
  或上多核机。**直面深码/多人意味着时限问题从一开始就是核心，不再有"先测最便宜的短码"那层缓冲。**
...
1. **限时求解器**：先离线产出「(节点数, 迭代数) → 单决策 wall」回归曲线，据此实现**按预算静态选树粒度**
   ...
   （该启发式层现不存在，须一并建）。墙钟中断仅作豁免退路。**曲线须直接标在深码 / 多人目标树上**（不是最便宜的小树）。
...
4. **多人 >3 树 + N-way 叶子值**：见 §2.2，立项选甲/乙；4+way 先补 S3 多人桶验证（C0）。
...
> **已删缺口（2026-06-09）**：原缺口⑥「NLHE best-response（短码 exploitability 离线锚）」随短码步骤一并砍掉。
> 它只在短码 HU（小树 + 两人零和）成立、不向深码 / 多人迁移，是目标无关的便利标尺（§0.5）。求解核本身的
> 均衡正确性仍锚到既有 Kuhn/Leduc exploitability 真值 + 小子树对 PokerKit 口径自洽（§5），**不需新写 NLHE BR**。
> 故下文缺口编号从 ⑦ 顺移为 ⑥（多人 AIVAT）。
...
并行数据采集见 §4.2，不在此顺序里。**无短码步骤**——短码由同一引擎当小树特例覆盖，不立里程碑（§0.5）。
...
| **B**（深码实时搜索） | depth-limit + 叶子续局值**按真实码深重建**（缺口③；biased 默认弃用，保守选择非「实锤有害」，§2.1）；接生产 advisor（缺口②，管线重建、**喂入各家真实栈**）；限时求解器（缺口①，据 A 的 wall 曲线静态选粒度，**含 off-tree 真实状态兜底启发式层**） | **离线（硬放行）**：叶子值按真实码深重建 + 守恒 + byte-equal；**advisor 实喂 per-seat starting_stacks（不再硬编码 100BB）**；**含非对称栈样例（如 hero 200BB vs 对手 60BB）的深码守恒 + SPR/all-in 阈值对真栈一致**；no-panic / 归一；单决策 wall ≤ budget；**限时解不动时降级动作来自真实状态启发式、不回落 100BB blueprint**。**live（观察项，非放行）**：见下「live 功效预算」——降为不退化护栏 |
| **C**（多人 >3） | 拆三步：**C0** S3 4+way 桶可复用性离线验证（缺口④，vultr 可跑）→ **C1** 据 C0 立项选甲（扩抽象 4-way，48GiB）/ 乙（实时 N-way 解，解到终局用真实 `payouts()`、深码须 N-way 叶子值）→ **C2** 建 N-way 叶子值 + live | **C0**：4+way 桶 signal/floor 达阈 或 重标边界方案定（独立放行，先于选型）；**C1**：甲/乙取舍拍板；**C2**：4+way 见 flop 桶有忠实树（离线核：守恒 + N-way side-pot payouts 正确）+ live 非劣（同 B，功效不足 → 过 ≠ 兑现多人核心，只兑现「解真游戏 + 不退化」） |
| **D**（后置可选） | 剥削外挂：置信度门控替换对手 range 数据源 | **前置**：对手 name 稳定可追踪（§4.2 已验）；数据足的对手上增量为正（同受 live 功效限制，复用下方护栏）；vs 池中最鲁棒对手分项不亏（防反剥削） |

**放行判据定义（须可判，不留含糊）**：

- **收敛（步 A①）= 距离达阈**：实时解与离线 CFR（≥ M 迭代）的 per-infoset 平均策略 L1 距离均值 < ε 且 root EV 差 < δ_conv（ε / δ_conv 在步 A 标定）。这是 **CFR-vs-CFR 一致性、不是 best-response**，抓的是实时路径独有的偏差（建树 / resample / 限时截断 / 索引 hero 真桶错位）；**任何可枚举的真实子树（深码 ≤3-way river/turn 子博弈即可）都能跑**，不依赖短码。
- **深码 / 多人非劣性 margin δ（步 B / C2）**：放行 = 功效内能排除「劣化超过 δ」（CI 下界 > −δ），不是「CI 跨 0」——后者把「真不劣」与「样本不足测不出」同判 PASS（接受零假设）。δ 须连同「账号可达手数内能分辨的最小 δ」一起给；若可达 δ 远超有意义阈值，诚实标注「该格 live 当前手数不可判」、**放行主锚落在离线结构性正确（解真游戏 + 引擎正确），不靠 live**。

**限时可行性是全表前提**（北极星硬轴，不只 §6 风险）：`subgame.rs` 求解器现固定迭代、无 `time_budget`，单决策
wall 从未隔离测过；Pluribus 28-core 平均 20s/手、vultr 连 P95<30s 都要上 ≥8-core。直面深码/多人意味着**时限问题
从一开始就是核心、不再有"先测最便宜的短码"那层缓冲**：步 A② 先在真实深码/多人树上回答「5s 档能否解动」，
B/C 各带时限可行性门；任一格 5s 解不动是「收窄时限轴 / 上多核」的决策点，不是静默宣称兑现。
...
**任何合理 effect 下 live 都需远超账号可达手数 → 强弱判据不能靠 live：核心区靠「结构性正确（解真游戏、引擎正确，
离线可硬证）」当主锚，live 只作非劣护栏 + 弱方向。这正是放弃短码 exploitability 标尺后、核心区唯一诚实的判据组合。**
...
- **引擎正确性 across 码深轴**：补「**深码中途根 + per-seat 非对称栈（如 hero 200BB vs 对手 60BB）+ 多人 side-pot
  ...
  是码深轴最高频形态、又是「现解天生处理、预计算表做不到」（§2.1）的承重前提，**必须验过才能当前提**。小树（短码）
  守恒是这套 fixture 的*便宜子集*、顺带覆盖，但 fixture 的目标是**深码 / 多人正确**、不是短码产品。
...
1. **搜索 off-distribution 的价值 = 有理由相信、未证**：必须在 off-stack / 多人 上对**外部对手**证它（步 B/C 的 live 半）。
   ...
   **不再**依赖短码 best-response / exploitability（已弃，§0.5）；live 锚依赖未算的功效预算。在 A 与功效预算落地前，
   B/C 的 **live 半**是 0% 可执行而非"待跑"；但 A 的**离线半**（引擎正确 + wall/时限）现在就能开。
2. **深码 / 多人叶子值 = 真硬骨头**：100BB blueprint 值不转移；小树解到终局能绕开它，但小树不是北极星——
   **深码 / 多人必须正面重建叶子值（缺口③④），这是 v1 核心而非可绕开项**。biased 叶子在 §11.5d 探针下更偏离
   blueprint（相对读数、非绝对有害），默认弃用是保守选择；且 depth-limit unbiased 本身只是 wash → 深码靠它拿净增益
   证据偏弱，须把叶子值按真实码深重建/重标，收益到外部对手上才算数。
...
4. **限时**：深 / 多人树在 5s 内解不解得动未知（设计 §8：vultr 4-core 连 P95<30s 都要上 ≥8-core，5s 档更没底）；
   ...
   是 §4.1 步 A② / B / C 的显式决策点，不是静默宣称兑现。
5. **验证闭环慢且脆**：放弃自对弈真值 + **放弃短码 exploitability 标尺**后，核心区离线只剩**结构性正确**
   （守恒 / byte-equal / PokerKit / 实时解≈离线CFR / 解真游戏的 SPR-all-in 一致），EV 量级只剩 OpenPoker live
...
7. **资源争用 / 排期未定**：缺口①②③④⑥均是非平凡新建项，无人天/wall/$ 估算。实时搜索的 AWS 评测（1M 手 / ≥8-core）
   ...
   列 wall + $，并明确与 preopen 续训的先后/互斥、向用户报预算后再起机。vultr 可跑的离线锚（步 A、C0）不受此约束。

## 7. 算力 / 排期（待补）

- **vultr（4-core/11.67GiB）**：跑得动深码 ≤3-way 子博弈正确性测试 + 离线小样本 + S3 多人桶验证（C0）；稳定 P95、
  多人多 board 采样、1M 手评测须上 ≥8-core（AWS c6a.8xlarge 量级）。
- **优先级**：引擎正确性 across 码深轴（步 A①，vultr 可跑）→ 静态限时求解器 + wall 曲线 + **深码/多人目标树**
  5s 时限可行性判定（缺口①，步 A②）→ 生产接线（缺口②，含喂真栈）→ 深码叶子值重建（缺口③，步 B）/ 多人
  （先 C0 桶验证 vultr 可跑、再缺口④ N-way 叶子值，步 C）→ live（含多人 AIVAT 缺口⑥降方差）。上 AWS 前须与
  preopen 续训抢机的取舍先拍板（§6 #7 硬前提）。
- 各步 wall + $ 估算 = 立项前补齐项（§6 #7）。

**状态：目标已定（本文）。下一步 = 步 A 两前置（见 §4.1 表 A①②：① 引擎正确性 across 码深轴，含深码中途根 +
非对称栈 + 多人 side-pot 守恒 + 实时解≈离线CFR 收敛；② wall 回归曲线 + 深码/多人目标树 5s 时限可行性判定，缺口①），
全在 vultr；live 功效预算（统一 mbb/g + 多人 AIVAT 缺口⑥）随 live 半段在步 B/C 前算。A 成立后直接开深码（步 B =
叶子值按真码深重建 + 接生产喂真栈 + live 不退化确认）或多人（步 C，入口由 §4.2 数据触发）——**无短码步骤**。
数据管道（§4.2）= 可并行后台采集，不卡住 A/B/C、随时可起。**
