...
- **本街多决策一致性 = `SubgameSolveCache` 加 prev-slot**（关键修复，commit `4141449`）：容量-1 主
  slot 逐街替换会**驱逐上一街解**——本街首跨街决策的 solve 一存就把上一街解挤掉，本街第二决策若只
  peek 主 slot 拿到的是本街自身解（自验拒）→ 退档一 → cross range≠prefix range（真桶）→ 不同 key →
  miss 重解 = **一街两个不同均衡**（破缓存「每轮一个 solve」不变量）。修法 = store 时**同手换街**把旧
  主 slot 提升到 prev（不参与 lookup/计数，只供跨街 peek），跨街 peek 先试 current 再试 prev，自验取
  首个匹配 → 本街多决策用**同一**上一街解 → 同 cross range → 同 key → 命中。零额外 solve；主 slot 的
  lookup/store/计数行为不变 → 既有缓存测试不受影响。
...
- **测试**（subgame 5 + advisor 2，vultr 全绿）：`cross_street_posterior_structure`（结构/归一/
  ...
  2-miss-1-hit）、`cross_street_within_street_consistency_prev_slot`（**真桶 (2,1) 证本街第二跨街决策
  命中同一上一街解 = prev-slot 修住一致性**；无 prev-slot 真桶会 (3,0)）、advisor `parse` + flop→turn
  端到端 plumbing/可复现。
- **✅ 决策级 A/B 已量化 → 生产默认改开（2026-06-19，同档一证据等级拍板）**：自对弈仍测不动（§5
  触发率 ~0），故走「干净标尺 = 构造 off-tree-flop→turn 线」。新工具 `six_max_cross_street_ab`
  （`tools/`）：构造 deep off-tree flop→turn 加注战线（UTG 短码 all-in 断点 → SB raise-over → flop
  SB bet/BB raise/SB call → turn；UTG 真栈调小 → 影子 abstract all-in amount-blind → synced 前缀不变、
  flop/turn 仍深），真 1B nolimp blueprint + 真桶，**受控配对差**：同 hand_seed / 同子树 / 同迭代，
  **只差 root range**（ON=flop σ 后验 vs OFF=档一同步前缀）→ TV 纯由 range 差驱动，欠收敛对两臂等量
  不污染差值。实测（62 手×多配置）：cross 触发 ~100%、hero turn 决策 **mean TV 0.27–0.41 / argmax
  翻转 29–61%**，对菜单粒度（二元 check-jam / 三档）/ seed / 迭代数（4000 iter 不洗掉）皆稳；**方向 =
  §动机证实**——OFF（档一粗前缀）近乎 auto-jam，ON 收成随牌力分化的极化策略（中等/摊牌牌 Kh/AK高/
  小对/中对带 draw 从梭哈拉回，纯空气 72o 构造极化诈唬）。**EV「更好」结构性不可得**（自对弈触发~0、
  live 功效太弱 1.1–1.33× SD），故据「强机制 + 决策级方向一致」开默认——和当初翻**档一**ON 同一证据
  等级（且 62 构造手覆盖 > 档一当时 1 手真 live 88@A73r）。代价诚实：构造谱非真实牌局频率（29–61%
  翻转 = 「给定这些 off-tree 谱」非真实频率），「不同」≠「EV 更好」，无 EV 锚。生产默认
  `--search-unanchored-cross-street` = ON（`DEFAULT_SEARCH_UNANCHORED_CROSS_STREET=true`），`off` =
  对照臂 / 回退；正确性早由 search=None / cross_street=None byte-equal 单测 + 缓存 key 隔离 + prev-slot
  一致性硬证。**真 live 多手 EV 确认仍 pending**（同档一，开 off 臂回填）。
