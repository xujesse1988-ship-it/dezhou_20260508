...
本方案**不是**为了改进当前 100M blueprint。`docs/status_v2.md` 已确认当前
6-action / 500-bucket profile 下 ES-MCCFR 与 LCFR 都在 100M update 即质量饱和
（100M→500M LBR < 1 SE），且 100M 甜点 run 只要 ~2h18min / ~$2.8。单看当前
profile，dense table 的边际收益薄。

真正动机是**为扩 information / action abstraction 打地基**（status_v2 "下一步"
认定的质量杠杆）。一旦 bucket 数或 action 粒度上去：

- infoset 数随 `bucket_count × node_count` 乘性增长，HashMap 的 RSS 和 checkpoint
  clone+sort 成本会先撑爆内存。
- 单 infoset 平均访问次数下降，"100M 饱和"结论失效：饱和点上移，重新落回
  Brown & Sandholm 2018 §Discounted MCCFR 所述 LCFR 真正有收益、且需要更多 update
  的 regime（单 infoset ≪ 100 visits）。更多 update 意味着 `step_parallel` 的 serial
  merge 重新成为吞吐瓶颈。

dense 第一版把 merge 从 HashMap playback 换成数组下标写，正打在这个瓶颈上。所以
**dense 是大 abstraction run 能不能跑得动的前置条件**，应在 abstraction 扩张前先落地。
sizing（Phase 0）也因此必须按**目标 abstraction** 估，而不是当前 profile（见下）。
...
**以上是当前 6-action / 500-bucket profile 的数。扩 abstraction 后这些数全部失效，
sizing 必须按目标 profile 重算**（见 Phase 0）。bucket 数与 action 数都是 infoset 量
的乘性因子，full dense 是否吃得下、是否要 paged / sparse，完全由目标 profile 的
`total_slots_variable_action` 决定。

### node_id 2^26 硬门槛（扩 abstraction 前必查）

`InfoSetId` v2 把 node_id 写在高 26 bit（`src/training/nlhe.rs` `pack_info_set_v2`，
`NLHE_V2_NODE_ID_BITS = 26`），**上限 2^26 = 67,108,864 节点**；dense indexer 直接继承
此编码（`node_id = info.raw() >> 38`）。

- 当前 6-action 树 240,096 节点，离顶 ~280×，看着宽裕。
- 但 betting tree 节点数随分支因子爆炸式增长（~b^d）。把 bet size 从 {0.5p,1p,2p}
  加几档，b 增大后很容易吃掉这 280× headroom。
- **约束只在 action 这一轴 / node 数**：bucket_id 占 24 bit = 16.7M，上到几万 bucket
  都远没到顶，不是约束。

风险：现有越界检查是 `debug_assert`（`pack_info_set_v2`），**release 下越界会静默
高位截断**，把两个不同 node 的 infoset 叠到同一 slot —— 正是本项目最不能容忍的那类
静默正确性错误。

因此 Phase 0 sizing 必须把 "target node_count < 2^26" 列为**硬门槛**：若目标 action
abstraction 的树节点 ≥ 2^26，则不只 dense indexer，`InfoSetId` v2 整个 bit 分配都要
先重新设计，dense 工作必须等编码改完再开始。
...
- 扩展 `tools/nlhe_betting_tree_sizing.rs`，让它**吃目标 abstraction 参数**（action
  集 / per-street bucket 数），而不是写死当前 6-action / 500-bucket。输出：
  - `node_count` + **是否 < 2^26 硬门槛**（破顶则直接拒绝，先改 `InfoSetId` 编码）
  - `total_rows`
  - `total_slots_variable_action`
  - full dense memory estimate
  - fixed stride 6 / 8 memory estimate
  - per-street rows / slots
- 这一个数（目标 profile 的 `total_slots_variable_action`）是 full dense vs paged
  dense 的 go/no-go 阀门；用当前 profile 的数替代会得出错误结论。
- 新增 indexer 单元测试：
...
| 目标 abstraction 节点数破 2^26 | node_id 高位截断，不同 node infoset 静默叠用同 slot | Phase 0 sizing 硬门槛拦截；破顶则先重设计 `InfoSetId` bit 分配再做 dense |
