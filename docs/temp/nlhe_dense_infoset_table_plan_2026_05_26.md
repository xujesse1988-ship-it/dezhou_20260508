...
|---|---:|---:|
| stride = 6 actions | 16.07 GiB | 32.14 GiB |
| stride = 8 actions | 21.43 GiB | 42.86 GiB |
...
`f64`。目标 profile 下大部分节点 2–4 动作、flop 4-size 节点最多 ~7 个，预估两表合计
**~19 GB**（359.6M × ~3.3 actions × 8B × 2），显著低于固定 stride 的 32–43 GiB。具体内存
要由 sizing 工具统计 `Σ bucket_count(node) * action_count(node)` 后给出。

visited bitset 很小：359.6M infoset 约 42.9 MiB / bitset。即使 regret 和
strategy 各维护一个 bitset，总量也不到 90 MiB。
...
- `tools/nlhe_betting_tree_sizing.rs` 已支持 per-street raise 集合，实测目标 profile
  = 719,764 节点 / 359.6M infoset（见"规模估算"）。**仍待补**：
  - `total_rows` = Σ bucket_count(node)
  - `total_slots_variable_action` = Σ bucket_count(node) * action_count(node)
  - variable-layout 与 fixed stride 6 / 8 memory estimate
...
- 稳态 RSS ~19 GB 量级，32–64 GB 机器无 OOM。
