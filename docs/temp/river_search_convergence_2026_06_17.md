...
> 一句话定论：**河牌子博弈在生产级预算下能干净收敛到 ≈0 可剥削（与 4hJh 转牌卡 ~10 BB 地板相反）；混合河牌点（KdAc）的策略跨 seed 完全不收敛（梭哈 4%–87%），但博弈值（−6.4 BB）和可剥削性（<0.7 BB）都收敛——这是真·非唯一均衡，不是 4hJh 那种欠收敛。**
...
与 4hJh 的关系：4hJh 是**转牌**根（下面还压着河牌发牌 + 河牌下注两层），可剥削性卡 ~200 内部 chip（~10 BB）不降；本文测的是**河牌**根（牌面已 5 张全发、下面只剩一轮下注、无发牌节点）。
...
- **可剥削性 ≈0**（uniform −0.14~+0.32 BB；LCFR 0.05~1.10 BB），全在 4hJh ~10 BB 地板之下。`EXPLOIT_CUR`（current 策略）11–20 BB = 正常 CFR 震荡 → averaging 健康、不饿死（ss_mass 21 万）。`EXPLOIT_STREET turn_only≈0 / river_only=full` 证实是单街河牌树。
...
| 4hJh **转牌** | 2 | ~19,000 | 有(~44) | 卡 ~10 BB（§10/§11） |
...
| | 跨 seed 策略 | 可剥削性 | 诊断 |
|--|--|--|--|
| 4hJh 转牌 | 不一致 | **卡 ~10 BB** | 都**远离**均衡 = 欠收敛（病） |
