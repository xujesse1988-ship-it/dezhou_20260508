...
- **vultr（4-core/11.67GiB）**：跑得动**单层 flop subgame 正确性测试**（≤3-way、≤5 raise 档、几千迭代）；但稳定 P95<30s
...
**vultr 中样本首信号（2026-06-03，24k 手/arm @ 3000 iters，真 1B nolimp ckpt）**：

| arm | hero(search-on) mbb/g | CI95 | desync | search 触发/真 |
|---|---|---|---|---|
| range-on（§5b） | **−71.6** | [−232, +89] | 0 | 2553/2599（fallback 1.8%）|
| uniform（MVP A/B） | **−81.6** | [−242, +79] | 0 | 2553/2599 |

结论：①**不退化**（两 arm CI 均跨 0），点估轻微为负（~−72 mbb/g，**不显著**）；②**range-on ≈ uniform**
（−72 vs −82，差 ≪ SE 82）——在 flop 首决策点这一窄触发面，去 confound 没明显改变结论；③**§2 灾难失败
未出现**（搜索没把对局打爆 / 没把 blueprint 偏差放大成大亏），但也**没显著帮上忙**；④plumbing 规模化稳
（24k 手 / 2599 搜索 desync=0）；⑤vultr 够快（24k 手 range-on 95s wall / 4 核 → 1M 手 ~70 min，**AWS 非必需**）。

CI 从 600 手 ±1200 收到 24k 手 ±160；要把 ~−72 的轻微负点估判成真/噪声，可上 vultr ~10⁵–10⁶ 手（仍 free、
...
> ⚠ **§10.5 实测推翻了本节 (a) 的归因**：实现 round-start re-solve 后 all-postflop **仍退化**（等迭代更差、
> 高迭代持平），残余 = blueprint/抽象质量（§2），**不是** §6 round-start landmine。详见 §10.5。

---

## 10.5 §6 #1 round-start re-solve 已落地——实测**不是**退化的杠杆；all-postflop 退化 = 欠训练 + 残余 §2（2026-06-04，commit `8fde9bc`，分支 `6max-rts-mvp`，vultr）

**做了什么**：实现 §10.4 顶出的「下一必做」= §6 #1 round-start re-solve。`ResolveRoot{CurrentDecision,
RoundStart}`（默认 RoundStart，`subgame.rs`）：RoundStart 从 **betting-round 起点**快照为根建子树（当前街
betting 落入 subgame 由 CFR 解、range 只用**当前街之前** reach、`raises_on_street=0`），解完沿 within-round
抽象动作序**导航**到当前决策点、读真实手该节点策略；**round-stable seed**（街索引而非 decision ordinal）→
同一轮多决策共享 **byte-identical** solve、读不同节点 = 一个均衡内自洽（= §6 #2「冻结」的**一致性意图**，无需
显式 reach-pin）。决策环每街首决策点 snapshot `round_start`（`blueprint_advisor.rs`）。`query_at(node_id,
game_state)` 泛化 `root_query`（读 off 用 `auth` **当前几何**——深层节点合法集须当前几何，round-start template
几何不符）；`navigate_subtree` 重放 within-round tags。探针 `--resolve`。

**测试 / harness 验证**：vultr lib **76/0/8**（+2：`navigate_subtree`、RoundStart within-round 导航 + round-stable
可复现）；集成测试改 AllPostflop+RoundStart exercise 全路径（守恒 + 可复现）。**两个 control 钉死 harness 一致**：
current-decision×all-postflop 复现 §10.4 的 **−192**（byte-equal 旧路径）；flop-first 复现 **−72**。

**主 A/B（24k 手 @3000 iters，真 1B nolimp ckpt，range-on，同 seed 池配对）**：

| arm | mbb/g | CI95 | fallback | SB / BB |
|---|---|---|---|---|
| round-start × all-postflop | **−407** | [−604, −211] | 6.1% | −1111 / −1249 |
| current-decision × all-postflop（§10.4 基线） | −192 | [−376, −8] | 3.7% | −693 / −972 |
| flop-first × round-start（sanity） | −62 | [−222, +98] | 4.2% | −557 / −685 |

→ **round-start 在等迭代下比 current-decision 更差**（−407 vs −192），**5/6 位一致更差**（仅 HJ 持平 → 非单点噪声）。

**根因 = deep-node 欠训练（不是 §6 landmine 修复奏效，反更差）**：

- **ARM3 是判别器**：flop-first round-start 读 subtree **root**（拿全 3000 iters → 训透）→ 持平（−62）；
  all-postflop round-start 读**深层**节点（只摊到一小撮 iters → 漂向 uniform）→ 退化（−407）。**同一套 round-start
  机制，唯一差别 = 读 off 节点深度。**
- round-start 解**整条街**子树、3000 iters 摊薄 → 实际（深层）决策节点欠训练 → 近 uniform → 盲位（正确打法离
  uniform 最远）最可剥削。current-decision 解**当前点起**的小子树 → 决策节点 = root 拿全 iters → 训得好。
- **reframe**：current-decision 的 `estimate_range` 已沿**全路径**（含 within-round）累乘 blueprint σ → range 已
  条件化于本轮 betting（用 blueprint 策略）。round-start 唯一多出的是 **CFR 均衡**的 within-round 条件化——边际
  收益小，不值欠训练代价。

**迭代扫（12k 手，确认 iter-bound）**：

| iters | current-decision | round-start |
|---|---|---|
| 3000 | −426 | −527 |
| 12000 | −310 | −366 |
| 24000 | **−273** | **−287** |

→ ① **两模式都随 iters 单调改善**（欠训练确是因子之一）；② round-start 改善更陡（−240 vs −153，深层更欠训练 →
headroom 更大）；③ **两模式随 iters 收敛**（gap 100→14）→ round-start 的「正确性」收益 ≈ **中性**、不 beat
current-decision；④ **但两模式都 plateau 在负**（−273/−287 @24k，**CI 上界仍 < 0**），改善减速（current-decision
末步仅 −37）→ 收敛到 ~−250~−290 的**负 plateau，不是 0**。

**关键结论（修正 §10.4）**：

1. **§6 #1 round-start re-solve 不是退化的杠杆**。§10.4 把残余 −192 归因「= §6 round-start landmine」**是错的**：
   实现 round-start **不修复**它（等迭代更差、高迭代持平）。round-start 是正确的 Pluribus 机制、保留可用，但
   **不 beat** current-decision。
2. **all-postflop 退化 = 部分欠训练（iters 修一截：−426→−273）+ 主要残余结构 ~−270**（iters 修不掉，CI<0）。
   残余 = **§2 materialized**：在**近似**（per-seat marginal range + 桶粒度）的 blueprint 派生子博弈上、对
   mid-street 多人点求解，**劣于 1B 训练的 blueprint 自身在那些点的响应**。flop-first（干净的 round-start 点、
   训透）保持中性（−62）。
3. **瓶颈是 blueprint/抽象质量（§2），不是搜索的根**。正是设计 §2 预警的弱基底风险落地（「在弱基底上建续局，
   搜索放大 blueprint 系统偏差而非修正」）。MVP 探针（路线乙）已**回答 §2**：当前 blueprint 对 all-postflop 搜索
   **太弱**，搜索帮不上、反退化；而 **flop-first（窄、干净点）中性不亏**。

**默认**：`trigger=FlopFirstUnraised`（中性、安全）+ `resolve_root=RoundStart`（正确机制；flop-first 下与
current-decision 计算等价）。`AllPostflop` 留研究 opt-in。

**下一步（战略岔路，待用户拍板）**：all-postflop 退化是 blueprint 质量的 cap（§2），**搜索 root / 迭代修不掉**。选项：

- **(甲) 强化 blueprint**：更多训练 / 更细抽象，给搜索好 range，再测搜索（设计 §2 路线甲；blueprint-only 单独
  已知达不到 Pluribus 级，但搜索的前提是 blueprint 近均衡）。
- **(6b) biased leaf / depth-limit**：Pluribus 真 6b——不解到终局而截 depth-limit 查 biased 续局值；可能换种方式
  取值，但仍建在弱 blueprint 上、§2 风险不消。
- **(丙) 收尾 flop-first-only**：接受 flop-first 搜索中性（不亏不赚）作 S6 MVP 终点，主力回 blueprint（甲）。
- **(丁) 更好 range 建模**：joint 而非 marginal / 更细桶，测残余是否 marginal 近似所致（隔离 §2 内的 range 分量）。
...
- **真 1B nolimp ckpt depth-limit smoke（flop-first，100 手/座 = 600 手，leaf-hands 20000）**：叶子值表从
  1B blueprint self-play 建成 **78949 项**（unbiased 覆盖 20503）、**无 OOM**（峰值 RSS 2.6 GiB）；**desync=0 /
  illegal=0**（depth-limit 路径在真数据上 plumbing 健康——叶子→全局映射 + 值表查询 + CFR 不破 lockstep/记账）；
