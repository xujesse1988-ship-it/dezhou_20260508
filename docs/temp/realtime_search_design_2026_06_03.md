...
### 10.1 进度（2026-06-03，分支 `6max-rts-mvp` commit `cf9efdb`，vultr 验证；**未并入 6max**）
...
| 5 | smoke（vultr）：6 新测试全过 + 全 lib 67/67 无回归（byte-equal 守门未破）；本地 build/fmt/clippy `--all-targets -D warnings` 绿 | ✅ |
