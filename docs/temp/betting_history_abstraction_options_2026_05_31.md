...
### A3. first-bet-small（小注只当开池领打，re-raise 一律大注）— 实测否决
...
放进谱系：比 `{1.0,2.0}`（10.0M/62 GiB）大 **3.6×**，甚至比 `{0.5,1.0}` cap=1（30.75M/199 GiB）还大。
**否决——离 64 GiB 差 3.5×，raise cap 叠加也救不动（cap 不绑这种深多路续局）。**
...
→ **A 类（结构性削减：raise cap / 规范化 / first-bet-small）整体挡不住小注**；要既留小注尺寸又进单机，
只能上 B 类有损摘要（把深多路下注历史折叠成有界 SPR/底池桶）。
...
- **A 类（raise cap / 状态规范化 / first-bet-small）风险最低、能与 B/C 叠加，但实测对小注无效**
  （§A1 + §A3：raise cap、first-bet-small 单用都救不了 `{0.5,1.0}`，均 ≥199 GiB）。

## 对我们最对症的组合

爆炸 = **re-raise 深链（A1 可砍）+ 小注底池几何导致的多路深筹码续局（结构性削减砍不动，见 §A1/§A3 实测）**。
实测已证结构性手段单用都救不了小注（`{0.5,1.0}` cap=1 仍 199 GiB、first-bet-small 仍 224 GiB）→ 若坚持要小注，最高杠杆 =
**A1 raise cap + B3 底池/SPR 桶化的 betting 摘要替掉 `node_id` 的完整路径**，B3 从"可选"升为"必需"。
本质 = 对下注历史做 imperfect-recall 抽象，也正是大规模求解器能把 6-max 塞进有界内存的原因——
代价是 bot 不再区分"底池怎么变大的"。
另一条更省事的路：**直接弃小注、只用少量大档 + A1**（`{1.0,2.0}` cap=1 已进 64 GiB），不碰有损摘要。
