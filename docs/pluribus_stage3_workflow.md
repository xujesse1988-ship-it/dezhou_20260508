...
- **2026-05-14（F1 [测试] closure）**：stage 3 F1 [测试] 完整落地 — 3 个新 integration crate + 1 baseline file + 1 capture script + `tests/api_signatures.rs` 同 commit 扩 stage 3 公开 API surface 最后一次 trip-wire（继承 stage 1 + stage 2 §F1 错误路径全覆盖 + 跨 host BLAKE3 baseline regression 模式）。
    - **`tests/checkpoint_corruption.rs`**（+ 12 测试 / ~330 行）：与 D1 `checkpoint_round_trip.rs::§3 5 类 CheckpointError 错误路径` 互补——D1 仅覆盖代表性触发路径（`0xDEAD_BEEF` schema flip + Kuhn↔Leduc / VanillaCfr↔EsMccfr variant flip + magic / pad / body byte flip / file too short），F1 在此之上加宽 **边界 / 极端值** 覆盖：
        - **schema_version 极值** 4 测试：`0` / `u32::MAX` / `SCHEMA_VERSION + 1`（典型 bump）/ `SCHEMA_VERSION - 1`（reader-on-newer-writer-on-older 边界）全部走 `SchemaMismatch` dispatch。
        - **trainer_variant / game_variant 越界 tag** 4 测试：trainer tag `2` / `0xFF`（仅 0/1 合法）+ game tag `3` / `0xFF`（仅 0/1/2 合法）全部走 `Corrupted { offset, reason: "unknown {trainer/game}_variant tag ..." }` dispatch（checkpoint.rs::parse_bytes line 215-233 字面）。
        - **bucket_table_blake3 mismatch (Kuhn 路径)** 1 测试：`KuhnGame::bucket_table_blake3()` 默认 `[0; 32]`，header offset 60..92 翻成 `[0xCC; 32]` → `VanillaCfrTrainer::<KuhnGame>::load_checkpoint` 走 `preflight_trainer` 返回 `BucketTableMismatch { expected: [0;32], got: [0xCC;32] }`。default profile active，无 v3 artifact 依赖（与 D1 `bucket_table_mismatch_via_byte_flip_at_offset_60` release/--ignored 互补）。
        - **trailer BLAKE3 直接翻 + 截断** 2 测试：D1 走 body byte flip 间接触发 trailer mismatch；F1 直接翻 trailer 中段 byte + 截断 trailer 一半，两条等价错误路径都走 `Corrupted`。
        - **5 类 CheckpointError Display/Debug sanity** 1 测试：5 variant 各构造一份，确保 `format!("{}")` / `format!("{:?}")` 不 panic（thiserror Error trait + derive Debug sanity）。
    - **`tests/trainer_error_boundary.rs`**（+ 14 测试 / ~340 行）：5 类 `TrainerError` 全覆盖（API-313 / D-324 / D-325 / D-323 (UnsupportedBucketTable) / D-330 / D-351 propagation）。
        - **ActionCountMismatch** 5 测试：4 条 panic 路径（`RegretTable::accumulate` + `current_strategy` + `StrategyAccumulator::accumulate` + `average_strategy` 各 1 条 `#[should_panic(expected = "action_count mismatch")]` exercise D-324 panic 不变量）+ 1 条构造 trip-wire。
        - **OutOfMemory** 1 测试：构造 trip-wire（stage 3 监控未接入；rss_bytes/limit u64 字段名 / Display "RSS ... exceeded limit ..." 文案 lock）。
        - **UnsupportedBucketTable** 3 测试：2 条真实路径 — `SimplifiedNlheGame::new(BucketTable::stub_for_postflop(BucketConfig::new(100,100,100)))` 走 second branch 返回 `{ expected: 2, got: 0 }`（config-mismatch sentinel）+ `(500, 500, 100)` river-only mismatch 等价；1 条构造 trip-wire。
        - **ProbabilitySumOutOfTolerance** 1 测试：构造 trip-wire（stage 3 D-329 warn-only 不触发；got/tolerance f64 字段 + Display "regret matching ... tolerance" 文案 lock）。
        - **Checkpoint(...) propagation** 3 测试：`From<CheckpointError>` 自动转换 5 variant 全 propagate + `std::error::Error::source()` 链回 underlying；`Trainer::load_checkpoint` 真实路径 nonexistent file → `CheckpointError::FileNotFound` + 用户侧 `.map_err(Into::into)` 提到 `TrainerError::Checkpoint(...)` 等价路径。
        - **5 variant exhaustive match lock** 1 测试：与 `checkpoint_round_trip.rs::checkpoint_error_5_variants_exhaustive_match_lock` 同型，5 variant 闭门 match 编译期 trip-wire（添加第 6 类 variant 必须显式同步本函数）。
    - **`tests/cross_host_blake3.rs`**（+ 5 测试 / ~220 行 + `tests/data/checkpoint-hashes-linux-x86_64.txt` 32 行 baseline + `scripts/capture-checkpoint-hashes.sh`）：32-seed Kuhn 5-iter checkpoint BLAKE3 regression guard（D-347 / D-362 + 继承 stage 1 `cross_arch_hash.rs` + stage 2 `bucket-table-arch-hashes-linux-x86_64.txt` 模式）。
        - **within-process determinism** 1 测试：连跑 2 次 32 seed → byte-equal（D-347 / D-362 "固定 seed 重复 BLAKE3 byte-equal" 强 anchor，default profile ~100 ms）。
        - **同 host baseline regression guard** 1 测试：当前 (os, arch) 对应 `tests/data/checkpoint-hashes-{os}-{arch}.txt` 存在 → byte-equal 必过；缺失 → eprintln 跳过。F1 commit 首次落地 `linux-x86_64.txt` 32 行 baseline。
        - **跨 (os, arch) 对照** 1 测试：`linux-x86_64` vs `darwin-aarch64` 两份 baseline 都存在时 byte-equal 必过（aspirational，未达成不算 stage 3 fail；继承 stage 1 D-052 carve-out 模式）。
        - **seed 名单 sanity** 1 测试：32 个 seed 包含 0 / u64::MAX / 0xCAFE_BABE 等基础锚点；与 stage 1 `cross_arch_hash.rs::ARCH_BASELINE_SEEDS` 同名单（跨 stage 调试对照）。
        - **capture-only entry** 1 ignored 子测试：由 `scripts/capture-checkpoint-hashes.sh` 调用 dump 当前 host 32 行到 stdout，shell script 重定向写入 baseline 文件（与 stage 1 `capture-arch-hashes.sh` 同型）。
    - **`tests/api_signatures.rs`**（+ ~60 行 stage 3 §训练错误枚举 + Game trait const + 默认方法 + Checkpoint pub field lock 段落）：
        - **TrainerError 5 variant 构造 trip-wire**（API-313）：5 variant + `From<CheckpointError> for TrainerError` 函数指针 UFCS lock；变体重命名 / 字段类型漂移 / `#[from]` 移除立即 `cargo test --no-run` fail。
        - **Game::VARIANT const lock × 3 impl**（API-300-rev1）：`<KuhnGame as Game>::VARIANT` / `<LeducGame as Game>::VARIANT` / `<SimplifiedNlheGame as Game>::VARIANT` 类型锁。
        - **Game::bucket_table_blake3 默认方法 UFCS lock × 3 impl**（API-300-rev1）：3 个 impl 函数指针类型锁（Kuhn/Leduc 走 default 返回 `[0; 32]`；SimplifiedNlhe override 返回 `self.bucket_table.content_hash()`）。
        - **Checkpoint pub field 类型 lock**（API-350 / D-350）：8 个 pub field 类型 + 顺序锁；任一字段类型漂移立即编译期 fail。
        - **HEADER_LEN / TRAILER_LEN / from_u8 helper lock**：D-350 binary layout 头号不变量与 `checkpoint_round_trip.rs::d350_binary_layout_offsets_lock` 双重锁定。
    - **角色边界**：本 batch 仅触及 `tests/*` + `tests/data/checkpoint-hashes-linux-x86_64.txt` + `scripts/capture-checkpoint-hashes.sh` + 文档（本 §修订历史 entry + `CLAUDE.md` ## Repository status E2 → F1 closed + 下一步 改 F2 [实现]）。**0 改动**：`src/training/*` + `src/error.rs` + `src/lib.rs` 产品代码（继承 stage 1 + stage 2 §F1 / §B-rev1 角色边界政策；F1 是 [测试] 角色，产品代码改动留给 F2 [实现]）+ `Cargo.toml`（F1 不引入新 crate）+ `docs/pluribus_stage3_{validation,decisions,api}.md`（F1 [测试] 不修改决策 / API 文档；F2 [实现] 同 commit 修 1 条 doc drift `pluribus_stage3_api.md` §API-313 ⑤ `CheckpointError(#[from] CheckpointError)` → `Checkpoint(#[from] CheckpointError)`）。F1 [测试] agent 0 越界。
    - **5 道 gate 全绿**（debug profile / dev box）：`cargo build --all-targets` ✓ / `cargo fmt --all --check` ✓（同 commit rustfmt 自动调整 1 处 `assert!` 单行折行）/ `cargo clippy --all-targets -- -D warnings` ✓（无新 lint）/ `RUSTDOCFLAGS="-D warnings" cargo doc --no-deps` ✓ / `cargo test --no-run` ✓。
    - **F1 active 测试全绿**：`cargo test --test trainer_error_boundary --test checkpoint_corruption --test cross_host_blake3 --test api_signatures` 实测 12 + 14 + 4 + 1 = 31 passed / 0 failed / 1 ignored（`cross_host_capture_only`）。
    - **default profile 全套实测**：`cargo test --no-fail-fast` 实测 **44 sections / 306 passed / 9 failed / 65 ignored**（E2 closed 时 41 / 276 / 9 / 64 → F1 后扩 +3 sections / +30 passed / +0 failed / +1 ignored，与 F1 新增 4 个 test crate 数字 byte-equal 对齐）。9 failed 全在 `bucket_quality.rs`（`adjacent_bucket_emd_above_threshold_*` × 3 + `bucket_id_ehs_median_monotonic_*` × 3 + `bucket_internal_ehs_std_dev_below_threshold_*` × 3）= CLAUDE.md "v3 bucket quality 实测：19 测试 10 passed / 9 failed" 预存在 v3 baseline 失败，非 F1 引入；E2 closed 时偶发的 `bucket_table_corruption::random_byte_flip_smoke_10_no_panic` SIGKILL 本轮未复现（10 测试全过 348.59 s release-mode bucket_table_corruption 段），符合 1-CPU host OOM 偶发模式。F1 触达路径相关测试（trainer_error_boundary / checkpoint_corruption / cross_host_blake3 / api_signatures / checkpoint_round_trip / cfr_kuhn / cfr_leduc / cfr_simplified_nlhe）全绿不退化。
    - **F1 → F2 工程契约**：F1 [测试] 主线交付 = 5 类 CheckpointError 边界覆盖完整 + 5 类 TrainerError 全 dispatch + 32-seed cross-host BLAKE3 baseline + api_signatures stage 3 surface 最后一次 trip-wire；F1 暴露 0 产品代码 bug **on test-content axis**，但 F1-rev1 vultr 实测揭示 D-361 NLHE SLO 双 fail（详见 F1-rev1 carve-out 段落）。F2 [实现] **不能** 走 stage 2 §F2 字面 "0 产品代码改动 carve-out closure" 模式；必须先经 E2-rev1 [实现] 优化（真改 trainer 热路径）翻面 SLO 才能进 F2 / F3。F2 仍负责修 1 条已知 doc drift `pluribus_stage3_api.md` §API-313 ⑤ 变体名拼写一致化（doc=`CheckpointError(#[from] CheckpointError)` → code=`Checkpoint(#[from] CheckpointError)`，code 形态简洁更合 Rust convention），但放在 E2-rev1 之后。
- **2026-05-14（F1-rev1 carve-out — vultr NLHE SLO 双 fail / 触发 D-321-rev2 / E2-rev1）**：F1 [测试] closure commit 后由 F1 [测试] agent 接 vultr 64.176.35.138 4-core AMD EPYC-Rome 4 GB idle box 跑 D-361 NLHE 2 条 SLO 实测（继承 stage 2 §E-rev1 §5 vultr 50-run mean 复跑 + 错误前移政策；E2 → F1 工程契约字面第一任务）。
    - **环境**：vultr fetch + reset 到 F1 commit `72cc978` HEAD；本地 dev box rsync v3 artifact `bucket_table_default_500_500_500_seed_cafebabe_v3.bin`（528 MiB / whole-file b3sum `5a7d31fe...` / body BLAKE3 `67ee5554...`，CLAUDE.md 当前 artifact 基线段一致）+ `.b3sum` sidecar 到 vultr `~/dezhou_20260508/artifacts/`；release 编译 9.41 s（增量；bincode + tempfile + poker 重编）。
    - **实测**（`cargo test --release --test perf_slo stage3_simplified_nlhe -- --ignored --nocapture` 17.65 s 总耗时）：
        - **`stage3_simplified_nlhe_single_thread_throughput_ge_10k_update_per_s` FAIL**：实测 20,000 update / 4.637 s = **4,313 update/s**，SLO 门槛 ≥ 10,000 update/s（实测仅 **43.1%** 阈值，差 ~2.3 倍）。
        - **`stage3_simplified_nlhe_4core_throughput_ge_50k_update_per_s` FAIL**：实测 50,000 update / 10.145 s = **4,929 update/s**，SLO 门槛 ≥ 50,000 update/s（实测仅 **9.9%** 阈值，差 ~10 倍）。
        - **4-core efficiency**：4,929 / 4,313 ≈ **1.14x**（预期 ≥ 2.0x = 50% efficiency on 4 cores）；E2 [实现] D-321-rev1 真并发 `step_parallel`（thread::scope × thread-local accumulator + batch merge）实测**几乎无加速**——加速比 1.14× 而非预期 2-3×。
    - **根因分析**（候选，待 E2-rev1 实测确认）：
        - (1) **单线程 baseline 远低于预期**：E2 commit 预测 "(b) single-thread `≥ 10K update/s` 大概率通过（C2 closed 形态已无回归，E2 改动局限在 step_parallel 路径，单线程 step 0 字节改动）" 是 **未实测的 wishful thinking** — C2 / E2 / D2 / D1 的任何 commit 都没有在 vultr 4-core EPYC + v3 artifact 上跑过单线程 throughput；naive impl 实测 4.3K/s 是合理的 "B2 时序级未优化 baseline"，不是 regression。E2 → F1 工程契约这一句应该撤回。
        - (2) **多线程 efficiency 灾难性低**：D-321-rev1 thread-local + batch merge 设计虽然 byte-equal preserve，但 4-core 加速 1.14× 说明实测瓶颈不在 RegretTable 写竞争（thread-local 已规避），而在 (a) σ 计算走 `shared_regret.current_strategy` 内部 `Vec::new` + `Vec::with_capacity` 分配频繁（hot path 每 InfoSet × n_actions 一次 Vec alloc）；(b) batch merge 阶段 single-threaded HashMap iter + Debug-sort + accumulate 成主导成本（4 thread 累积的 delta tables 在 main thread 顺序合并）；(c) spawn/join overhead — 50K update / 4 thread = 12,500 次 `thread::scope` spawn，每次 spawn 创建 4 OS thread，按 Linux 估每 spawn ~10-50 μs，12,500 × 4 × ~30 μs = ~1.5 s overhead，约占总 10.145 s 的 15%。
    - **触发 D-321-rev2 / E2-rev1 翻面**（E2 → F1 工程契约字面候选）：
        - **D-321-rev2 候选**（thread::scope 改 thread pool 长寿命 worker / 减 spawn 频率）：rayon `par_iter` 池化 worker 替 `thread::scope`，让 12,500 次"调度"摊分到 4 长寿命 thread；spawn overhead 应 ≈ 0。或一次 `step_parallel` 内做更多 update（例如 per-call N=1000 update）摊分 spawn 成本。
        - **E2-rev1 hot path 优化** 候选（不破 BLAKE3 byte-equal）：(a) `RegretTable::current_strategy` 改 `SmallVec<[f64; 8]>` hot path 替 `Vec<f64>`，让 typical 5-action / 8-action 路径走 stack alloc（`SmallVec` crate 是 D-373 候选第 4 crate，需要 D-373-rev2 翻面）；(b) `sample_discrete` 内部 CDF binary search 改 lookup-table 加速（5-action 离散分布预 CDF 表 5 entry，常数小但调用 ~1-2M 次）；(c) `recurse_es_parallel` per-action `cfvs: Vec<f64>` 改 `SmallVec<[f64; 8]>`；(d) `state.clone()` per-recursion call 改 `Cow<'_, State>` / borrow 替（`SimplifiedNlheState { game_state: GameState, action_history: Vec<SimplifiedNlheAction>, bucket_table: Arc<BucketTable> }` 三字段中 `Arc::clone` 已便宜，但 `GameState` 内部字段 + `Vec<SimplifiedNlheAction>` 都走完整 clone）。
        - **D-322 strategy_sum_offset 增 batch merge 优化**：batch merge 阶段改并行 — 把 4 个 thread-local delta tables 按 InfoSet hash 分桶，让 4 个 thread 各自合并自己负责的桶；但破环 sort 决定性顺序，会破 BLAKE3 byte-equal（D-321-rev1 lock 段落明确：sort-by-Debug 顺序保 byte-equal）；候选 deferred 到 stage 4 起步，stage 3 不引入。
    - **角色边界（F1-rev1 carve-out 严格遵守 [测试] 角色）**：本 carve-out **0 产品代码改动**（不修 `src/training/*` 或 `src/error.rs`）；仅追加本 §修订历史 entry + 后续 commit 改 `CLAUDE.md` ## Repository status F1 → F1-rev1 / 下一步改 E2-rev1。E2-rev1 [实现] 由独立 commit 落地（继承 stage 2 §E-rev1 / stage 1 §F-rev1 错误前移模式：F1 [测试] 暴露不达标 → E2-rev1 [实现] 真优化）。
    - **F1-rev1 → E2-rev1 工程契约**：E2-rev1 [实现] agent 任务清单：(a) 实测确认根因（在 dev box 上跑 `cargo bench --bench stage3 nlhe_es_mccfr_update` + criterion profile，定位 hot path）；(b) 选择 D-321-rev2（rayon thread pool）或 E2-rev1 hot path 优化（SmallVec / lookup-table）或两者结合；如选 SmallVec → 走 D-373-rev2 翻面引入 `smallvec` 第 4 crate；(c) 优化后 vultr 复跑 SLO 翻面 — 单线程 ≥ 10K + 4-core ≥ 50K **必须双过**；(d) `cfr_simplified_nlhe.rs::test_5_blake3_repeat_byte_equal_release` 100M update × 3 BLAKE3 byte-equal 必须不退化（D-362 强 anchor）；(e) E1 落地的 6 SLO 测试除 NLHE 单线程 + 4-core 之外的 4 条（Kuhn / Leduc 阈值）必须不退化；(f) 5 道 gate 全绿；(g) E2-rev1 closure commit 后由用户授权 F2 [实现] 接 doc drift 修复；F2 → F3 [报告] 路径恢复。**E2-rev1 时间预算**：~1 周（hot path 优化 + criterion profile + vultr 复跑），与原 E2 0.5 周预算同型超额 carve-out。
- **2026-05-14（E2-rev1 [实现] code shipped — vultr SLO 实测 pending followup commit）**：F1-rev1 → E2-rev1 工程契约任务清单 (a)..(f) 落地（不含 (g) closure，留 vultr 实测 followup commit 翻面）。E2-rev1 [实现] agent 在 dev box 上落实 SmallVec hot path + rayon thread pool + append-only delta merge 三条优化路径（详见 `docs/pluribus_stage3_decisions.md` §10.5 D-373-rev2 lock + §10.6 D-321-rev2 lock）。
    - **`Cargo.toml` + `Cargo.lock`**（D-373-rev2 翻面，详见 §10.5）：追加 `smallvec = "1"` 第 4 stage 3 crate（bincode + tempfile + serde + smallvec）。`rayon` 不算 stage 3 新增 crate（stage 2 §G-batch1 §3.4-batch1.5 已引入用于 production artifact 训练 par_iter；E2-rev1 仅消费已有依赖）。
    - **`src/training/regret.rs`**（+149 行）：新增 `pub(crate) type SigmaVec = SmallVec<[f64; 8]>` + `pub(crate) struct LocalRegretDelta<I>` / `pub(crate) struct LocalStrategyDelta<I>` 容器（`Vec<(I, SigmaVec)>` append-only DFS-顺序 push，无 dedup / 无 sort，merge 时按 push 顺序 playback）+ `pub(crate) fn current_strategy_smallvec(&self, info_set, n) -> SigmaVec` hot path 入口（`pub fn current_strategy` 公开 API-320 surface 不变，trait 入口仍 `Vec<f64>`）。helper 类型全 `pub(crate)` 不进 D-376 公开 API surface，亦不进 `tests/api_signatures.rs` 公开 trip-wire（与 D-376 + 内部 helper 政策一致）。
    - **`src/training/trainer.rs`**（+230 行 / 重写 ~85 行 step_parallel 主体）：(a) `EsMccfrTrainer::step_parallel` 由 `std::thread::scope` 改 rayon `par_iter_mut().enumerate().collect()` 长寿命 worker pool dispatch（解 12,500 spawn × 4 OS thread = ~1.5 s overhead）；(b) batch merge 由 `(RegretTable, StrategyAccumulator)` thread-local + `format!("{:?}", I)` × O(N log N) Debug-sort merge 改 `(LocalRegretDelta, LocalStrategyDelta)` append-only + main thread playback merge（按 tid 升序 × 每 thread 内 push 顺序，merge cost O(N log N × \|Debug\|) → O(N)）；(c) `recurse_es` / `recurse_es_parallel` / `recurse_vanilla` 三处 hot path 短向量改 `ShortVec<T> = SmallVec<[T; 8]>`（per-action `cfvs` / `delta` / `weighted` / `nonzero_dist`）；(d) 删除 `merge_regret_delta` / `merge_strategy_delta` 旧 helper（替换为 inline append-loop）；(e) 新增 `where G: Sync, G::InfoSet: Send` bound on step_parallel（rayon par_iter 跨 worker move `LocalRegretDelta<I>` 需 `I: Send`，3 个 Game impl InfoSet 类型自动满足，api_signatures 函数指针 byte-equal 维持）。
    - **角色边界**：本 batch 仅触及 `src/training/{trainer, regret}.rs` 产品代码 + `Cargo.toml` / `Cargo.lock` + 文档（本 §修订历史 entry + `CLAUDE.md` ## Repository status F1-rev1 → E2-rev1 ship + `docs/pluribus_stage3_decisions.md` §10.5 + §10.6 lock）。**0 改动**：`tests/*`（继承 stage 2 §B-rev1 / stage 3 §B-rev0 / §C2 / §D2 / §E1 / §E2 角色边界政策；F1 落地的 30 active 测试 + 1 ignored 0 改动；api_signatures 不引入新 trip-wire 因 SigmaVec / LocalRegretDelta / LocalStrategyDelta 全 pub(crate) 不进公开 API）+ `src/error.rs`（错误枚举追加不删，本 rev 不新增 variant）+ `src/lib.rs`（顶层 re-export 不变）+ `docs/pluribus_stage3_{validation, api}.md`（API surface 不变）。E2-rev1 [实现] agent 0 越界。
    - **5 道 gate 全绿**（debug profile / dev box 1-CPU）：`cargo build --all-targets` 21.94 s ✓ / `cargo fmt --all --check` ✓（agent 已 fmt，本人手动 pub→pub(crate) 收紧后无 fmt 漂移） / `cargo clippy --all-targets -- -D warnings` 9.72 s ✓（无新 lint；`#[allow(dead_code)]` × 4 加在 `LocalRegretDelta::{len, is_empty}` + `LocalStrategyDelta::{len, is_empty}` 因仅 monitor 用未在 trainer 调用）/ `RUSTDOCFLAGS="-D warnings" cargo doc --no-deps` ✓ / `cargo test --no-run` ✓。
    - **D-362 1M update × 3 BLAKE3 byte-equal anchor PASS**：`cargo test --release --test cfr_simplified_nlhe -- --ignored` 实测 1676.89 s（~28 min on 1-CPU dev box；vultr 4-core EPYC 应 ~10 min）：`simplified_nlhe_es_mccfr_1k_update_no_panic_no_nan_no_inf` ok + `simplified_nlhe_es_mccfr_fixed_seed_repeat_3_times_blake3_identical_1m_update` **ok**。SmallVec hot path 替换在单线程 `step` 路径数值等价维持（`f64` 算术与原 `Vec<f64>` 路径完全等价 — 同样 R⁺ 累积 + 除法归一化 + per-action cfv / delta sequence 顺序不变）。`step_parallel` 路径不在此 anchor 范围（perf_slo 仅 throughput 不断言数值；`step_parallel` 跨 run determinism 由 §10.6 D-321-rev2 lock 段落 "DFS 顺序 + tid 顺序 + push 顺序" 三重保证）。
...
- **2026-05-14（F2 [实现] closure — 0 产品代码改动 doc drift 修复，继承 stage 2 §F2 字面模式）**：F2 [实现] 主线交付 = 修 1 条已知 doc drift `pluribus_stage3_api.md` §API-313 ⑤ 变体名 doc=`CheckpointError(#[from] CheckpointError)` → code=`Checkpoint(#[from] CheckpointError)`（code 形态简洁更合 Rust convention，与 `src/error.rs:141` 字面一致 + `tests/api_signatures.rs:630` / `tests/trainer_error_boundary.rs:329` / `tests/trainer_error_boundary.rs:368` / `tests/trainer_error_boundary.rs:394` 4 处真实代码引用一致）。
    - **0 产品代码改动 lock**：本 commit 触及 `docs/pluribus_stage3_api.md`（1 行 variant 名修复）+ `docs/pluribus_stage3_workflow.md`（本 §修订历史 entry 追加）+ `CLAUDE.md`（## Repository status E2-rev1 closure → F2 closure ship + 下一步改 F3 [报告]）；0 `src/training/*` + 0 `src/error.rs` + 0 `src/lib.rs` + 0 `tests/*` + 0 `Cargo.toml` 改动。继承 stage 2 §F2 字面 "0 产品代码改动 carve-out closure" 模式 — F1 [测试] 主线交付 = 0 产品代码 bug on test-content axis（F1-rev1 揭示的 SLO 问题已经由 E2-rev1 [实现] 优化吸收 + E2-rev1 Option C closure 接受），F2 退化为纯 doc drift 修复。
    - **5 道 gate 全绿（F2 commit）**：`cargo build --all-targets` ✓ / `cargo fmt --all --check` ✓ / `cargo clippy --all-targets -- -D warnings` ✓ / `RUSTDOCFLAGS="-D warnings" cargo doc --no-deps` ✓ / `cargo test --no-run` ✓。default profile `cargo test` 维持 stage 3 E2-rev1 closure 之后形态（v3 bucket quality 9 failed 是 §G-batch1 §3.10 v3 artifact 预存在基线，详见 CLAUDE.md "v3 bucket quality 实测" 段落，非 F2 退化）。
    - **F2 → F3 工程契约**：F2 closure commit 后 → F3 [报告]（stage 3 验收报告 `docs/pluribus_stage3_report.md` + git tag `stage3-v1.0` + Kuhn / Leduc / 简化 NLHE 100M update milestone checkpoint artifact 上传 GitHub Release，详见 §步骤 F3）。**F3 触发条件**：vultr 4-core EPYC 实测 stage 3 6 SLO 完整数字（D-361 NLHE 双 fail 数字写入 §8 已知偏离 + carry-forward 到 stage 4 起步并行清单）+ Kuhn 1000× / Leduc 10× BLAKE3 byte-equal 实测 + 简化 NLHE 100M update vultr 复跑（D-362 3× BLAKE3 byte-equal）+ OpenSpiel 收敛轨迹趋势对照实测（D-366 `tools/external_cfr_compare.py` 一次性 instrumentation + `docs/pluribus_stage3_external_compare.md` 对照报告）+ checkpoint artifact Python reader `tools/checkpoint_reader.py`（D-357）。F3 时间预算继承 §时间预算汇总字面 0.5 周（vultr 实测复跑 + 报告起草 + git tag）。
    - **角色边界**：F2 [实现] agent 0 越界；本 closure entry 与 §10.1..§10.6 D-NNN-revM lock entry 同型 stage 3 doc closure 模式（[实现] 角色可修 doc drift + §修订历史 entry，但不修改决策 / API 签名 / 测试形态）。F2 closure 是 stage 3 最后一次 [实现] 角色 commit；后续 F3 [报告] 是 [报告] 角色（出验收报告 + git tag + GitHub Release artifact 上传）。

- **2026-05-14（F3 [报告] 起步 batch 1 — tools/ 一次性 instrumentation 落地 + D-362 anchor 用户授权降标 100M → 10M × 3）**：F3 [报告] 起步落 3 个一次性 instrumentation 工具，对应 §步骤 F3 字面 deliverables 中 (c) (d) 两条 + (b) 实测路径所需 anchor binary。`docs/pluribus_stage3_report.md` + `docs/pluribus_stage3_external_compare.md` + git tag + GitHub Release 在 vultr 实测数字回来后由后续 batch 闭合。
    - **D-362 anchor 用户授权降标 carve-out**：原 §步骤 F3 字面 "简化 NLHE 100M update D-362 BLAKE3 anchor" = 单 100M run；用户授权 F3 推进时降标为 **10M × 3 run BLAKE3 byte-equal**（继承 D-362 NLHE 3× 字面口径，规模 100M → 10M 收窄）。**降标动机**：(a) 100M × 3 = 30M update vultr 4-core ~12 h（按 E2-rev1-vultr-measured 7,741 update/s 计算），10M × 3 = 30M update / × 3 重复 = ~65 min vultr，可单批闭合 (b) 实测路径；(b) D-362 字面字面只要求 NLHE 3× BLAKE3 byte-equal 重复确定性，100M vs 10M 在 D-362 字面 unchanged 范围内（D-362 不锁定 update 规模）；(c) 100M scale 验收门槛由 `tests/simplified_nlhe_100M_update.rs::simplified_nlhe_es_mccfr_100m_update_no_panic_no_nan_no_inf` 既有 release/--ignored 测试承担（无 BLAKE3 byte-equal 维度，仅 no-panic/finite/sum=1 维度），不在本 F3 D-362 anchor 范围。**carve-out**: stage 3 出口 D-362 NLHE 3× BLAKE3 byte-equal anchor scale = `10M × 3 run`（非 100M），列入报告 §8 已知偏离 + carry-forward 到 stage 4 起步并行清单（评估提升回 100M 必要性）。**SLO 阈值 D-362 字面 unchanged**（不走 D-362-revM，路径与 E2-rev1 Option C 同型 known-deviation carve-out 而非降阈值）。
    - **`tools/checkpoint_reader.py`（D-357 落地）**：minimal Python 跨语言 reader，解码 108-byte header + bincode body（Vec<(InfoSet, Vec<f64>)>）+ 32-byte BLAKE3 trailer。变体感知 InfoSet decoder：Kuhn (`actor:u8 + private_card:u8 + history:u32_tag`) / Leduc (`actor:u8 + private_card:u8 + public_card:Option<u8> + street:u32_tag + history:Vec<u32_tag>`) / SimplifiedNlhe (`InfoSetId(u64)` newtype = 8 bytes LE)。bincode 1.x default config：fixint LE + 4-byte enum variant tag + 1-byte Option discriminant。错误路径覆盖 magic / schema / trainer_variant tag / game_variant tag / pad / trailer BLAKE3 / 偏移越界 / 文件过短 共 7 类。Python self-test 合成 Kuhn/Leduc/NLHE round-trip + 7 错误路径全 PASS；真 artifact 端到端验证由后续 F3 vultr sweep 落地。继承 stage 1 `tools/history_reader.py` + stage 2 `tools/bucket_table_reader.py` minimal-stdlib + 可选 `pip install blake3` 同型。
    - **`tools/external_cfr_compare.py`（D-366 落地）**：OpenSpiel 收敛轨迹趋势对照 sanity 脚本，接 PyPI `open_spiel.python.algorithms.cfr.CFRSolver`，跑 Kuhn / Leduc 各 10K iter，在 D-364 字面 4 sample point `(1K, 2K, 5K, 10K)` 取 `exploitability` 数值。D-365 P0 阻塞检查：4 点单调下降（允许 ±5% 早期 sampling noise 容忍），不满足返回 exit code 1。dev box 实测：Kuhn 10K iter ~19 s，OpenSpiel 4 sample point `(0.000938, 0.000539, 0.000180, 0.000113)` 单调下降 trend PASS（vs 我们 Rust 端 10K iter 0.000148，量级一致）。Leduc 10K iter 实测 ~89 min dev box 单线程（CFRSolver Python 实现单 InfoSet ~50K，无并行）。
    - **`tools/nlhe_blake3_anchor.rs`（F3 anchor binary）**：F3 [报告] 一次性 instrumentation Rust binary，继承 stage 2 `tools/bucket_quality_dump.rs` F3 [报告] tools/* entry 模式。`Cargo.toml [[bin]] name = "nlhe_blake3_anchor" path = "tools/nlhe_blake3_anchor.rs"` 追加。CLI: `--artifact <path>` + `--updates <n>`（默认 10M）；跑 `EsMccfrTrainer::step` × `updates` × 3 run with FIXED_SEED = `0x46_33_5F_4E_4C_48_45_5F`（"F3_NLHE_" ASCII，与 stage 3 C1 [测试] / D1 [测试] FIXED_SEED 不重合避免 cross-contamination）；每个 run 结束 `blake3_avg_strategy_snapshot` 与既有 `tests/cfr_simplified_nlhe.rs::blake3_avg_strategy_snapshot` 同型 hashing；断言 3 个 BLAKE3 byte-equal。5 道 gate 全绿（cargo fmt / clippy --all-targets -- -D warnings / cargo doc / cargo build --release 编译 40.38 s 含 stage 1+2+3 全产品代码 link）。
    - **角色边界**：本 batch 落地仅 `tools/*` 一次性 instrumentation + `Cargo.toml` [[bin]] entry 追加 + `docs/pluribus_stage3_workflow.md`（本 §修订历史 entry 追加）。0 `src/training/*` + 0 `src/error.rs` + 0 `src/lib.rs` + 0 `tests/*` + 0 `docs/pluribus_stage3_{validation,decisions,api}.md` 改动；继承 stage 2 F3 [报告] tools/* 一次性 instrumentation 模式（`bucket_quality_dump.rs` + `external_compare.py` + `bucket_table_reader.py` 同型 entry）。F3 [报告] 角色 carve-out：tools/* 工具新增允许（不动产品代码 + 不动测试代码），不在 [实现]↔[测试] 边界冲突范围。`CLAUDE.md` ## Repository status 翻面到 "F3 [报告] 起步 batch 1 closed，下一步 vultr 实测 + 报告起草 + git tag" 由后续 batch 闭合（避免本 commit 引入 CLAUDE.md drift 后又被 batch 2 覆盖）。
    - **F3 [报告] 起步 → vultr 实测 + 报告起草 工程契约**：F3 [报告] 起步 batch 1 闭合后，下一步路径（顺序顺序推进，由用户授权后由 [报告] agent 一次性闭合）：(a) vultr fetch/reset 到本 commit；(b) vultr 跑 `cargo test --release --test perf_slo -- --ignored --nocapture stage3_` 取 D-360..D-369 全 6 SLO 完整数字；(c) vultr 跑 `cargo test --release --test cfr_kuhn cfr_leduc cfr_simplified_nlhe -- --ignored` 取 Kuhn 1000× / Leduc 10× / NLHE 1M × 3 BLAKE3 byte-equal 复测数字（继承 B2 closure 出口数据 + E2-rev1-vultr-measured carve-out NLHE 双 fail 已知偏离）；(d) vultr 跑 `cargo run --release --bin nlhe_blake3_anchor -- --updates 10000000` 取 10M × 3 NLHE BLAKE3 anchor + 3 run wall time + throughput；(e) dev box 完成 `tools/external_cfr_compare.py --game both --output-md docs/pluribus_stage3_external_compare.md` Leduc 10K iter 后写 OpenSpiel 对照 markdown；(f) 起草 `docs/pluribus_stage3_report.md` §1..§11 完整（§8 含 D-361 NLHE 双 SLO + D-362 100M → 10M × 3 anchor 降标 2 条已知偏离 + carry-forward 到 stage 4 起步并行清单）；(g) git tag `stage3-v1.0` + GitHub Release artifact 上传（由用户授权后由 [报告] agent 落地）。F3 [报告] 时间预算继承 §时间预算汇总字面 0.5 周（本 batch ~半天 + vultr 实测 ~2 h + 报告起草 ~半天 + git tag ~1 h）。

- **2026-05-14（F3 [报告] closure — stage 3 13 步全闭合）**：F3 [报告] batch 2 闭合 stage 3 全部 13 步 A0..F3。vultr 4-core EPYC sweep 84.7 min wall 完成 6 SLO + Kuhn 1000× + Leduc 10× + NLHE 1M × 3 + NLHE 10M × 3 anchor 实测；`docs/pluribus_stage3_report.md` §1..§11 完整 + `docs/pluribus_stage3_external_compare.md` Kuhn / Leduc 4 sample point 对照完整 + CLAUDE.md ## Repository status 翻面到 "stage 3 closed at stage3-v1.0"。
    - **vultr sweep 84.7 min 实测出口数据**（commit `71d2d89` F3 [报告] 起步 batch 1 vultr sweep）：
        - **STEP 1 build release**：6.61 s（release profile incremental，nlhe_blake3_anchor binary 编译）。
        - **STEP 2 6 SLO (perf_slo stage3_*) 34.27 s**：4 PASS + 2 FAIL。Kuhn 10K 0.102 s (97,731 iter/s, SLO `<1s` ~10× 余量) ✅ / Leduc 10K 34.265 s (292 iter/s, SLO `<60s` ~1.75× 余量) ✅ / Kuhn BR 0.03 ms (SLO `<100ms` ~3300× 余量) ✅ / Leduc BR 19.14 ms (SLO `<1s` ~52× 余量) ✅ / NLHE 单线程 4,428 update/s ❌ (SLO `≥10K` 44% 阈值，与 E2-rev1-vultr-measured 4,357 update/s ±2% noise 一致) / NLHE 4-core 7,588 update/s ❌ (SLO `≥50K` 15% 阈值，与 E2-rev1 7,741 update/s ±2% noise 一致)。
        - **STEP 3 Kuhn 1000× BLAKE3 96.10 s**：seed `0x5a5a5a5a5a5a5a5a` BLAKE3 = `7dc587a464e68a72…35e9787fc09593b9c429a76aed0be8b1b12ec672c749eb07`，1000 run × 10K iter Vanilla CFR fixed-seed byte-equal PASS ✓（与 B2 closure dev box BLAKE3 byte-equal 跨 host 确定性强 anchor）。
        - **STEP 4 Leduc 10× BLAKE3 350.02 s**：seed `0xa5a5a5a5a5a5a5a5` BLAKE3 = `ee2d6e0a01093cae…5397480944 7fd0708b1cc13ac892f4c7beec0a75fd41e4a8`，10 run × 10K iter byte-equal PASS ✓ + 10K iter expl `0.093582 < 0.1` ✅；curve test 5% tolerance fail 维持 §B-rev0 carve-out 字面 deferred 状态。
        - **STEP 5 NLHE 1M × 3 BLAKE3 453.13 s**：seed `0x534e4c48455f4331` ("SNLHE_C1" ASCII) BLAKE3 = `8fa6a8fd284d25fdbc9cfff0700306dc884a0966da17b98d895a521fd7d1763a`，3 run × 1M update byte-equal PASS ✓（vultr 4-core 实测 ~7.55 min total vs dev box E2-rev1 ~28 min total，vultr 实测显著快于 dev box，warm-up 后 ~7,170 update/s 单线程稳定）。
        - **STEP 6 NLHE 10M × 3 BLAKE3 anchor 4192.8 s ≈ 69.9 min**：seed `0x46335f4e4c48455f` ("F3_NLHE_" ASCII) BLAKE3 = `9e8258d1d76cf002e2f86fe3203f76e0b0f082eb659143bb081f64ad5ae5a8c9`，3 run × 10M update byte-equal PASS ✓ — **D-362 NLHE 3× BLAKE3 anchor 100M → 10M × 3 用户授权降标后实测 PASS**。总 30M update / 4192.8 s = **7,155 update/s avg throughput**（warm-up 后稳定 ~7,170 update/s vs perf_slo 短测 4,428 update/s 快 62%，setup overhead 短测主导 — F3 sweep 实测刷新 NLHE 单线程 real-world throughput baseline）。
    - **Leduc OpenSpiel python 10K iter 实测出口数据**（dev box Python 3.10.12 + open_spiel 1.6.11）：4 sample point `(1K, 2K, 5K, 10K)` exploitability = `(0.011818, 0.006849, 0.003552, 0.002042)`，wall = `(533.1, 942.9, 1825.9, 3164.6) s` 总 ~52.7 min；trend 严格单调下降 (5.8× 下降 1K→10K) — D-365 P0 阻塞条件未触发 ✓。OpenSpiel `cfr.CFRSolver` 默认 alternating updates + average_policy `linear weighting`，与我们 Rust `VanillaCfrTrainer` simultaneous updates + uniform weighting 数学公式微妙不同 — D-364 字面口径数值差异（OpenSpiel 0.002042 vs 我们 0.094 ~46× 差）不构成 stage 3 验收阻塞，趋势 trend match 通过条件成立。
    - **D-342 100M no-panic + D-343 sublinear monitor deferred 到 stage 4 carve-out（§8.1 第 2 条 carve-out 扩展）**：F3 [报告] 起步 batch 1 用户授权 D-362 anchor 100M → 10M × 3 降标后，D-342 字面 "100M update no-panic" 验收时间预算 ~6.27 h vultr 4-core 单 run 不在 F3 时间预算内；deferred 到 stage 4 起步并行清单 (V)。**替代证据**：(i) 10M × 3 = 30M update vultr 113 min wall 无 panic / NaN / inf 隐式覆盖 D-342 1/3 规模工程稳定性；(ii) 1M × 3 既有 anchor（B2 closure + E2-rev1 dev box + F3 vultr 复测）。**D-342 / D-343 字面 path.md spec unchanged**（不走 D-NNN-revM）；与 D-361 NLHE 双 fail 同型 known-deviation carve-out 而非降阈值。
    - **F3 出口检查清单复核 13 ✅ + 3 ⏸ + 2 ❌**：13 项 PASS + 3 项 ⏸ carve-out（D-342 100M no-panic deferred / D-343 100M sweep deferred / `tools/train_cfr.rs` CLI 主体未实现，全 stage 4 carry-forward）+ 2 项 ❌（D-361 NLHE 双 fail，§8.1 第 1 条已知偏离）。**stage 3 acceptance 通过显式 known-deviation carve-out 模式**（继承 stage 2 §G-batch1 §3.4-batch2..§4 + §8 carve-out 模式 + E2-rev1 Option C accepted 字面口径），path.md / D-NNN spec unchanged。
    - **5 道 gate 全绿（F3 closure commit）**：`cargo build --all-targets` ✓ / `cargo fmt --all --check` ✓ / `cargo clippy --all-targets -- -D warnings` ✓ / `RUSTDOCFLAGS="-D warnings" cargo doc --no-deps` ✓ / `cargo test --no-run` 47 sections ✓。default profile `cargo test --no-fail-fast` 276 passed / 9 failed / 64 ignored（9 failed 全为 stage 2 §G-batch1 §3.10 v3 artifact `bucket_quality` 预存在基线，非 stage 3 退化）。
    - **角色边界**：F3 [报告] batch 2 闭合 commit 落地 `docs/pluribus_stage3_report.md`（新增）+ `docs/pluribus_stage3_external_compare.md`（新增）+ `docs/pluribus_stage3_workflow.md`（本 §修订历史 entry 追加）+ `CLAUDE.md`（## Repository status F3 [报告] 起步 → closure 翻面）；0 `src/training/*` + 0 `src/error.rs` + 0 `src/lib.rs` + 0 `tests/*` + 0 `Cargo.toml` 改动 + 0 `tools/*` 改动；继承 stage 2 F3 [报告] tools/* 一次性 instrumentation + report 落地同型 commit 形态（stage 2 F3 [报告] closure commit `48f5e76` 仅 docs/*，stage 3 F3 [报告] closure 与之同型）。
    - **stage 3 → stage 4 工程契约**：(a) F3 closure commit 后由用户授权 `git tag stage3-v1.0` 指向同 commit；(b) 由用户授权后 milestone checkpoint artifact（Kuhn / Leduc / 简化 NLHE 10M anchor checkpoint snapshot）通过 `cargo run --release --bin nlhe_blake3_anchor` + `tools/train_cfr.rs` D-372 / API-370 CLI 主体落地（stage 4 起步前）+ GitHub Release artifact 上传（gh CLI memory `reference_github_accounts.md` workflow）；(c) stage 4 起步前阅读本报告 §8 carve-out 清单 + `docs/pluribus_stage4_*.md`（待 stage 4 [决策] 落地）+ `docs/pluribus_stage4_6_path.md` user 路线 [stage4_6_path] 锚点。**stage 3 closed at stage3-v1.0 tag 工程意义**：stage 1 + stage 2 + stage 3 三阶段 baseline byte-equal 不退化 anchor 全部建立（D-272 + 继承 + D-362 byte-equal 跨 host），stage 4 起步可与 §8.1 carve-out (I)..(VII) 并行推进。
