...
### S6：实时 depth-limited search（2026-06-03 起推进；设计成文 + 6a 核心已落地）

> **状态（2026-06-03）**：原 parked，用户拍板推进。① 设计成文（`temp/realtime_search_design_2026_06_03.md`，
> 含文献调研 + 代码复用映射 + 对抗验证 + 分阶段方案）；② **MVP subgame-solver 核心已落地 + vultr 验证**
> （分支 `6max-rts-mvp` commit `cf9efdb`，**未并入 6max**）。剩 = advisor 插桩 + 不退化 h2h 探针（6a 收尾）→ 6b/6c。
...
**S6-MVP 进度（commit `cf9efdb`，分支 `6max-rts-mvp`，vultr 验证）**：
...
| `SubgameNlheGame impl Game` | `training/subgame.rs` | ✅ 落地（delegate + 重写 root；复用 EsMccfrTrainer 零改）+ 1 端到端 smoke（CFR 跑通 + 可复现）|
| advisor 插桩 `subgame_search` @ `blueprint_advisor.rs:421` + `should_search` | — | ⏳ 未做（6a 收尾）|
| 不退化 h2h 探针（search-on vs blueprint-only，需真 ckpt）| — | ⏳ 未做（6a 收尾）|

- **vultr 验证**：6 新测试全过 + 全 lib 67/67 无回归（byte-equal 守门未破：默认树 240,096 / dense indexer ↔ sizing
  工具 / nlhe 语义 / dense checkpoint 全绿）。本地 build/fmt/clippy `--all-targets -D warnings` 绿。
- **MVP 当前边界**（设计 §10）：uniform range（非 blueprint 加权）、同抽象（无 finer 菜单）、解到终局（无 depth-limit/
  biased leaf）——这些是 6b。
