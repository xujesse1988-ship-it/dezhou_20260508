...
**桶 caveat（决定下一步）**：6-max 暂用 **HU 单对手 equity 桶占位** → game 可端到端构造 + CFR 机制跑通，但
**有意义训练待 S3 多路 equity 桶**（`equity.rs` 假设 1 对手）。**这是训练前真正的 gate、全项目最大未知数**，见 S3。

### S3：多人信息抽象（最大研究点，先验证再训练）

> 前置已就绪（见「S2 续」）：betting-tree 层 + `SimplifiedNlheGame` 参数化（P4）让 6-max A3×A4 game 可端到端
> 构造、CFR 机制跑通。**S3 多路桶是训练前唯一剩余 blocker**（现用 HU 单对手桶占位，无意义）。
