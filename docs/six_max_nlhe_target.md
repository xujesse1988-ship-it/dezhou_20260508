...
③flop → raise 60=**0.5pot bet**(blueprint)。**仅剩 live WS 连通性 smoke**（需用户 api_key + 据真实消息校准
`[LIVE?]` 协议假设，如 `player_action.amount` 是否总 to 额）。
...
**下一步**：① 引擎 + h2h 已闭环（两变体强度相当）；② 客户端已建 + 离线验证 → **S5 共用底座（①②）全就位**。
**唯一未闭 = ② live 连通性 smoke**，需用户：(a) 注册 OpenPoker（`POST api.openpoker.ai/api/register` {name,email,
terms_accepted:true} → api_key 只显示一次）；(b) 把 api_key 给 driver `--api-key` 挂场跑 → 据真实消息校准
`[LIVE?]` 协议假设（`player_action.amount` 总 to 额 / turn_token 字段名等）→ 读排行榜。部署 blueprint 默认
`preopen`（范围更近 GTO，S4续⑥；reshape 两变体 ① 测强度相当）。多对手 AIVAT（S5③）live 拿到方向后再按需上。
