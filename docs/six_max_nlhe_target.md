...
`{1.0,2.0}` 有限可解（树 ×2.14）但 62.03 GiB 需 ≥96 GiB 机；② 起步抽象在 **`{1.0}`（64 GiB 富余、稳）** 与
**`{1.0,2.0}`（表达力更好但需 ≥96 GiB 机 + infoset 2.14× → 训练更久）** 之间二选一，小注在全街全宽抽象下出局
（只能 per-street 加档 + raise cap 才能引入）；③ 参数化 `SimplifiedNlheGame`（`nlhe.rs:310` 硬编码 `n_seats=2`）
到 6 座 + 重建树（注：sizing 不需要它，直接 `GameState` + 6max config 枚举即可；训练才需要）；④ 据最终抽象的
