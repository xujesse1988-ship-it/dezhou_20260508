...
- **S4 gate PASS @ 200M ckpt**（每 baseline 102 万手）：vs random **+3882** [CI +3742,+4021]、call-station
  **+5161** [+5102,+5219]、overly-tight **+301** [+292,+310] mbb/g，三者 CI95 下界全 > 0 → **门槛达成**。
  注：弱 baseline，门槛「必要非充分」——PASS 证 blueprint 没退化、能正常打牌，**不证强**；真质量在 S5 测。
  最终 1B ckpt 再跑一次权威 gate。
- update 数：监控显示 200M 已强收敛（regret 0.0006）；HU "100M 饱和"虽不照搬，但弱 baseline gate 在 200M
  即过。真正「first-usable」强度判据待 S5 强对手评测。
- 单次训练连续 24h 无崩溃 / 无内存泄漏（本 run ~1.7h 验稳定 + RSS 平；长稳留后续；checkpoint 续训已验）。
