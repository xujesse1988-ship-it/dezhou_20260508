...
- `temp/realtime_search_design_2026_06_03.md`：**S6 实时搜索实现方案**（2026-06-03 用户拍板推进 S6）。
  方法定型 + 整体架构 + 逐构件落地 + 分阶段验收 + MVP，见该文档；S6 节给摘要 + 进度。
- 本文档 = 当前主线验收目标。`pluribus_path.md` 的阶段 6（实时搜索）**原 parked，2026-06-03 起推进**
  （见 S6）；阶段 7–8 仍为非目标。
...
**② OpenPoker 客户端已建 + 离线验证**（commits `798b279`/`ebcab41`，§7 步 4）：`tools/openpoker_advisor.rs`
（常驻 Rust advisor，复用 blueprint_advisor off-tree 核，泛化 6 座：座位相对 button rotate 到
`default_6max_100bb` 座 0、OpenPoker 筹码 ×scale=5 换算）+ `tools/openpoker_play.py`（WS driver：Bearer 鉴权 +
join_lobby{2000} + 累计本手历史 + your_turn→advisor→action + §4 码深漂移 leave/rejoin + 断线重连）。
**鲁棒兜底**：任何重放失败（码深漂移 / 结构性 gap / 非 6 人桌 / 非法历史）→ 安全合法动作（check else fold）+
`source=fallback:<reason>`，live 不崩。vultr 验证：advisor 3 单测 + Python driver `--selftest`（真 preopen
blueprint、无网络）3 场景全绿 —— ①folds-to-BTN → raise 45=**2.25BB**(blueprint, raise0.5pot, scale ✓)；
②对手 open-limp → fold(fallback:structural_gap，发现 A 在 ② 兑现：no-limp blueprint 无 limp 节点 → 优雅兜底)；
③flop → raise 60=**0.5pot bet**(blueprint)。**live 连通性 smoke 已过**（2026-06-03，用户 api_key，账号 jesse_xu）：
连接 + Bearer 鉴权 + join_lobby + 入座 + 打 4 手 = **4 决策全 blueprint 驱动（0% 兜底）、0 报错、动作全被接受、
干净退出**（off-tree 重放在近-100BB 桌忠实驱动）。协议据真实消息 + `docs.openpoker.ai/llms-full.txt` 校准完成
（关键修：action 报文 `client_action_id` 须**字符串**，早先 int 被拒 invalid_message；`player_action.amount_mode==to_total`
确认 raise.amount=总 to 额）。**实测确认 §4 码深漂移严重**：同桌栈 14BB–800BB，blueprint 假设 100BB →
off-distribution 手会走兜底（本 4 手恰近 100BB 故 0 兜底；长跑兜底率会升，属已知短板非 bug）。
...
**下一步**：① 引擎 + h2h 闭环（两变体强度相当）；② 客户端 + **live smoke 全过** → **S5 端到端打通**（§7 步 1-4 全完成）。
剩下是「**绝对强度量化**」（非连通性）：挂场跑数百+手累积 mbb/100 + 排行榜位次。两点须先定：
- **码深漂移污染**（§4，已实测严重）：成绩是「近 100BB 场近似」非干净 100BB；报告须按我方开局有效栈分桶、
  标注兜底率；off-distribution 手大量兜底会拉低强度信号。
- **账号/时长**：免费号 1 bot 公开挂场是用户账号长时间在线 → 需用户授权时长；部署 blueprint 默认 `preopen`。
多对手 AIVAT（S5③）在 live 拿到裸 mbb/100 方向后、若 CI 太宽再按需上。

### S6：实时 depth-limited search（2026-06-03 起推进；设计成文 + 6a 已落地）

> **状态（2026-06-03）**：原 parked，用户拍板推进。① 设计成文（`temp/realtime_search_design_2026_06_03.md`）；
> ② **6a 全落地 + §5b range 去 confound + vultr 验证**（分支 `6max-rts-mvp` commit `ac3968b`，**未并入 6max**）：
> MVP subgame-solver + advisor 插桩（`subgame_search` @ L421）+ 不退化探针 + **§5b**（root 按 blueprint 沿历史
> 累乘 reach 的 per-seat marginal range 加权采样 → 探针升级成真正 §2 判别器，用户选路线乙）。smoke 证 plumbing
> 健康（desync=0、search 真触发 fallback 1.5%、range-on/uniform A/B 均无 OOM）。**未闭 = 大样本判决**（600 手 CI
> 无意义，需 vultr 中样本→AWS 大样本收紧 CI）→ 6b/6c。详见设计 §10.2–10.3。

路线 B / `pluribus_path.md` 阶段 6。Pluribus 真正赢人的机制，占总量 30–40%、极易写错。

**方法定型（对抗验证 TRUE）**：走 **Pluribus(Science 2019)/Modicum(NeurIPS 2018) 式 tabular depth-limited
search + 多 biased continuation strategies**；**不**用 DeepStack safe re-solving（gadget/CFV 安全性依赖 2 人
零和，6-max 结构性失效）、**不**用 ReBeL（NN 路线，非目标）。诚实结论：6-max 放弃全部理论保证——但
DeepStack/ReBeL 的保证在一般和**本就 void**，留 tabular 没损失真有效保证。详见设计文档 §1。

**最大前提风险（独立判断）**：续局/搜索前提 = blueprint 已近似均衡，但本仓库 blueprint 自评欠训练（S4 续①
56–65% 覆盖仍爬）→ **搜索可能放大 blueprint 系统偏差而非修正**。MVP 第一要务 = 用「单层 search vs blueprint-only
是否退化」当 blueprint 质量探针（路线乙）。

**架构（低侵入）**：实时搜索只替换决策循环里「查 blueprint 出分布」那一行（`blueprint_advisor.rs:421`），
`outgoing_action`/影子推进不动。subgame solver = 新 `SubgameNlheGame impl Game`（只重写 `root()`），复用
`EsMccfrTrainer`/`VanillaCfrTrainer`（`trainer.rs` 一行不改）。

**关键发现（钉进设计）**：thin-wrapper 被 `GameState` 封装挡住（全字段私有、只 deal-from-scratch、无 in-game
chance）→ 用户拍板加**加性** `GameState::resample_hidden(rng)`（`state.rs`，pub(crate)）：克隆中途态、保留公共牌
+ 下注态、重发隐藏牌（各家底牌 + 未见 runout）、重算 showdown_ranks；终局收益仍走**权威** `payouts()`（side pot/
showdown 一行不动 → **S1 PokerKit 跨验证不受影响**）。`EsMccfrTrainer::step` 每 step 调 `game.root` → resample
即 per-step external chance。

**S6 6a 进度（commit `a8f1b96`，分支 `6max-rts-mvp`，vultr 验证）**：

| 构件 | 文件 | 状态 |
|---|---|---|
| `PublicBettingTree::build_subtree(state, abs, rules, entrants, raises)` | `nlhe_betting_tree.rs` | ✅ 落地（walk 卡牌无关，从中途态建子树）+ 2 cross-check 测试（from-root ≡ 全树 / from-interior ≡ 后代块）|
| `GameState::resample_hidden(rng)`（加性） | `rules/state.rs` | ✅ 落地 + 3 测试（保留态/无重复牌/showdown 自洽、推进终局守恒、byte-equal 可复现）|
| `SubgameNlheGame impl Game` + `root_query` | `training/subgame.rs` | ✅ 落地（delegate + 重写 root；复用 EsMccfrTrainer 零改）+ smoke + 契约测试 |
| advisor 插桩 `subgame_search` @ `blueprint_advisor.rs:421` + `should_search`（flop 未起注首决策点）+ `SearchObserver` | `training/subgame.rs` / `blueprint_advisor.rs` | ✅ 落地（`Contestant.search` 默认 None = byte-equal；失败回落 blueprint）+ search-on 守恒/可复现测试 |
| 不退化 h2h 探针 `tools/six_max_search_probe`（search-on vs search-off + fallback 计数）| `tools/` | ✅ 工具落地 + vultr smoke；**大样本判决待定** |
| **§5b range 去 confound**：`resample_hidden_with_holes` + `estimate_range`（逐街 re-bucket）+ range-weighted root + `--uniform-range` A/B | `rules/state.rs` / `training/subgame.rs` | ✅ 落地（commit `ac3968b`，用户选路线乙）+ 测试 |

- **vultr 验证**：lib **74/0/8** 无回归（byte-equal 守门未破：默认树 240,096 / dense indexer ↔ sizing 工具 / nlhe
  语义 / dense checkpoint 全绿）。本地 build/fmt/clippy `--all-targets -D warnings` 绿。
- **真 1B nolimp ckpt smoke**（600 手，range-on vs `--uniform-range` A/B）：plumbing 健康——**desync=0/illegal=0**、
  search 真触发（66 决策点、fallback 1.5%）、加载无 OOM；mbb/g CI 600 手太宽不可判强弱（两路均跨 0）。子树实测：
  6-max first_small(3) flop=4434 节点 < cap 8000。
...
的具体改动清单 + 量化门槛 + 6 个 Pluribus 易错点见设计文档 §4–§7。

## 明确非目标

- **更新（2026-06-03）**：单层 depth-limited search（6a）**已从非目标转为推进中**（见 S6）。仍未做：多人
  **continual re-solving + biased leaf strategies（6b）** / off-tree 6c 升级（pseudo-harmonic）/ 完整 Pluribus 复现。
