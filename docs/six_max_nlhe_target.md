...
- **训练入口**：`train_cfr` 加 `--profile {hu, six-max}` + `--postflop-cap {2, 3}`。six-max 走
...
- ① **真训练运行**：N=3 生产甜点（树 1.15M 节点 / infoset 230.5M / dense 两表 8.04 GiB@200），vultr 装不下
  （11 GiB 总，N=3 dense ~8 GiB 太紧），按 `feedback_high_perf_host_on_demand` 报预算起 AWS 大机
  （≥ 64 GiB；HU 用 c6a.8xlarge=64 GiB 同档够内存）。update 数预期 10¹⁰–10¹¹ 量级（对标 Pluribus，HU
  "100M 饱和"**不照搬**），按监控曲线（regret 增长 sublinear、漂移收敛）判 first-usable。
- ② **N 座 baseline 评测（S4 gate）**：`nlhe_eval.rs` 当前**硬编码 2 座**（`[SeatId(0), SeatId(1)]` 轮换），
  需推广到 N 座（1 blueprint 座轮遍 6 位 vs 5 baseline）+ 按位置拆收益。门槛：1,000,000 手稳定击败
  random / call-station / tight-aggressive（必要非充分）。与 S5 评测重构同源，可一并做。
- 单次训练连续 24h 无崩溃 / 无内存泄漏（真训练运行时验；checkpoint 续训已验）。
