...
> 含文献调研 + 代码复用映射 + 对抗验证 + 分阶段方案）；② **6a 全落地 + vultr 验证**（分支 `6max-rts-mvp`
> commit `a8f1b96`，**未并入 6max**）：MVP subgame-solver 核心 + advisor 插桩（`subgame_search` @ L421）+ 不退化
> 探针工具（`tools/six_max_search_probe`）。smoke 证 plumbing 健康（desync=0、search 真触发）。**未闭 = step 6
> 大样本判决**（需 100k–1M 手上 AWS，且被 §2 三 confound 削弱）→ 战略岔路（甲 直接 AWS / 乙 先 §5b range 去
> confound）待用户拍板 → 6b/6c。详见设计 §10.2。
...
- **vultr 验证**：lib **71/0/8** 无回归（byte-equal 守门未破：默认树 240,096 / dense indexer ↔ sizing 工具 / nlhe
  语义 / dense checkpoint 全绿）。本地 build/fmt/clippy `--all-targets -D warnings` 绿。
- **真 1B nolimp ckpt smoke**（600 手）：plumbing 健康——**desync=0/illegal=0**、search 真触发（66 决策点、fallback
  1.5%）、加载无 OOM；mbb/g CI 600 手太宽不可判强弱。子树实测：6-max first_small(3) flop=4434 节点 < cap 8000。
- **MVP 当前边界**（设计 §10 / §10.2）：uniform range（非 blueprint 加权）、同抽象（无 finer 菜单）、解到真实终局
  （无 depth-limit/biased leaf/blueprint 叶子值）——故**测不到 §2「搜索放大 blueprint 偏差」**，只测「均匀-range
  全解 vs blueprint」；去 confound 需 §5b range（6a 后续刀）。biased leaf 等是 6b。
