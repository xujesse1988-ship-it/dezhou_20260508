...
- **2026-05-15（F2 [实现] 落地）**：stage 4 F2 [实现] 单 commit 落地 — `src/training/baseline_eval.rs` 3 baseline opponent (`RandomOpponent` / `CallStationOpponent` / `TagOpponent`) `act()` 全部实现 + `evaluate_vs_baseline<G, T, O>` 主路径（6-max 仿真 + opponent seat 0 / blueprint seats 1..n_players + chip pnl 累 mean ± SE + Bessel 校正）；`src/training/slumbot_eval.rs` `SlumbotBridge::new` / `play_one_hand` / `evaluate_blueprint`（HTTP POST `/new_hand` blocking client + JSON parse + 错误路径返 `ProbabilitySumOutOfTolerance` 占位 variant 让 caller 走 D-465 carve-out）+ `OpenSpielHuBaseline::play_one_hand`（HU 退化路径 self-play vs `CallStationOpponent` 占位 baseline）；`src/training/metrics.rs` `MetricsCollector::observe`（cadence 检查 + regret_growth proxy + entropy / oscillation 累更新 + Linux `/proc/self/status` RSS best-effort + 5-variant alarm dispatch）+ `write_metrics_jsonl`（serde_json::to_writer + writeln）；`src/training/trainer.rs` `Trainer` trait 加 `game_ref(&self) -> &G` 方法 + `VanillaCfrTrainer` / `EsMccfrTrainer` 双 impl + `EsMccfrTrainer` 加 `metrics: TrainingMetrics` 字段 + `pub fn metrics(&self) -> &TrainingMetrics` getter（API-472 字面 D-473）+ load_checkpoint dispatch reconstruct metrics zero state；`src/training/nlhe_6max.rs` `NlheGame6` 加 `pub fn config(&self) -> &TableConfig` getter；`tools/eval_blueprint.rs` CLI 主体落地（8 flag — `--checkpoint` / `--artifact` / `--slumbot-endpoint` / `--slumbot-hands` / `--baseline-hands` / `--master-seed` / `--duplicate-dealing` / `--no-slumbot` + 3 baseline × 6-max eval + Slumbot HU eval + JSONL stdout 输出）；5 道 gate 全绿。
    - **覆盖决策清单**：D-460 Slumbot HTTP bridge + D-461 100K 手协议 + duplicate dealing 2-pass + D-462 mbb/g 单位 / 95% CI 字面 + D-463 reqwest blocking 路径 + D-463-revM `OpenSpielHuBaseline` fallback（policy 文件解析 deferred 到 F3 [报告] 一次性 sanity）+ D-465 evaluation infrastructure 失败时返 `ProbabilitySumOutOfTolerance` 占位 variant + D-468 master seed 派生 hand-level seed（splitmix64 finalizer）+ D-469 fold equity metrics（Head2HeadResult 字段不扩展，F3 [报告] handwritten 表覆盖）+ D-470/471/472 3 条监控曲线 proxy 估计 + warn-only alarm dispatch + D-473 trainer 不主动 abort（CLI 走 alarm decision）+ D-474 JSONL log 输出 + D-476 default sample_interval 10⁵ + D-478 EV sum residual 占位（6-traverser 累积 deferred 到 stage 5）+ D-480 3 类 baseline `Opponent6Max` impl + D-481 1M 手 6-max 5 blueprint copies + 1 opponent seat 0 / blueprint seats 1..n_players + D-482 阈值理由继承 + D-483 baseline opponent trait + D-484 BLAKE3 byte-equal regression（splitmix64 + ChaCha20Rng hand-level rng 让重跑 byte-equal）+ D-489 borderline carve-out（TAG ±20% noise 余地通过 F3 [报告] 评估翻面）。
    - **`Trainer::game_ref()` 接口新增**：API-310 surface 既有 `step` / `current_strategy` / `average_strategy` / `update_count` / `save_checkpoint` / `load_checkpoint` 6 method + stage 4 既有 `current_strategy_for_traverser` / `average_strategy_for_traverser` 2 default method 之外，F2 [实现] 加 `game_ref(&self) -> &G` 第 7 必实现方法 让 `evaluate_vs_baseline` / `slumbot_eval` / `metrics::observe` 路径能访问 trainer 内部 game （`G::n_players` / `G::root` / `G::config()` 等）— 不破 API-310 既有签名（继承 D-NNN-revM 模式 additive 扩展），VanillaCfrTrainer + EsMccfrTrainer 2 impl 同 commit 翻面。
    - **§F2-revM 1 处 carve-out**（D-460 Slumbot HTTP schema 占位）：Slumbot `/new_hand` + `/act` HTTP 协议字段具体 schema（`{token, old_action, hole_cards, board, ...}` 形式）by F3 [报告] 起步前用户授权访问 Slumbot API 一次性 lock；F2 [实现] commit 走最小占位 — POST + JSON parse + outcome 提取走 `serde_json::Value`，chip_delta 字段读取 deferred 到具体 endpoint schema 落地；当前实现路径上 chip_delta 返 0（不影响 panic-fail 翻面，因 Slumbot API 不可用时返 `ProbabilitySumOutOfTolerance` 占位让 caller 走 D-465 carve-out fallback）。
    - **`EsMccfrTrainer::metrics()` getter 新增**：API-472 字面 `pub fn metrics(&self) -> &TrainingMetrics` 落地（D-473 trainer 不主动 abort，read-only 接口）。`metrics` 字段初始化为 `TrainingMetrics::zero()`；`load_checkpoint` 路径 reconstruct 同样 zero state（checkpoint 只序列化 regret / strategy_sum / config，metrics 是 in-memory only）；step 路径**不**自动调 `MetricsCollector::observe`（D-473 字面 trainer 不主动 abort，CLI 走 `metrics_interval` cadence 显式调用 — `tools/eval_blueprint.rs` 不消费 metrics（评测路径 read-only），`tools/train_cfr.rs` 整合 metrics 走 stage 5 起步并行清单）。
    - **OOM step-path dispatch 仍 deferred**：`TrainerError::OutOfMemory` variant A1 已落地，F2 [实现] `MetricsCollector::observe` 走 `read_rss_bytes()` `/proc/self/status` best-effort 读取（Linux 平台），但 trainer step 路径未接入 `TrainerError::OutOfMemory` 主动 dispatch（D2 §D2-revM (v) carve-out 字面延续，要求 CLI `--abort-on-alarm p0` flag 接 `MetricsCollector.last_alarm` 走 abort 逻辑，deferred 到 stage 5 起步并行清单 with `tools/train_cfr.rs` 整合）。
    - **byte-equal anchor 不破声明**：F2 [实现] 全部 stage 1 + stage 2 + stage 3 baseline byte-equal 维持（`stage{1,2,3}-v1.0` tag 在 F2 commit 上仍可重跑 byte-equal）；stage 3 1M update × 3 BLAKE3 anchor 维持；stage 4 D2 closure 6-traverser checkpoint round-trip BLAKE3 byte-equal 维持；stage 2 bucket_quality 9 known fail 不退化。
    - **5 道 gate 全绿**：`cargo fmt --all --check` ✅ / `cargo build --all-targets` ✅ / `cargo clippy --all-targets -- -D warnings` ✅（0 warning）/ `RUSTDOCFLAGS="-D warnings" cargo doc --no-deps` ✅ / `cargo test --no-fail-fast` ✅（stage 1/2/3 baseline 维持 + F1 30 #[ignore] opt-in 测试不退化 + 13 active anchor 测试通过 + stage 2 bucket_quality 9 known fail 不退化）。
    - **角色边界**：F2 [实现] 单次 commit 触及 `src/training/baseline_eval.rs`（3 opponent `act` 实现 + `evaluate_vs_baseline` + 6 helper） + `src/training/slumbot_eval.rs`（`SlumbotBridge` 3 方法 + `OpenSpielHuBaseline::play_one_hand` + 1 helper） + `src/training/metrics.rs`（`MetricsCollector::observe` + `write_metrics_jsonl` + `read_rss_bytes` helper） + `src/training/trainer.rs`（`Trainer::game_ref()` trait method + 2 impl + `EsMccfrTrainer::metrics` 字段 + `metrics()` getter + load_checkpoint metrics init） + `src/training/nlhe_6max.rs`（`NlheGame6::config()` getter） + `tools/eval_blueprint.rs`（CLI main body 落地，8 flag + 3 baseline × 6-max + Slumbot HU + JSONL 输出） + `docs/pluribus_stage4_workflow.md`（本节） + `CLAUDE.md`（stage 4 progress 翻面到 "F2 [实现] closed"）。**0 改动** `tests/*` / `Cargo.toml` / `src/error.rs` / `src/training/{checkpoint,game,kuhn,leduc,lbr,nlhe,sampling,best_response,regret,mod}.rs` / `src/abstraction/*` / `src/rules/*` / `src/core/*` / `docs/pluribus_stage4_{validation,decisions,api}.md`（F2 [实现] 不修改测试 / 决策 / API / validation 文档；如发现 spec 错误走 D-NNN-revM 流程，本 commit 未触发）。
    - **F2 → F3 工程契约**：F2 [实现] 落地后 stage 4 first usable `10⁹` update 训练由用户授权 D-491 AWS c7a.8xlarge 启动（wall-time ~4-6h with §E-rev2 batch=8 实测 66K update/s × $1.63/h = ~$6.85 / 14h-budget cost ceiling 远未达）；F3 [报告] 起草 wait for first usable 训练完成 + LBR / Slumbot / baseline 评测落地 + first usable blueprint artifact 上传 GitHub Release（用户手动触发）。如 Slumbot API 访问遇 rate-limit / 维护中 → D-463-revM `OpenSpielHuBaseline` policy 文件解析翻面（F3 [报告] 起步前用户授权 lock）。

- **2026-05-16（§F2-revM 追认 — train_cfr CLI 落地提前到 F3 起步）**：F2 [实现] commit message + workflow §F2 line 564 字面把 `tools/train_cfr.rs` 整合 deferred 到 stage 5；F3 [报告] 起步前 AWS c7a.8xlarge first usable 10⁹ update 训练发现实际无可调用入口（既有 train_cfr.rs = 33 行 stage 3 A1 stub，main 直接 `eprintln + exit(2)`，与 API-490 字面 11 flag spec 不符），用户授权 §F2-revM carve-out 把 train_cfr CLI 落地从 "stage 5 整合" 提前到 "F3 起步同 commit"。**scope**：本 carve-out commit 只覆盖 stage 4 F3 critical path — `--game nlhe-6max --trainer es-mccfr-linear-rm-plus --abstraction pluribus-14` + `--bucket-table` + `--updates` + `--seed` + `--warmup-update-count` + `--threads` + `--parallel-batch-size` + `--checkpoint-dir` + `--checkpoint-every` + `--keep-last` + `--metrics-interval` + `--log-file` + `--max-rss-bytes` + `--abort-on-alarm` + `--resume` + `--quiet` 共 16 flag（API-490 字面 11 flag 全覆盖 + `--bucket-table` / `--seed` / `--threads` / `--parallel-batch-size` / `--quiet` 5 flag 因 stage 3 D-372 既有继承）。Kuhn / Leduc / SimplifiedNlhe stage 3 game variant 走显式 "deferred to stage 5" 错误退出（exit code 2 + stderr 提示），实际 stage 3 训练入口走 `cargo test --release --test cfr_simplified_nlhe -- --ignored` 等集成测试，不消费本 binary（本 carve-out 不重新激活 stage 3 D-372 spec scaffold path）。
    - **覆盖功能清单**：(a) Trainer 构造分流 — `--resume` 路径走 `EsMccfrTrainer::load_checkpoint`，否则 `EsMccfrTrainer::new(NlheGame6::new(table), seed).with_linear_rm_plus(warmup).with_parallel_batch_size(batch)`；(b) RNG pool — `n_threads` 个独立 ChaCha20Rng，per-thread seed = `master_seed.wrapping_add(0xDEAD_BEEF * (tid + 1))`（与 perf_slo / 24h_continuous test 同型 splitmix-style，D-027 字面）；(c) 主循环 dispatch — `single_threaded` (n_threads==1) 走 `trainer.step(rng_pool[0])`，否则走 `trainer.step_parallel(&mut rng_pool, n_threads)`；(d) Metrics cadence — 每 `metrics_interval` update 调 `MetricsCollector::observe` + `write_metrics_jsonl`（log-file 缺省走 stdout 一行 JSON）+ D-431 RSS over-limit 主动 dispatch `TrainingAlarm::OutOfMemory`（CLI 决策，trainer 不主动 abort）；(e) Alarm dispatch — `--abort-on-alarm {none,p0,all}` 三种策略，p0 = `RegretGrowthTrendUp` / `OutOfMemory` / `EvSumViolation` 三类强阻塞 → 写 `_abort.ckpt` 后 exit 1；(f) Checkpoint cadence — 每 `checkpoint_every` update 写 `nlhe6max_linear_rm_plus_t{20-digit-update-count}_auto.ckpt` + `keep_last` 个最新 auto checkpoint mtime-倒序保留（其它 cleanup）；(g) Final checkpoint — 训练结束写 `_final.ckpt`（不参与 keep_last rotation 由用户手动管理）；(h) Progress log — 每 30s 一次 `eprintln!` 实时 throughput / ETA / RSS。
    - **D-431 OOM step-path dispatch 翻面**：F2 [实现] 落地的 `MetricsCollector::observe` 走 `read_rss_bytes()` 已就位但 trainer step 路径未接入，§F2-revM commit `tools/train_cfr.rs::run_nlhe6max` 主循环内 `if metrics.peak_rss_bytes > args.max_rss_bytes { metrics.last_alarm = Some(TrainingAlarm::OutOfMemory { ... }) }` + `--abort-on-alarm p0` 触发 `_abort.ckpt` write + exit 1，**正式翻面 D2 §D2-revM (v) carve-out / F2 line 564 deferred 项**（stage 5 起步并行清单删除该项）。
    - **5 道 gate 全绿**：`cargo fmt --all --check` ✅ / `cargo build --all-targets` ✅ / `cargo clippy --all-targets -- -D warnings` ✅（修 3 处 `doc_list_item_without_indentation` + 1 处 `manual_checked_ops` + 1 处 `unnecessary_sort_by`）/ `RUSTDOCFLAGS="-D warnings" cargo doc --no-deps` ✅ / `cargo test --no-run` ✅。Smoke test：1-CPU 本地 dev box 单线程 `--updates 5000 --metrics-interval 1000 --quiet` 跑通 84 update/s × 5000 update / 60 s + 5 行 metrics.jsonl 全 finite + final checkpoint 11.5 MB schema=2（D2 §D2-revM dispatch 路径）write 成功。
    - **角色边界**：§F2-revM 单次 commit 触及 `tools/train_cfr.rs`（33 行 stub → 565 行 完整实现） + `docs/pluribus_stage4_workflow.md`（本节）+ `CLAUDE.md`（stage 4 progress 翻面到 "§F2-revM closed + F3 起步"）。**0 改动** `src/*` / `tests/*` / `Cargo.toml` / `docs/pluribus_stage4_{validation,decisions,api}.md`（[实现] 不修改 src 库代码 / 测试 / API / 决策 / validation 文档；本 commit 走 "tools/ binary 主体落地" 的 [实现] 任务延伸，与 F2 [实现] commit 同型政策）。
    - **§F2-revM → F3 工程契约**：§F2-revM commit 落地后立即触发 (a) AWS c7a.8xlarge 32-vCPU host 配置（rust 1.95.0 + clone repo + scp v3 artifact 528 MiB）；(b) `target/release/train_cfr --game nlhe-6max --updates 1000000000 --threads 32 --parallel-batch-size 8 --checkpoint-dir artifacts/stage4_first_usable --log-file artifacts/stage4_first_usable/metrics.jsonl --abort-on-alarm p0` 后台启动（wall-time ~4.2h × $1.63/h = ~$6.85，FIXED_SEED `0x53_54_47_34_5F_46_33_18` ASCII "STG4_F3\x18"）；(c) 训练完成后 `target/release/lbr_compute --six-traverser --checkpoint <_final.ckpt>` + `target/release/eval_blueprint --checkpoint <_final.ckpt> [--no-slumbot 如不可用]` 跑 LBR / Slumbot / baseline 三轨评测；(d) 落地 `docs/pluribus_stage4_report.md` + `docs/pluribus_stage4_external_compare.md` + git tag `stage4-v1.0`。如 Slumbot API 访问遇 rate-limit / 维护中 → D-463-revM `OpenSpielHuBaseline` policy 文件解析翻面（用户授权 lock）。
