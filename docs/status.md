...
| 简化 NLHE ES-MCCFR | ⚠️ 默认 action profile 为 `0.5/0.75/1.0/2.0 pot + all-in`；起始筹码 profile 支持 `100BB` / `200BB`（Slumbot 对齐）；checkpoint schema v5，旧 profile checkpoint 不兼容需重训；InfoSetId v2 layout（hand_bucket / node_id / street_tag）跨街抽象动作历史单射仍保留 | `src/abstraction/action.rs` + `src/training/nlhe.rs` + `src/training/nlhe_betting_tree.rs` + `tests/cfr_simplified_nlhe.rs` |
...
- 简化 NLHE action / stack profile 更新（2026-05-19）：
  - 默认 bet/raise ratio = `0.5 / 0.75 / 1.0 / 2.0 pot`，外加 all-in。
  ...
  - checkpoint `SCHEMA_VERSION = 5`；NLHE checkpoint 兼容 fingerprint 包含 bucket table
    hash + action profile + stack profile + table config。旧 v4/v3/v2 checkpoint 当前代码会拒绝加载，
    需要按新 profile 重训。
  - 100BB 抽象 betting tree 节点数 = 361,424（node_id 19 bit，max depth 14）；
    200BB 节点数 = 1,403,936（node_id 21 bit，max depth 15），均小于当前
    InfoSetId v2 的 26-bit node_id 字段。
