...
preopen 0.68×(157.9M/5.46GiB)。**nolimp 1B 已跑完(vultr,wall 7.5h)并验证诊断**:删 limp 的 4 个非盲位 preflop
...
**当前 = preopen 待 AWS**(加 2.25BB 档,修 SB + 放宽近 GTO);S5 评测(强参考对手缺口 + `nlhe_eval` 推广)未动。
详见 `six_max_nlhe_target.md` S4 / S4 续。
