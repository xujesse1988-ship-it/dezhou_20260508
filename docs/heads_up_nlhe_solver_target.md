...
- `有效筹码`：求解器仍以有限有效筹码为状态变量，默认 `200BB`（与 Slumbot 等 heads-up 参考 bot 对照对齐，见 H4）；后续可扩展 `50BB`、`100BB` 或更深筹码 profile。
...
- 新增或明确 `TableConfig::default_hu_200bb()`。
...
当前求解器按 No-Limit 德州扑克处理下注空间，但仍使用有限有效筹码 profile。默认 `200BB`，与 Slumbot heads-up 对照对齐（见 H4 验收）。数学意义上的无限 stack 不进入当前目标。
