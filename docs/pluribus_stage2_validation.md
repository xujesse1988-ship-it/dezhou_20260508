...
- **2026-05-09（C2 [实现] 关闭同步）**：C2 [实现] 落地全部 6 类产品代码（详见 `pluribus_stage2_workflow.md` §修订历史 §C-rev1 §1..§3 / `pluribus_stage2_api.md` §修订历史 C2 关闭节）。本文档 §3 `Bucket 占用` / `Bucket 内方差上限` / `Bucket 间距下限` / `Bucket 序号单调性` 4 条 path.md 字面验收门槛 + §通过标准 line 124 字面 "每条街 0 空 bucket、bucket 内 EHS std dev 全部 < 0.05、相邻 bucket 间 EMD 全部 ≥ T_emd = 0.02、bucket id ↔ EHS 中位数单调一致"，**在 C2 hash-based canonical_observation_id 设计下不可达**——FNV-1a 32-bit hash mod N（C2 收紧到 flop=3K / turn=6K / river=10K）是 approximate canonical id，与 D-218-rev1 字面要求的 *联合花色对称等价类唯一 id* 在 hash 碰撞场景下不严格等价。`tests/bucket_quality.rs` 12 条质量门槛断言（4 dim × 3 街）经 §C-rev1 §2 carve-out 保留 `#[ignore]` 标注 + 早返回 stub，待 stage 3+ true equivalence class enumeration（D-218-rev2，~25K flop 等价类 + 查表 + Pearson hash 完整化）落地后取消 stub 重新启用。本文档不修改 §3 / §通过标准 字面（这些是 path.md 锁定的最终验收门槛，仍为 stage 2 出口必达），仅在本节标注 C2 carve-out 状态：**§3 4 条质量门槛验收延迟到 stage 3+ true equivalence class enumeration 落地**；其它出口标准（preflop 169 lossless / clustering BLAKE3 byte-equal / 1M mapping determinism / mmap schema / corrupted byte flip / 错误路径 / 性能 SLO）C2 路径下完整满足。`cargo test`（默认）`187 passed / 34 ignored / 0 failed across 25 test crates`；`cargo test --release -- --ignored` 全绿（含 32-seed bucket table baseline ~5 min release）；`cargo fmt / clippy / doc / build` 全绿；stage 1 baseline `104 passed / 19 ignored` byte-equal 不退化（D-272）。**Carve-out 取消标准**（stage 3+）：D-218-rev2 真等价类落地 → `canonical_observation_id` collision-free + lookup_table[obs_id] 唯一映射 → bucket_quality.rs 12 条断言取消 stub + ignore，验证 4 类质量门槛全绿。
