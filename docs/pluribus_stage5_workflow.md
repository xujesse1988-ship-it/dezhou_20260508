...
- **batch 4**（commit 309d8d4）= workflow §10 13-step commit checklist 字面 entry/exit 全套 + §4 carry-forward 9 项最终分流（2 closed + 3 主线触发 + 2 并行清单 + 2 stage 6）+ CLAUDE.md stage 5 段 A0 closure + A1 [实现] scaffold 准备完成。

stage 5 A0 [决策] **闭合**。后续 §X-revN carve-out 按 stage 1-4 同型 flow append。

- **2026-05-16（A1 [实现] scaffold 落地）**：stage 5 A1 [实现] 单 commit 4d67e24 — 4 新 module（`src/training/{regret_compact,quantize,shard,pruning}.rs`）+ 1 新 binary stub（`tools/perf_baseline.rs`）+ `trainer.rs` 加 `EsMccfrLinearRmPlusCompactTrainer<G>` 字段集字面 lock + 7 必实现方法 + 3 builder + 9 getter 全 `unimplemented!()` 占位 + `checkpoint.rs` 加 `SCHEMA_VERSION_V3` / `HEADER_LEN_V3` / `STAGE3_HEADER_LEN` / `STAGE4_HEADER_LEN` 常量 + `CheckpointHeaderV3` struct 16 字段字面锁 + `ensure_trainer_schema` preflight 函数实落地 + `TrainerVariant::from_u8(3)` 识别 + `error.rs` 加 `TrainerVariant::EsMccfrLinearRmPlusCompact = 3` + `expected_schema_version` const fn + `metrics.rs::TrainingMetrics` 加 14 stage 5 新字段全 `0` 起步 + `tests/api_signatures.rs` 扩 `_stage5_api_signature_assertions()` 全 API-500..API-599 trip-wire + `Cargo.toml` +memmap2 + rustc-hash dep + `[[bin]] perf_baseline` 注册。5 道 gate 全绿 + stage 1+2+3+4 baseline byte-equal 维持。
- **2026-05-16（B1 [测试] 落地）**：stage 5 B1 [测试] 单 commit 落地 — 6 新 test 文件 / 模块共 71 新测试覆盖（22 active default-pass + 49 `#[ignore]` opt-in）+ 5 道 gate 全绿 + stage 1+2+3+4 baseline byte-equal 维持。
    - **新 test 文件落地（6 文件）**：
        * `tests/regret_table_compact_unit.rs`（20 测试 — RegretTableCompact + StrategyAccumulatorCompact Robin Hood probe + q15 quantization 内嵌 + RM+ clamp + Linear discounting lazy + iter / len / section_bytes D-544 公式 + renormalize_scales + collision_metrics D-569 三阈值 + D-517 字面 keys 不共享）— 18 `#[ignore]` opt-in（A1 stub `unimplemented!()` 走 B2 [实现] 落地后转 pass）+ 2 active sanity（CollisionMetrics struct + PhantomData lock）。
        * `tests/quantize_unit.rs`（10 测试 — `f32_to_q15` round-trip / `q15_to_f32` 零输入 / saturating clamp / `scale == 0.0` defensive / `compute_row_scale` max(|row|) / 全零 row 返 0 / `quantize_row` padding `[14..16] = i16::MIN` / `dequantize_row` 忽略 padding / `dequantize_action` 单 action byte-equal row path / 整行 round-trip 精度）— 10 `#[ignore]` opt-in（B2 [实现] 落地后转 pass）。
        * `tests/pruning_state.rs`（9 测试，D-527 字面 5 + 额外 active 4）：(1) `pruning_threshold_negative_300m_inline_check` / (2) `resurface_period_10m_iter_full_scan` / (3) `resurface_epsilon_0_05_proportional_reactivation` / (4) `pruning_warmup_boundary_1m_update_no_prune_before` / (5) `pruning_off_equivalent_to_stage4_path_byte_equal_lbr` 5 `#[ignore]` opt-in（E2 [实现] 落地后转 pass）+ (6) `pruning_config_default_values_match_d520_d521_literal` D-520 / D-521 字面值 lock active / (7) `resurface_metrics_default_zeroes` Default impl active / (8) `resurface_rng_derivation_deterministic_per_pass_id` D-528 RNG 派生 reproducibility active / (9) `pruning_path_phantom_data_lock` PhantomData type marker active。
        * `tests/checkpoint_v3_round_trip.rs`（13 测试 — API-596 D-549 / D-563 字面 schema_version 2 → 3 翻面 + HEADER_LEN_V3 192 byte + 8 新 header field + 6×2 sub-region body BLAKE3 self-consistency + 跨 binary 拒绝）— 8 active sanity（SCHEMA_VERSION_V3 / HEADER_LEN_V3 / STAGE3_HEADER_LEN / STAGE4_HEADER_LEN 常量 lock + `CheckpointHeaderV3::zero_with_lock` 16 字段字面值 lock + `ensure_trainer_schema` Ok / Err / `TrainerVariant::EsMccfrLinearRmPlusCompact.expected_schema_version() = 3`）+ 5 `#[ignore]` opt-in（D2 [实现] 落地 `Checkpoint::save_schema_v3` + `parse_bytes_v3` + body BLAKE3 self-consistency + 三路径 dispatch + 跨 binary version 拒绝后转 pass）。
        * `tests/stage5_anchors.rs`（10 测试 — API-597 D-560..D-563 4 anchor 实测覆盖 + D-564..D-569 measurement protocol）— 5 active sanity（D-560 LBR 阈值字面 baseline × 1.05 / D-561 baseline 3 类阈值字面 baseline × {0.9 / 0.8 / ±100 mbb} / D-562 Slumbot CI lower 字面 lock / D-566 anchor baseline JSON 路径 + stage 4 first usable checkpoint SHA256 hex lock）+ 5 `#[ignore]` opt-in（F1 [测试] / F3 [报告] c6a.8xlarge host 实测 4 anchor 全 PASS 后转 pass + D-567 字面合成判定）。
        * `tests/regret_table_compact_collision.rs`（8 测试 — API-598 D-569 collision metrics anchor）— 3 active sanity（D-569 3 阈值字面 0.75 / 16 / 2.0 lock + `CollisionMetrics` field layout lock + 空表 sentinel 满足全部阈值）+ 5 `#[ignore]` opt-in（E2 [实现] 落地 + c6a host run 1M warm-up / 10M steady-state snapshot + 6 traverser independent / shard production path / alarm dispatch 后转 pass）。
    - **既有 test 文件扩展（1 文件）**：`tests/perf_slo.rs` 追加 3 新 stage 5 `#[test] #[ignore]` 函数（API-594 字面）— `stage5_compact_regret_table_throughput_c6a_32vcpu_geq_200k` D-530 SLO + `stage5_compact_regret_table_memory_geq_50_percent_reduction` D-540 SLO + `stage5_compact_regret_table_collision_metrics_within_bounds` D-569 anchor。3 个 `#[ignore]` opt-in 全 panic-fail（A1 stub trainer 路径 `unimplemented!()`），E2 [实现] 落地 + c6a host run 后转 pass。
    - **§stage5-rev0 carve-out（A1 [实现] 跟进 ignore）**：stage 5 A1 commit 4d67e24 把 `TrainerVariant::EsMccfrLinearRmPlusCompact = 3` 加 + `from_u8(3) -> Some(EsMccfrLinearRmPlusCompact)`，破坏 3 个 stage 4 baseline test 的"tag 3 未占用"字面断言：
        * `tests/checkpoint_corruption.rs::trainer_variant_unknown_tag_3_returns_corrupted`（dispatch 路径下 tag 3 走 EsMccfrLinearRmPlusCompact 不再 Corrupted）。
        * `tests/checkpoint_v2_schema.rs::trainer_variant_es_mccfr_linear_rm_plus_tag_is_2`（含 `from_u8(3) == None` 字面断言失效）。
        * `tests/checkpoint_v2_schema.rs::checkpoint_variant_cardinality_anchor`（含 `from_u8(3) == None` 字面断言失效）。
        本 commit 给这 3 个 test 加 `#[ignore = "§stage5-rev0 ..."]` 沿用 §D2-revM (i) dispatch carve-out 同型模式（dispatch 语义翻面后既有 anchor 不可达，留待 stage 5 D1 [测试] 起步前 re-author 完整 4-variant cardinality anchor，已部分由 `tests/checkpoint_v3_round_trip.rs::trainer_variant_es_mccfr_linear_rm_plus_compact_expected_schema_3` 承接）。3 处 `#[ignore]` minimal edit 走 §stage5-rev0 procedural follow-through carve-out — A1 [实现] commit 漏 ignore，B1 [测试] commit 补回；属同型 stage 4 §F2-revM `tools/train_cfr.rs` A1 stub 落地提前到 F3 起步同 commit 翻面的 procedural follow-through 模式（用户授权阈不要求重新征求，因为是 A1 漏 ignore 的修复而非新 D-NNN-revM 翻语义）。
    - **5 道 gate 全绿**：`cargo fmt --all --check` ✅ / `cargo build --all-targets` ✅ / `cargo clippy --all-targets -- -D warnings` ✅（修 1 处 `manual_range_contains` + 1 处 `needless_range_loop` + 1 处 `assertions_on_constants` 走 `const _: () = { assert!(...) }` 路径 + 3 处 `doc_lazy_continuation` 段落分隔订正）/ `RUSTDOCFLAGS="-D warnings" cargo doc --no-deps` ✅ / `cargo test --no-run` ✅。stage 1 + 2 + 3 + 4 baseline 测试套件 byte-equal 维持（B1 [测试] 0 改动 `src/*` / `tools/*` / `Cargo.toml`；§stage5-rev0 carve-out 仅给 3 个既有 test 加 `#[ignore]` 不改 test 主体）。
    - **角色边界**：B1 [测试] 单次 commit 触及 6 新 test 文件 + `tests/perf_slo.rs`（扩 3 个 stage 5 SLO `#[ignore]` 函数）+ `tests/checkpoint_corruption.rs`（§stage5-rev0 minimal edit）+ `tests/checkpoint_v2_schema.rs`（§stage5-rev0 minimal edit ×2）+ `docs/pluribus_stage5_workflow.md`（本节）。**0 改动** `src/*` / `tools/*` / `Cargo.toml` / `docs/pluribus_stage5_{validation,decisions,api}.md`（B1 [测试] 不修改决策 / API / validation 文档；如发现 spec 错误走 D-NNN-revM 流程，本 commit 未触发）。
    - **B1 → B2 工程契约**：B1 [测试] 落地后 B2 [实现] 起步前必 lock (a) `src/training/regret_compact.rs` `RegretTableCompact::with_initial_capacity_estimate` / `regret_at` / `add_regret` / `clamp_rm_plus` / `scale_linear_lazy` / `iter` / `section_bytes` / `renormalize_scales` / `collision_metrics` 全 `unimplemented!()` 翻面落地（Open-addressed Robin Hood + FxHash + load factor 0.75 + SoA 三 Vec + capacity 2^20 起步，D-510 字面）；(b) `src/training/quantize.rs` `f32_to_q15` / `q15_to_f32` / `compute_row_scale` / `quantize_row` / `dequantize_row` / `dequantize_action` 全 helper 翻面落地（D-511 字面 per-row scale + scale-only lazy decay）；(c) `StrategyAccumulatorCompact` 同型 9 方法翻面；(d) c6a.8xlarge host 实测 A 项（紧凑 array）+ B 项（q15 quantization）throughput delta + D-571 / D-572 gate 评估（≥ 20% / ≥ 12% compound vs A）。B2 [实现] commit 触及 `src/training/{regret_compact,quantize}.rs` + 实测数字 commit message；**0 改动** `tests/*`（继承 stage 1/2/3/4 [实现] 角色边界）+ `docs/pluribus_stage5_{validation,decisions,api}.md`（B2 [实现] 不修改决策 / API / validation 文档；如发现 spec 错误走 D-NNN-revM 流程）。
