...
//! 返回 `SigmaVec` = `SmallVec<[f64; 12]>`，让 current NLHE default action profile /
...
pub(crate) type SigmaVec = SmallVec<[f64; 12]>;
...
impl<I: Eq + Hash + Clone> RegretTable<I> {
    ...
    /// 热路径变体：返回 `SigmaVec` = `SmallVec<[f64; 12]>`，让当前 NLHE 默认动作集
    /// / 2-action（Kuhn）/ 3-action（Leduc）路径走 stack alloc，规避
...
/// - 同 InfoSet 多次访问（traverser enumerate 偶发触发）按 push 顺序
...
impl<I> LocalRegretDelta<I> {
    ...
    /// 顺序 playback）。`delta` 调用方持有 `SmallVec<[f64; 12]>`（`SigmaVec`），
    /// 本方法直接 owned 接收避免再分配。
