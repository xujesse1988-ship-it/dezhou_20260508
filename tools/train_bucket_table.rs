...
//! 同 `(seed, BucketConfig, cluster_iter)` 输出的文件 byte-equal（D-237）。

use std::process::ExitCode;
use std::sync::Arc;

use poker::eval::NaiveHandEvaluator;
use poker::{BucketConfig, BucketTable, HandEvaluator};
...
fn main() -> ExitCode {
    ...
    eprintln!(
        "[train_bucket_table] seed={:#018x} bucket_config=({}/{}/{}) cluster_iter={} output={:?}",
        opts.seed, opts.flop, opts.turn, opts.river, opts.cluster_iter, opts.output
    );
    ...
    let table = BucketTable::train_in_memory(config, opts.seed, evaluator, opts.cluster_iter);
...
fn print_usage(args: &[String]) {
    ...
    eprintln!(
        "usage: {prog} --output <path> [--seed <u64>] [--flop <N>] [--turn <N>] [--river <N>] [--cluster-iter <N>]\n\
         \n\
         Defaults: seed=0xCAFEBABE, flop=turn=river=500, cluster-iter=10000.\n\
